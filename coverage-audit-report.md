...
**Date:** 2026-05-21 **Auditor:** Sisyphus **Scope:** TypeScript (vitest/v8) +
...
consuming 100% CPU. This is a known issue documented in `AGENTS.md`.
...
TypeScript and Python coverage are run separately with no aggregation. There is
no single dashboard or consolidated coverage metric.
...
1. **Fix the hanging test** — Identify and isolate the CPU-bound test blocking
   full coverage runs
2. **Add coverage thresholds** — Set `coverage.thresholds` in `vitest.config.ts`
   to enforce minimum coverage
3. **Consolidate reporting** — Create a single script that runs both TS and
   Python coverage and merges results
4. **Cover `index.ts`** — The `src/lib/utils/index.ts` file shows 0% coverage;
   add re-export tests
5. **CI integration** — Add coverage reporting to CI pipeline with trend
   tracking
