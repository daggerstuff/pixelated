...
3.  **Plan**: Draft in `[REDACTED]/plans/` (`YYYY-MM-DD-task-name.md`).
...
- `[REDACTED]/`: **PRIVATE** internal plans, research, and notes (git-ignored).
...
- The `.env` file is not committed. Copy `.env.example` to `.env`
  ...
  external service credentials but some features/tests require them.
- `pnpm lint` exits with code 1 due to pre-existing errors.
  Use `pnpm lint:ci` for a non-blocking variant.
