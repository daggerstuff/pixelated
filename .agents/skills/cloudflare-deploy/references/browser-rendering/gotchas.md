...
**Workers vs REST:** REST auto-closes after timeout. Workers must call `close()`
or session stays open until `keep_alive` expires.
...
**Session reuse:** Cold start ~1-2s, warm connect ~100-200ms. Store sessionId in
KV for reuse.
