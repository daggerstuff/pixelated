...
Client-side JavaScript API for tracking events, setting properties, and managing
consent.
...
**Params:** `eventName` (string), `properties` (object, optional).
Fire-and-forget.
...
Properties persist for page session. Use for user identification and
segmentation.
...
**Events:** `Product Viewed`, `Product Added`, `Product Removed`, `Cart Viewed`,
...
**Flow:** Configure purposes in dashboard → Map tools to purposes → Show
modal/set programmatically → Tools fire when allowed
