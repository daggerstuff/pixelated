...
**Body inspection:** Supports `application/json`, `*/*`, `application/*`.
...
Critical for BOLA Detection, Sequence Mitigation, and analytics. Configure
header/cookie that uniquely IDs API users.
...
Detects Broken Object Level Authorization attacks (enumeration + parameter
pollution).
...
- Enable per-endpoint monitoring, set thresholds, action: Log | Challenge |
  Block
