...
Create token: Dashboard → Workers → API tokens → Create with Pipeline Send
permission
...
**String types for CAST:** `string`, `int32`, `int64`, `float32`, `float64`,
`bool`, `timestamp`
...
**Note:** Iceberg tables support standard SQL queries with GROUP BY, JOINs,
WHERE, ORDER BY, etc.
