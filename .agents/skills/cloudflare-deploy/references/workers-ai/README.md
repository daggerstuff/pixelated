...
Expert guidance for Cloudflare Workers AI - serverless GPU-powered AI inference
at the edge.
...
- 50+ pre-trained models (LLMs, embeddings, image generation, speech-to-text,
  translation)
...
**Architecture**: Inference runs on Cloudflare's GPU network. Models load on
first request (cold start 1-3s), subsequent requests are faster.
...
- Use `@cf/meta/llama-3.1-8b-instruct` or `@cf/meta/llama-3.1-70b-instruct`
  (native tool support)
...
- **Stable Diffusion**: `@cf/stabilityai/stable-diffusion-xl-base-1.0` (~10,000
  neurons)
...
**When**: Using Vercel AI SDK features (streaming UI, tool calling
abstractions)  
...
**Pricing**: Free 10K neurons/day, then pay per neuron consumed (varies by
model)
...
- [configuration.md](./configuration.md) - wrangler.jsonc setup, TypeScript
  types, bindings, environment variables
- [api.md](./api.md) - env.AI.run(), streaming, function calling, REST API,
  response types
- [patterns.md](./patterns.md) - RAG with Vectorize, prompt engineering,
  batching, error handling, caching
- [gotchas.md](./gotchas.md) - Deprecated @cloudflare/ai package, rate limits,
  pricing, common errors

## See Also

- [vectorize](../vectorize/) - Vector database for RAG patterns
- [ai-gateway](../ai-gateway/) - Caching, rate limiting, analytics for AI
  requests
