...
Complete API reference for the Cloudflare Workers TCP Sockets API
(`cloudflare:sockets`).
...
DNS names are resolved at connection time. IPv4, IPv6, and private IPs (10.x,
172.16.x, 192.168.x) supported.
...
**`allowHalfOpen`:** When `false` (default), closing read stream auto-closes
write stream. When `true`, streams are independent.
...
Upgrades connection to TLS. Only available when `secureTransport: "starttls"`
was specified.
...
See [patterns.md](./patterns.md) for multi-chunk reading, error handling, and
...
- [configuration.md](./configuration.md) - Wrangler setup and environment
  variables
