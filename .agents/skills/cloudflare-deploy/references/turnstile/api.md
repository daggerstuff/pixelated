...
The Turnstile JavaScript API is available at `window.turnstile` after loading
the script.
...
- `options` (TurnstileOptions): Configuration object (see
...
Resets a widget (clears token, resets challenge state). Useful when form
validation fails.
