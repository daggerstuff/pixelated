...
**Playground is for rapid prototyping only.** For production apps, use
`wrangler` CLI.
...
**Note:** Deployed Workers are production-ready but start on Free plan (100k
requests/day).
...
**Note:** DevTools show client-side console, not Worker execution logs. For
production logging, use Logpush or Tail Workers.
...
**Exceeding CPU time** throws error immediately. Optimize hot paths or upgrade
to Paid plan (50ms CPU).
