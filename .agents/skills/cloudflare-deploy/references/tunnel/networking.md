...
**Note:** Use DNS resolution for `*.argotunnel.com` rather than hardcoding IPs.
Cloudflare may add edge locations.
...
Cloudflared honors proxy environment variables (`HTTP_PROXY`, `HTTPS_PROXY`,
`NO_PROXY`).
...
**Large file transfers:** Use R2 or Workers with chunked uploads instead of
streaming through tunnel.
