...
Insert/upsert/delete return immediately but vectors aren't queryable for 5-10
seconds.
...
`returnMetadata: "indexed"` returns only first 64 bytes of strings. Use `"all"`
for complete metadata (but max topK drops to 20).
...
Cannot change dimensions/metric after creation. Must create new index and
migrate.
