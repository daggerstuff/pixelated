...
Starts Worker with real local bindings for integration tests. Stable API
(replaces `unstable_startWorker`).
...
Use for unit tests (test functions, not full Worker) or scripts that need
bindings.

## Type Generation

Generate types from config: `wrangler types` → creates
