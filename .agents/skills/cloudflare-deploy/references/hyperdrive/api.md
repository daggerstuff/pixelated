...
[configuration.md](./configuration.md) for setup.
...
**Generate types:** `npx wrangler types` (auto-creates worker-configuration.d.ts
from wrangler.jsonc)
...
**⚠️ Workers connection limit: 6 per Worker invocation** - use connection
pooling wisely.
...
**⚠️ `prepare: true` is enabled by default and required for Hyperdrive
caching.** Setting to `false` disables prepared statements + cache.
...
**⚠️ MySQL support is less mature than PostgreSQL** - expect fewer optimizations
...
See [patterns.md](./patterns.md) for use cases, [gotchas.md](./gotchas.md) for
limits.
