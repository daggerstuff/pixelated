...
Expert guidance for Cloudflare R2 SQL - serverless distributed query engine for
...
R2 SQL is Cloudflare's **serverless distributed analytics query engine** for
querying Apache Iceberg tables in R2 Data Catalog. Features:
...
- **Log analytics** - Query application/system logs with WHERE filters and
  aggregations
...
- **Workers/Pages runtime** - R2 SQL has no Workers binding, use HTTP API from
  external systems
- **Real-time queries (<100ms)** - Optimized for analytical batch queries, not
  OLTP
- **Complex joins/CTEs** - Limited SQL feature set (no JOINs, subqueries, CTEs
  currently)
