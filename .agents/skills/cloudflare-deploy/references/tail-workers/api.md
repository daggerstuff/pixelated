...
**CRITICAL:** Tail handlers don't return values. Use `ctx.waitUntil()` for async
operations.
...
**Note:** Official SDK uses `TraceItem`, not `TailItem`. Use
`@cloudflare/workers-types` for accurate types.
