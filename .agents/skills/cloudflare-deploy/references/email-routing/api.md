...
- `forward(rcptTo, headers?)`: Forward to verified destination, optionally add
  headers
...
`subject`, `from`, `to`, `x-cf-spamh-score` (spam score), `message-id`
(deduplication), `dkim-signature` (auth)
...
- **From address**: Must be on verified domain (your domain with Email Routing
  enabled)
