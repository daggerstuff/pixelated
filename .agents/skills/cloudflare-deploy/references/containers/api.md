...
Called when container process starts (ports may not be ready). Runs in
`blockConcurrencyWhile` - no concurrent requests.
...
Called when SIGTERM received. 15 minutes until SIGKILL. Use for graceful
shutdown.
...
Called when `sleepAfter` timeout reached. Return `true` to stay alive, `false`
to stop.
...
**⚠️ Use `getState()` for external checks, `ctx.container.running` for
internal.**
