...
**Generate types from wrangler.jsonc** (replaces deprecated
...
Creates `worker-configuration.d.ts` with typed `Env` interface based on your
bindings.
...
**Note:** If overriding `vars`, `kv_namespaces`, `d1_databases`, etc., ALL must
be redefined (non-inheritable)
