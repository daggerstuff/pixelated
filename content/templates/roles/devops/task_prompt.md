...
2. Make changes idempotent — running the pipeline twice should produce the same
   result
3. Pin tool versions explicitly; never use `latest` tags in production configs
4. Test pipeline changes locally before committing (e.g. `act` for GitHub
   Actions)
5. Document any manual steps required (secrets, env vars, one-time setup) in a
   comment
