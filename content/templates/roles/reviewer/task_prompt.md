...
2. Check for: correctness, type safety, missing error handling at system
   boundaries, test coverage
3. Classify each finding: `blocking` (must fix before merge) or `non-blocking`
   (suggestion)
4. Provide concrete fix suggestions with code, not just descriptions of the
   problem
5. Run the test suite to verify current state:
