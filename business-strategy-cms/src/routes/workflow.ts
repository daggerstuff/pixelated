...
type WorkflowCreateRequest = AuthenticatedRequest<
  Record<string, string>,
  unknown,
  WorkflowCreateBody
>
type WorkflowSubmitRequest = AuthenticatedRequest<
  Record<string, string>,
  unknown,
  WorkflowSubmitBody
>
type WorkflowActionRequest = AuthenticatedRequest<
  Record<string, string>,
  unknown,
  WorkflowActionBody
>
type WorkflowCommentRequest = AuthenticatedRequest<
  Record<string, string>,
  unknown,
  WorkflowCommentBody
>
type WorkflowSearchRequest = AuthenticatedRequest<
  Record<string, string>,
  unknown,
  unknown,
  WorkflowSearchQuery
...
router.get(
  ...
  async (_req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Failed to fetch workflow templates' })
    }
  },
)
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Failed to fetch workflow template' })
    }
  },
)
...
router.post(
  '/instances',
  authenticateToken,
  async (req: WorkflowCreateRequest, res: Response) => {
    try {
      ...
      const priority =
        toWorkflowPriority(req.body.priority) ?? ReviewPriority.MEDIUM
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const instances =
        WorkflowService.getWorkflowInstancesForDocument(documentId)
      return res.json(instances)
    } catch {
      return res
        .status(500)
        .json({ error: 'Failed to fetch workflow instances' })
    }
  },
)
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!instanceId) {
        return res
          .status(400)
          .json({ error: 'Workflow instance ID is required' })
      }
      ...
      return res
        .status(500)
        .json({ error: 'Failed to fetch workflow instance' })
    }
  },
)
...
router.get(
  '/instances',
  authenticateToken,
  async (req: WorkflowSearchRequest, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Failed to search workflow instances' })
    }
  },
)
...
router.post(
  '/instances/:id/submit',
  authenticateToken,
  async (req: WorkflowSubmitRequest, res: Response) => {
    try {
      const instanceId = toString(req.params['id'])
      if (!instanceId) {
        return res
...
router.post(
  '/instances/:id/action',
  authenticateToken,
  async (req: WorkflowActionRequest, res: Response) => {
    try {
      const instanceId = toString(req.params['id'])
      if (!instanceId) {
        return res
...
router.post(
  ...
  authenticateToken,
  async (req: WorkflowCommentRequest, res: Response) => {
    try {
      const instanceId = toString(req.params['id'])
      if (!instanceId) {
        return res
...
router.get(
  ...
  async (_req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Failed to fetch overdue workflows' })
    }
  },
)
