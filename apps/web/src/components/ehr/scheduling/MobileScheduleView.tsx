...
export function MobileScheduleView({
  ...
  const [appointments, setAppointments] =
    useState<ScheduleAppointmentItem[]>(initialAppointments)
  const [queuedActions, setQueuedActions] = useState<QueuedAppointmentAction[]>(
    () => offlineSyncService.getQueuedAppointmentActions(),
  )
  const [selectedDate, setSelectedDate] = useState<string>(
    new Date().toISOString().split('T')[0],
  )
  const [filterView, setFilterView] = useState<'day' | 'upcoming' | 'all'>(
    'day',
  )
  const [isOnline, setIsOnline] = useState(
    () => offlineSyncService.getStatus().isOnline,
  )
  ...
  useEffect(() => {
      ...
      void (async () => {
        try {
          const res = await fetch(
            `/api/portal/v1/scheduling?date=${selectedDate}`,
          )
          if (!cancelled && res.ok) {
    ...
    const unsubOffline = offlineSyncService.on('offline', () =>
      setIsOnline(false),
    )
  ...
  const handleBookAppointment = async (e: React.FormEvent) => {
    ...
    const optimisticItem: ScheduleAppointmentItem = {
      ...
      patientId: currentPatientId,
      practitionerId: bookingPractitioner,
      practitionerName: DEFAULT_PRACTITIONERS.find(
        (p) => p.id === bookingPractitioner,
      )?.name,
      start,
      end,
      status: 'pending',
  ...
  return (
    <div
      className="w-full max-w-full space-y-4 font-sans"
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            className="focus:ring-lime-500 min-h-[44px] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2"
            style={{
              background: 'var(--np-surface)',
              color: 'var(--np-text)',
              border: '1px solid var(--np-line)',
        ...
        <div
          ...
          style={{ background: 'var(--np-surface)' }}
        >
          {(['day', 'upcoming', 'all'] as const).map((view) => (
            <button
              ...
              style={{
                background:
                  filterView === view ? 'var(--np-elevated)' : 'transparent',
                color:
                  filterView === view ? 'var(--np-text)' : 'var(--np-muted)',
              }}
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="flex justify-center p-8">
            <RefreshCw className="text-lime-500 h-6 w-6 animate-spin" />
          </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs" style={{ color: 'var(--np-muted)' }}>
                      {appt.practitionerName ?? 'Assigned Clinician'} ·{' '}
