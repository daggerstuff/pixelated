...
export function ModalityNoteEditor({
  ...
  const [draftId, setDraftId] = useState<string>(
    () => initialDraftId ?? `draft_${Date.now()}`,
  )
  const [templateId, setTemplateId] = useState(() => {
    const existing = offlineSyncService.getDraftNote(draftId)
    return existing?.templateId ?? initialTemplateId
  })
  const [content, setContent] = useState<Record<string, string>>(() => {
    const existing = offlineSyncService.getDraftNote(draftId)
    return existing?.content ?? {}
  })
  const [docStatus, setDocStatus] = useState<'preliminary' | 'final'>(() => {
    const existing = offlineSyncService.getDraftNote(draftId)
    return existing?.docStatus ?? 'preliminary'
  })
  ...
  const [lastSavedAt, setLastSavedAt] = useState<string | null>(() => {
    const existing = offlineSyncService.getDraftNote(draftId)
    return existing?.clientUpdatedAt ?? null
  })
  ...
  // Render-phase adjustment: reload draft when draftId changes
  ...
  const template =
    noteTemplateService.getTemplate(templateId) ??
  ...
  useEffect(() => {
    ...
    const unsubConflict = offlineSyncService.on('conflict', (data: unknown) => {
      ...
      if (payload?.id === draftId) {
        ...
        setConflictMessage(
          'Server version differs. Click resolve or force sync.',
        )
      }
    })
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            >
              ...
              {lastSavedAt && (
                <span>
                  · Last: {new Date(lastSavedAt).toLocaleTimeString()}
                </span>
              )}
            </div>
          </div>
              ...
              {!isCollapsed && (
                <div className="p-4" id={`section-input-${sec.key}`}>
                  <textarea
                    value={value}
                    onChange={(e) =>
                      handleContentChange(sec.key, e.target.value)
                    }
                    placeholder={`Document ${sec.label.toLowerCase()}...`}
                    rows={4}
                    className="focus:ring-lime-500 w-full resize-y rounded-lg p-3 text-sm focus:outline-none focus:ring-2"
                    style={{
