...
export function dashboardToCSV(data: DashboardExportData): string {
  ...
  for (const section of data.sections) {
    ...
    lines.push(section.columns.map((c) => escapeCSV(c.label)).join(CSV_DELIMITER));
    for (const row of section.rows) {
      lines.push(section.columns.map((c) => escapeCSV(row[c.key] ?? null)).join(CSV_DELIMITER));
    }
    lines.push(CSV_NEWLINE);
  }
 ...
 */
export function exportDashboardCSV(data: DashboardExportData, filename?: string): void {
  const csv = dashboardToCSV(data);
  const name = filename ?? `${data.dashboard}-dashboard-${data.generatedAt.split('T')[0]}`;
...
export async function exportDashboardPDFAndDownload(
  ...
  const name = filename ?? `${data.dashboard}-dashboard-${data.generatedAt.split('T')[0]}.pdf`;
