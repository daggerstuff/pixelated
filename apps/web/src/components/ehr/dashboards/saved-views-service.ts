...
function readLocalViews(userId: string, dashboard: DashboardType): DashboardLayout[] {
  try {
...
function writeLocalViews(userId: string, dashboard: DashboardType, views: DashboardLayout[]): void {
  try {
...
async function fetchViewsFromAPI(userId: string, dashboard: DashboardType): Promise<DashboardLayout[]> {
  const res = await fetch(`${API_BASE}?userId=${encodeURIComponent(userId)}&dashboard=${encodeURIComponent(dashboard)}`, {
    method: 'GET',
...
export const savedViewsService = {
   ...
   */
  async loadViews(userId: string, dashboard: DashboardType): Promise<DashboardLayout[]> {
    try {
   ...
   */
  async saveView(userId: string, dashboard: DashboardType, view: DashboardLayout): Promise<DashboardLayout> {
   ...
   */
  async deleteView(userId: string, dashboard: DashboardType, viewId: string): Promise<void> {
    const local = readLocalViews(userId, dashboard).filter((v) => v.id !== viewId);
    writeLocalViews(userId, dashboard, local);
   ...
   */
  async setDefaultView(userId: string, dashboard: DashboardType, viewId: string): Promise<void> {
    const local = readLocalViews(userId, dashboard);
