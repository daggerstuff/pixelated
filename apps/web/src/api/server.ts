...
async function initializeDatabases() {
  ...
  try {
    ...
    logger.warn(
      'MongoDB connection failed — continuing without MongoDB (dev mode)',
    )
  }
  ...
  try {
    ...
    if (isProduction) {
      logger.error(
        'PostgreSQL connection failed in production — aborting startup',
      )
      throw error
    }
    logger.warn(
      'PostgreSQL connection failed — continuing without PostgreSQL (dev mode)',
    )
  }
