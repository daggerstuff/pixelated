...
const requirePermission =
  (_permission: string) => (_req: Request, _res: Response, next: () => void) =>
    next();
const requireRole = (_roles: string[]) => (_req: Request, _res: Response, next: () => void) =>
  next();
...
router.get(
  ...
  asyncHandler(async (req: unknown, res: unknown) => {
    ...
    const filter: Record<string, unknown> = {
      $or: [{ owner: userId }, { "permissions.view": userId }, { "permissions.edit": userId }],
    };
...
router.get(
  ...
  asyncHandler(async (req: unknown, res: unknown) => {
    ...
    if (format === "md") {
      ...
      expressRes.setHeader("Content-Disposition", `attachment; filename="${document.slug}.md"`);
      ...
      expressRes.setHeader("Content-Disposition", `attachment; filename="${document.slug}.json"`);
