...
function alertDataToInstance(a: AlertLike): AlertInstance {
  return {
    ...
    message: a.message ?? "",
    acknowledged:
      "acknowledged" in a && typeof (a as { acknowledged?: boolean }).acknowledged === "boolean"
        ? (a as { acknowledged: boolean }).acknowledged
        : false,
    escalated:
      "escalated" in a && typeof (a as { escalated?: boolean }).escalated === "boolean"
...
export class BiasAlertSystem {
  ...
  private readonly notificationChannels: Map<string, NotificationChannelConfig> = new Map();
  ...
  private detectDemographicDisparity(result: BiasAnalysisResult): boolean {
    try {
      ...
      const positiveIndicators = disparityIndicators.filter((indicator) => indicator).length;
      ...
      const multipleIndicatorsDetected =
        positiveIndicators >= Math.ceil(totalIndicators * INDICATOR_THRESHOLD_PERCENTAGE);
   ...
   */
  private analyzeDemographicLayerDisparities(result: BiasAnalysisResult): boolean[] {
    ...
    try {
      ...
      if (result.layerResults.interactive?.counterfactualAnalysis?.problematicScenarios) {
        const scenarios =
          result.layerResults.interactive.counterfactualAnalysis.problematicScenarios;
        ...
        const hasGenderDisparity = scenarios.some(
          (scenario: ProblematicScenario) =>
            scenario.biasType === "gender_bias" && scenario.severity === "medium",
        );
   ...
   */
  private analyzeFairnessMetricDisparities(result: BiasAnalysisResult): boolean[] {
    const indicators: boolean[] = [];
   ...
   */
  private analyzeCounterfactualDisparities(result: BiasAnalysisResult): boolean[] {
    const indicators: boolean[] = [];
    ...
    try {
      ...
      if (result.layerResults.interactive?.featureImportance) {
        ...
        features.forEach((feature: FeatureImportanceResult) => {
          ...
          if (feature.demographicSensitivity) {
            const sensitivityValues = Object.values(feature.demographicSensitivity);
            const numericSensitivities = sensitivityValues
              .map((v) => (typeof v === "number" ? v : Number(v)))
              .filter((n) => !Number.isNaN(n));
            const maxSensitivity =
              numericSensitivities.length > 0 ? Math.max(...numericSensitivities) : 0;
            const minSensitivity =
              numericSensitivities.length > 0 ? Math.min(...numericSensitivities) : 0;
      ...
      if (result.layerResults.evaluation?.temporalAnalysis?.interventionEffectiveness) {
        const interventions =
          result.layerResults.evaluation.temporalAnalysis.interventionEffectiveness;
  ...
  async initialize(): Promise<void> {
    try {
      ...
      logger.info("BiasAlertSystem initialized successfully (local processing enabled)", {
        alertRules: this.alertRules.length,
        notificationChannels: this.notificationChannels.size,
      });
    } catch (error: unknown) {
  ...
  async checkAlerts(result: BiasAnalysisResult): Promise<void> {
    try {
      ...
      const serverAlerts =
        ((serverAlertsResponse as Record<string, unknown>)?.["alerts"] as
          | AlertInstance[]
          | undefined) ?? [];
      ...
      if (allAlerts.length > 0) {
        logger.info(`Generated ${allAlerts.length} alerts for session ${result.sessionId}`, {
  ...
  private triggerMonitoringCallbacks(alerts: AlertInstance[], result: BiasAnalysisResult): void {
  ...
  private async sendNotificationToChannel(
    ...
    try {
      ...
      logger.warn(`[${channel.toUpperCase()}] Alert notification: ${alert.message}`, {
        alertId: alert.id,
        level: alert.level,
        sessionId: alert.sessionId,
        timestamp: new Date().toISOString(),
  ...
  async getAlertStatistics(timeRangeMs: number = 86400000): Promise<{
    ...
    try {
      ...
      const cutoffTime = new Date(Date.now() - timeRangeMs);
      const recentAlerts = this.alertQueue.filter((alert) => alert.timestamp >= cutoffTime);
