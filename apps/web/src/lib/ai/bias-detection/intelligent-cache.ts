...
export class IntelligentCache {
  ...
  async get<T>(key: string, strategy?: string): Promise<T | null> {
    ...
    try {
      const cacheStrategy =
        strategy && this.strategies.has(strategy) ? (this.strategies.get(strategy) ?? null) : null;
   ...
   */
  private async getFromTiers<T>(key: string, strategy: CacheStrategy | null): Promise<T | null> {
  ...
  private async batchSet(key: string, value: unknown, options: CacheOptions): Promise<void> {
    return new Promise((resolve, reject) => {
      this.batchQueue.push({
        operation: "set",
        key,
        ...
        await Promise.all(
          sets.map(async (op) => {
            try {
              ...
              const str =
                safeOptions.strategy && this.strategies.has(safeOptions.strategy)
...
export async function getCachedSessionData(sessionId: string): Promise<TherapeuticSession | null> {
  const cache = getIntelligentCache();
...
export async function cacheReport(reportId: string, report: BiasReport): Promise<void> {
  const cache = getIntelligentCache();
  await cache.set(`report:${reportId}`, report, { strategy: "report" });
}

export async function getCachedReport(reportId: string): Promise<BiasReport | null> {
  const cache = getIntelligentCache();
