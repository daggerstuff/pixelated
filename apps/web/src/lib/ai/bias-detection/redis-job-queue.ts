...
const logger = createBuildSafeLogger("redis-job-queue");
...
export class RedisJobQueue<T = any, R = any> {
  ...
  constructor(
    ...
    this.redis
      ...
      .catch((err) => {
        logger.error("[RedisJobQueue] Redis connection error", err);
      });
  }
  ...
  async getJob(id: string): Promise<Job<T, R> | undefined> {
    ...
    return jobStr ? (JSON.parse(jobStr) as Job<T, R>) : undefined;
  }

  async getAllJobs(): Promise<Job<T, R>[]> {
    const jobs = await this.redis.hGetAll(this.jobsKey);
    return Object.values(jobs).map((j) => JSON.parse(j) as Job<T, R>);
  }
  ...
  private async processNext() {
    ...
    try {
      ...
      try {
        ...
        logger.error("[RedisJobQueue] Job failed", {
          jobId: job.id,
          error: job.error,
          finishedAt: Date.now(),
        });
  ...
  async getMetrics() {
    ...
    const avgDuration =
      completedJobs.length > 0
        ? completedJobs.reduce((sum, j) => sum + (j.finishedAt! - j.startedAt! || 0), 0) /
