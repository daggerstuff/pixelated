...
import type { PythonAnalysisResult, PythonHealthResponse } from "./bias-detection-interfaces";
...
/**
 * Converts an unknown error value to a string without producing "[object Object]".
 * Must handle: Error instances, plain objects, primitives, and falsy values.
 */
...
export class PythonBiasDetectionBridge {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      throw new Error(
        `Python service initialization failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        { cause: error },
      );
    }
  }

  private async queueRequest<T>(requestFn: () => Promise<T>, priority: number = 1): Promise<T> {
  ...
  private async executeRequest(
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
      try {
        ...
        if (this.connectionPool && typeof this.connectionPool.acquireConnection === "function") {
        ...
        logger.debug(`Making request to ${url} (attempt ${attempt}/${this.retryAttempts})`);
  ...
  async runPreprocessingAnalysis(
    ...
    try {
      ...
      if (layerResult) {
        ...
        const ling =
          typeof metrics["linguistic_bias"] === "object" && metrics["linguistic_bias"]
            ? (metrics["linguistic_bias"] as Record<string, unknown>)
            : ({} as Record<string, unknown>);
        const sentiment =
          typeof ling["sentiment_analysis"] === "object" && ling["sentiment_analysis"]
        ...
        const rep =
          ...
          metrics["representation_analysis"]
            ...
            : ({} as Record<string, unknown>);
        const dq =
          typeof metrics["data_quality_metrics"] === "object" && metrics["data_quality_metrics"]
        ...
        return {
          ...
          linguisticBias: {
            ...
            sentimentAnalysis: {
              ...
              demographicVariations:
                (sentiment["demographicVariations"] as Record<string, number>) ?? {},
            },
          },
          representationAnalysis: {
            ...
            demographicDistribution:
              (rep["demographic_distribution"] as Record<string, number>) ?? {},
          ...
          dataQualityMetrics: {
            ...
            missingDataByDemographic:
              (dq["missingDataByDemographic"] as Record<string, number>) ?? {},
          },
  ...
  async runModelLevelAnalysis(sessionData: TherapeuticSession): Promise<ModelLevelLayerResult> {
    try {
      const result = (await this.makeRequest(
        "/analyze/model_level",
        "POST",
      ...
      if (layerResult) {
        ...
        const fairness = (metrics["fairness_metrics"] ?? {}) as Record<string, unknown>;
        const performance = (metrics["performance_metrics"] ?? {}) as Record<string, unknown>;
        const groupComp = (metrics["group_performance_comparison"] ?? []) as Record<
          string,
          unknown
        >[];
        ...
        return {
          ...
          performanceMetrics: {
            ...
            demographicBreakdown:
              (performance["demographic_breakdown"] as Record<string, number>) ?? {},
          },
  ...
  async runInteractiveAnalysis(sessionData: TherapeuticSession): Promise<InteractiveLayerResult> {
    try {
      const result = (await this.makeRequest(
        "/analyze/interactive",
        "POST",
      ...
      if (layerResult) {
        const metrics = (layerResult.metrics ?? {}) as Record<string, unknown>;
        const counterfactual = (metrics["counterfactual_analysis"] ?? {}) as Record<
          string,
          unknown
        >;
  ...
  async runEvaluationAnalysis(sessionData: TherapeuticSession): Promise<EvaluationLayerResult> {
    try {
      const result = (await this.makeRequest(
        "/analyze/evaluation",
        "POST",
      ...
      if (layerResult) {
        const metrics = (layerResult.metrics ?? {}) as Record<string, unknown>;
        const huggingFace = (metrics["hugging_face_metrics"] ?? {}) as Record<string, unknown>;
        const custom = (metrics["custom_metrics"] ?? {}) as Record<string, unknown>;
        const temporal = (metrics["temporal_analysis"] ?? {}) as Record<string, unknown>;
  ...
  async dispose(): Promise<void> {
    ...
    try {
      ...
      if (this.connectionPool && typeof this.connectionPool.dispose === "function") {
