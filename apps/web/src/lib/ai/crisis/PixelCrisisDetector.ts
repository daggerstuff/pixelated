...
const logger = createBuildSafeLogger("PixelCrisisDetector");
...
export class PixelCrisisDetector {
  ...
  async analyze(text: string): Promise<MentalHealthAnalysis> {
    try {
      ...
      if (this.config.fallbackToKeywords) {
        logger.warn("Pixel inference failed, falling back to keyword detection:", error);
   ...
   * Call Pixel inference API
   */
  private async callPixelInference(text: string): Promise<PixelInferenceResponse> {
  ...
  private calculateCrisisSeverity(signals: string[]): number {
    ...
    const maxSeverity = Math.max(...signals.map((signal) => severityMap[signal] ?? 0.6));
  ...
  private extractEQIndicators(eqScores: EQScores): HealthIndicator[] {
    ...
    if (eqScores.emotional_regulation < 0.3) {
      indicators.push({
        ...
        evidence: [`Low emotional regulation: ${eqScores.emotional_regulation.toFixed(2)}`],
    ...
    if (eqScores.empathy_recognition < 0.3) {
      indicators.push({
        type: "disconnection",
        severity: 0.6,
        evidence: [`Low empathy recognition: ${eqScores.empathy_recognition.toFixed(2)}`],
   ...
   */
  private categorizeIssues(indicators: HealthIndicator[]): MentalHealthCategoryDetail[] {
  ...
  private keywordFallbackAnalysis(text: string): MentalHealthAnalysis {
    ...
    const matches = crisisKeywords.filter((keyword) => normalizedText.includes(keyword));
