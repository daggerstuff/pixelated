...
export class OfflineSyncService {
  private readonly notes: Map<string, DraftNote> = new Map()
  private readonly appointmentActions: Map<string, QueuedAppointmentAction> =
    new Map()
  private readonly messages: Map<string, QueuedMessage> = new Map()
  private readonly outcomeSubmissions: Map<string, QueuedOutcomeSubmission> =
    new Map()
  ...
  private readonly listeners: Map<string, Set<SyncEventListener>> = new Map()
    ...
    try {
      const response = await fetch('/api/ehr/v1/notes', {
        ...
        body: JSON.stringify({
          ...
          encounterRef: draft.encounterId
            ? `Encounter/${draft.encounterId}`
            : undefined,
      ...
      if (response.status === 409) {
        ...
        const serverVer =
          conflictData.serverVersion ?? (draft.serverVersion ?? 1) + 1
        ...
        this.notes.set(draftId, {
          ...
          conflictDetails:
            'Remote note has been updated by another session or user.',
        })
        await this.persistAll()
  ...
  async syncAll(options: SyncOptions = {}): Promise<SyncResult> {
    ...
    try {
          ...
          if (success) syncedCount++
            ...
            if (current?.syncStatus === 'conflict') {
              ...
              errors.push({
                id,
                type: 'note',
                message: 'Failed to sync draft note',
              })
            }
          }
        }
      ...
      for (const [id, outcome] of Array.from(
        this.outcomeSubmissions.entries(),
      )) {
        if (
          ...
          if (success) syncedCount++
            ...
            errors.push({
              id,
              type: 'outcome',
              message: 'Failed to sync outcome measure',
            })
          }
        }
      }
  ...
  getStatus(): OfflineSyncStatus {
    ...
    return {
      ...
      totalPendingCount:
        pendingNotes + pendingAppts + pendingMsgs + pendingOutcomes,
