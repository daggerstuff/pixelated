...
function createMockRepo(overrides: Partial<AnalyticsRepository> = {}): AnalyticsRepository {
  return {
    ...
    getOutcomeScores: vi.fn().mockResolvedValue({ average: 12.5, change: -2.1, trend: [] }),
    ...
    countEncountersByType: vi.fn().mockResolvedValue({ inPerson: 150, telehealth: 50 }),
    ...
    getTrainingCompletion: vi.fn().mockResolvedValue({ completion: 0.88, overdue: 3 }),
...
describe('AnalyticsService.getPracticeDashboard', () => {
  ...
  it('throws for role without practice permissions', async () => {
    // frontDesk may not have read_patient in all configurations
    await expect(service.getPracticeDashboard('frontDesk', baseFilter)).rejects.toThrow(
...
describe('AnalyticsService.getUtilizationDashboard', () => {
  ...
  it('calculates telehealth rate from byType data', async () => {
    const metrics = await service.getUtilizationDashboard('physician', baseFilter);
...
describe('AnalyticsService.getBillingDashboard', () => {
  ...
  it('returns billing metrics and calculates collectionsRate', async () => {
    const metrics = await service.getBillingDashboard('billingSpecialist', baseFilter);
...
describe('AnalyticsService.getComplianceDashboard', () => {
  ...
  it('returns compliance metrics from repository', async () => {
    const metrics = await service.getComplianceDashboard('complianceOfficer', baseFilter);
...
describe('AnalyticsService.getDashboard', () => {
  it('dispatches to getPracticeDashboard for "practice"', async () => {
    ...
    const result = await service.getDashboard('practice', 'physician', baseFilter);
  ...
  it('dispatches to getOutcomesDashboard for "outcomes"', async () => {
    const repo = createMockRepo();
    const service = new AnalyticsService(repo);
    const result = await service.getDashboard('outcomes', 'physician', baseFilter);
  ...
  it('dispatches to getBillingDashboard for "billing"', async () => {
    const repo = createMockRepo();
    const service = new AnalyticsService(repo);
    const result = await service.getDashboard('billing', 'billingSpecialist', baseFilter);
