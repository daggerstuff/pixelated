...
function makeValidClaimInput() {
  return {
    ...
    items: [
      {
        productOrService: {
          coding: [{ system: 'http://hl7.org/fhir/sid/cpt', code: '90834' }],
        },
    ...
    diagnoses: [
      {
        ...
        diagnosisCodeableConcept: {
          coding: [{ system: 'http://hl7.org/fhir/sid/icd-10', code: 'F41.1' }],
        },
      },
    ],
    insurance: [{ focal: true, coverage: 'Coverage/coverage-001' }],
  }
}
...
describe('ClaimsService', () => {
  ...
  describe('calculateTotal', () => {
    ...
    it('sums multiple items', () => {
      ...
      input.items.push(
        {
          productOrService: {
            coding: [{ system: 'http://hl7.org/fhir/sid/cpt', code: '90837' }],
          },
          unitPrice: { value: 250, currency: 'USD' },
          quantity: 1,
        },
        {
          productOrService: {
            coding: [{ system: 'http://hl7.org/fhir/sid/cpt', code: '90791' }],
          },
  ...
  describe('validateStatusTransition', () => {
    it('allows draft → active', () => {
      expect(service.validateStatusTransition('draft', 'active').allowed).toBe(
        true,
      )
    })
    ...
    it('allows active → cancelled', () => {
      expect(
        service.validateStatusTransition('active', 'cancelled').allowed,
      ).toBe(true)
    })

    it('allows active → entered-in-error', () => {
      expect(
        service.validateStatusTransition('active', 'entered-in-error').allowed,
      ).toBe(true)
    })

    it('allows draft → cancelled', () => {
      expect(
        service.validateStatusTransition('draft', 'cancelled').allowed,
      ).toBe(true)
    })

    it('disallows cancelled → active', () => {
      expect(
        service.validateStatusTransition('cancelled', 'active').allowed,
      ).toBe(false)
    })

    it('disallows entered-in-error → active', () => {
      expect(
        service.validateStatusTransition('entered-in-error', 'active').allowed,
      ).toBe(false)
    })
  })
