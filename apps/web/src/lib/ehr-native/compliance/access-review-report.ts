...
function isRoleAssignmentEvent(event: AuditEvent): boolean {
  ...
  return (
    ...
    action.includes('role_grant') ||
    action.includes('user_created') ||
    action.includes('user_role_change')
  )
}
...
function isPermissionGrantEvent(event: AuditEvent): boolean {
  const action = String(event.action).toLowerCase()
  return (
    action.includes('permission_granted') || action.includes('permission_grant')
  )
}
...
function buildRoleAssignments(events: AuditEvent[]): AccessRoleAssignment[] {
  const userRoleMap = new Map<
    string,
    { role: string; assignedAt: string; active: boolean }
  >()
