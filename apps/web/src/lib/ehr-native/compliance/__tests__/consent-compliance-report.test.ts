...
describe('generateConsentComplianceReport', () => {
  ...
  it('groups by treatment category', async () => {
    ...
    const mh = report.byTreatment.find(
      (t) => t.treatmentCategory === 'mental_health',
    )
    expect(mh?.totalConsents).toBe(2)
    const sa = report.byTreatment.find(
      (t) => t.treatmentCategory === 'substance_abuse',
    )
  ...
  it('uses unknown state code when scope has no state', async () => {
    mockQuery.mockResolvedValue({
      rows: [makeConsentRow({ consent_id: 'c1', scope: 'mental_health' })],
    })
