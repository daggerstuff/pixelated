...
describe('generateHIPAAAuditReport', () => {
  it('classifies PHI access events correctly', async () => {
    ...
    const report = await generateHIPAAAuditReport(period, 'tenant-001', validChain);
  ...
  it('counts unique users and patients', async () => {
    const events: AuditEvent[] = [
      makeEvent({ id: '1', userId: 'user-1', metadata: { patientId: 'patient-A' } }),
      makeEvent({ id: '2', userId: 'user-2', metadata: { patientId: 'patient-B' } }),
      makeEvent({ id: '3', userId: 'user-1', metadata: { patientId: 'patient-A' } }),
    ];
    mockQueryAuditEvents.mockResolvedValue(events);
  ...
  it('sets chain verification result', async () => {
    ...
    const brokenChain: ChainVerificationResult = {
      valid: false, totalEvents: 0, brokenAtId: 'evt-99', reason: 'Hash mismatch',
    };

    const report = await generateHIPAAAuditReport(period, 'tenant-001', brokenChain);
  ...
  it('updates totalEvents in chain verification', async () => {
    ...
    const report = await generateHIPAAAuditReport(period, 'tenant-001', { ...validChain, totalEvents: 0 });
