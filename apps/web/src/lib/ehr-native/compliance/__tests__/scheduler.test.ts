...
describe('createScheduledReport', () => {
  it('creates a scheduled report with defaults', () => {
    const config = createScheduledReport(
      'hipaa_audit', 'monthly', 'tenant-001', ['admin@example.com'],
    );
  ...
  it('creates with custom format and dayOfMonth', () => {
    const config = createScheduledReport(
      'soc2_security', 'quarterly', 'tenant-001', ['admin@example.com'],
      'csv', 15,
    );
...
describe('listScheduledReports', () => {
  it('lists all scheduled reports', () => {
    ...
    createScheduledReport('soc2_security', 'quarterly', 'tenant-002', ['b@x.com']);
    const all = listScheduledReports();
...
describe('getScheduledReport', () => {
  it('returns config by ID', () => {
    const created = createScheduledReport('hipaa_audit', 'monthly', 't1', ['a@x.com']);
...
describe('updateScheduledReport', () => {
  it('updates schedule and active status', () => {
    const created = createScheduledReport('hipaa_audit', 'monthly', 't1', ['a@x.com']);
    const updated = updateScheduledReport(created.scheduleId, { active: false, schedule: 'quarterly' });
  ...
  it('returns undefined for unknown ID', () => {
    expect(updateScheduledReport('nonexistent', { active: false })).toBeUndefined();
  });
});
...
describe('processScheduledReports', () => {
  it('processes due reports', async () => {
    const config = createScheduledReport('hipaa_audit', 'monthly', 't1', ['a@x.com']);
    // Make it due by setting dayOfMonth to today
    updateScheduledReport(config.scheduleId, { dayOfMonth: new Date().getDate() });
  ...
  it('skips inactive schedules', async () => {
    const config = createScheduledReport('hipaa_audit', 'monthly', 't1', ['a@x.com']);
    updateScheduledReport(config.scheduleId, { active: false, dayOfMonth: new Date().getDate() });
...
describe('triggerScheduledReportNow', () => {
  ...
  it('throws for unknown schedule ID', async () => {
    await expect(triggerScheduledReportNow('nonexistent')).rejects.toThrow('Scheduled report not found');
  });
});
