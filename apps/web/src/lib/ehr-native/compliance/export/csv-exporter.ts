...
function reportTitle(reportType: ComplianceReport['reportType']): string {
  switch (reportType) {
    case 'hipaa_audit': return 'HIPAA Audit Report';
    case 'soc2_security': return 'SOC 2 Security Report';
    case 'soc2_availability': return 'SOC 2 Availability Report';
    case 'consent_compliance': return 'Consent Compliance Report';
    case 'access_review': return 'Access Review Report';
    default: {
...
function table(headers: string[], data: (string | number | boolean | undefined | null)[][]): string {
...
function renderHIPAAAuditCSV(report: HIPAAAuditReport): string {
  ...
  const accessRows = report.phiAccessEvents.map((e) => [
    e.eventId, e.timestamp, e.userId, e.action, e.resourceType, e.resourceId,
    e.patientId ?? '', e.severity, e.status, e.ipAddress ?? '', e.hash ?? '',
  ]);
  parts.push(table(
    ['Event ID', 'Timestamp', 'User', 'Action', 'Resource Type', 'Resource ID', 'Patient ID', 'Severity', 'Status', 'IP Address', 'Hash'],
    accessRows,
  ));
  ...
  const modRows = report.phiModificationEvents.map((e) => [
    e.eventId, e.timestamp, e.userId, e.action, e.resourceType, e.resourceId,
    e.previousHash ?? '', e.hash ?? '', e.status,
  ]);
  parts.push(table(
    ['Event ID', 'Timestamp', 'User', 'Action', 'Resource Type', 'Resource ID', 'Previous Hash', 'Hash', 'Status'],
    modRows,
  ));
  ...
  const bgRows = report.breakGlassEvents.map((e) => [
    e.eventId, e.timestamp, e.userId, e.reason, e.resourceType, e.resourceId, e.severity,
  ]);
  parts.push(table(
    ['Event ID', 'Timestamp', 'User', 'Reason', 'Resource Type', 'Resource ID', 'Severity'],
    bgRows,
  ));
...
function renderSOC2SecurityCSV(report: SOC2SecurityReport): string {
  ...
  const roleRows = report.accessControls.roleAssignments.map((r) => [
    r.role, r.count, r.permissions.join('; '),
  ]);
  ...
  parts.push(table(
    ...
    [
      ...
      ['MFA Required Permissions', report.accessControls.mfaRequiredPermissions],
    ],
  ));
  ...
  const incRows = report.incidents.map((i) => [
    i.incidentId, i.timestamp, i.severity, i.type, i.description, i.resolved,
  ]);
  parts.push(table(
    ['Incident ID', 'Timestamp', 'Severity', 'Type', 'Description', 'Resolved'],
    incRows,
  ));
...
function renderSOC2AvailabilityCSV(report: SOC2AvailabilityReport): string {
  ...
  const uptimeRows = report.uptime.incidents.map((i) => [
    i.incidentId, i.startTime, i.endTime ?? '', i.durationMinutes, i.description,
  ]);
  parts.push(table(
    ['Incident ID', 'Start Time', 'End Time', 'Duration (min)', 'Description'],
    uptimeRows,
  ));
...
function renderConsentComplianceCSV(report: ConsentComplianceReport): string {
  ...
  const stateRows = report.byState.map((s) => [
    s.stateCode, s.totalConsents, s.activeConsents, s.expiredConsents,
    s.revokedConsents, s.requiredLevel, `${s.complianceRate}%`,
  ]);
  parts.push(table(
    ['State', 'Total', 'Active', 'Expired', 'Revoked', 'Required Level', 'Compliance'],
    stateRows,
  ));
  ...
  const treatRows = report.byTreatment.map((t) => [
    t.treatmentCategory, t.totalConsents, t.activeConsents, t.expiredConsents,
    t.revokedConsents, `${t.complianceRate}%`,
  ]);
  parts.push(table(
  ...
  const expRows = report.expiringConsents.map((e) => [
    e.consentId, e.patientId, e.stateCode, e.category, e.expiresAt, e.daysUntilExpiry,
  ]);
  parts.push(table(
    ['Consent ID', 'Patient ID', 'State', 'Category', 'Expires At', 'Days Until Expiry'],
    expRows,
  ));
...
function renderAccessReviewCSV(report: AccessReviewReport): string {
  ...
  const assignRows = report.roleAssignments.map((a) => [
    a.userId, a.role, a.permissions.join('; '), a.assignedAt, a.active,
  ]);
  parts.push(table(
  ...
  const changeRows = report.permissionChanges.map((c) => [
    c.changeId, c.timestamp, c.userId, c.type, c.permission, c.role, c.reason ?? '',
  ]);
  parts.push(table(
    ['Change ID', 'Timestamp', 'User ID', 'Type', 'Permission', 'Role', 'Reason'],
    changeRows,
  ));
...
export async function exportReportToCSV(
  ...
  switch (report.reportType) {
    ...
    default: {
      ...
      throw new Error(`Unsupported report type for CSV: ${JSON.stringify(exhaustive)}`);
    }
  }
