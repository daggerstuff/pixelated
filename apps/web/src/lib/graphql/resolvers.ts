  ...
  Session: {
    ...
    turns: async (
      ...
      try {
        ...
        const messages = await getSessionTurns(
          parent.id,
          parent.clientId,
          limit,
          offset,
        ...
        return messages.map((msg) => {
          const msgId =
            (msg as unknown as Record<string, unknown>)['_id']?.toString?.() ??
            ''
          return {
            id: msgId,
            role:
