...
import type { RoleTransitionRequest, RoleTransitionApproval, RoleTransitionAuditLog, RoleAssignmentRequest, RoleTransitionValidation } from './role-transitions.types'
export type { RoleTransitionRequest, RoleTransitionApproval, RoleTransitionAuditLog, RoleAssignmentRequest, RoleTransitionValidation } from './role-transitions.types'
...
export async function requestRoleTransition(
  ...
  try {
    ...
    const currentRole = sessionData.role as UserRole
    ...
    await logRoleTransitionAudit({
      ...
      actorId: requestedBy,
      actorRole: sessionData.role as UserRole,
      reason,
      timestamp: now,
      ipAddress: clientInfo.ipAddress,
      userAgent: clientInfo.userAgent,
...
export async function processRoleTransitionApproval(
  ...
  // Destructure outside try for catch block access
  const {
    requestId,
    approverId,
    approverRole,
...
async function executeRoleTransition(
  ...
  try {
    ...
    await logRoleTransitionAudit({
      ...
      actorRole: approverSession.role as UserRole,
...
export async function validateRoleAssignment(
  ...
  // Initialize outside try for catch block access
