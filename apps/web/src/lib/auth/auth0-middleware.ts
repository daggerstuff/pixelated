...
import { extractTokenFromRequest, getClientIp, getClientInfo } from './auth0-middleware.utils'
import type { ClientInfo } from './auth0-middleware.utils'
export { extractTokenFromRequest, getClientIp, getClientInfo } from './auth0-middleware.utils'
export type { ClientInfo } from './auth0-middleware.utils'
...
export async function csrfProtection(request: Request): Promise<{
  ...
  const csrfToken = (request.headers?.get?.('X-CSRF-Token') ??
    ...
    (request.headers as unknown as Record<string, unknown> | undefined)?.[
      'X-CSRF-Token'
    ] ??
    (request.headers as unknown as Record<string, unknown> | undefined)?.[
      'x-csrf-token'
...
export async function authenticateRequest(
  ...
  if (strategy === 'jwtOnly' || strategy === 'either') {
    ...
    const authHeader = (request.headers?.get?.('Authorization') ??
      ...
      (
        request.headers as unknown as { get?: (name: string) => string | null }
