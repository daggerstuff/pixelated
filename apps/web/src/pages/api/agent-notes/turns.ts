...
const turnSubmitSchema = z.object({
  ...
  assumptions: z
    .array(z.string().trim().min(1, 'assumptions cannot be blank.'))
    .nonempty('At least one assumption is required.'),
  openQuestions: z
    .array(z.string().trim().min(1, 'openQuestions cannot be blank.'))
    .nonempty('At least one open question is required.'),
  decision: z.string().trim().min(1, 'decision is required.'),
  evidence: z
    .array(z.string().trim().min(1, 'evidence cannot be blank.'))
    .nonempty('At least one evidence item is required.'),
...
export const GET: APIRoute = async ({ url, locals }) => {
  ...
  let turns: NoteTurn[]
  if (query.artifactId) {
    const turnsForArtifactAccess = await ledger.replayByArtifact(
      query.artifactId,
    )
    if (
      !hasArtifactTurnAccess(
        turnsForArtifactAccess,
    ...
    turns = await ledger.list(
      queryParams as unknown as import('../../../lib/agent-note-collab/store').TurnQuery,
    )
  } else {
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    const artifactTurns = await ledger.replayByArtifact(
      submitPayload.artifactId,
    )
    const actorCanWrite =
      actorContext.hasPrivilege ||
    ...
    if (!result.ok) {
      return createErrorResponse(
        422,
        'SUBMISSION_REJECTED',
        'Turn rejected by governance policy.',
        result.errors,
      )
    }
    ...
    return createErrorResponse(
      ...
      error instanceof Error
        ? error instanceof Error
          ? error.message
          : 'Unknown error'
        : 'An unexpected error occurred while processing the turn.',
    )
  }
}

function filterByQueryFilters(
  turns: NoteTurn[],
  query: TurnQueryParams,
): NoteTurn[] {
  return turns.filter((turn) => {
    ...
    if (
      query.requestedAction &&
      turn.requestedAction !== query.requestedAction
    ) {
      return false
    }
