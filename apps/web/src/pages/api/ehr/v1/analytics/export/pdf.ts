 ...
 */
export const POST = withV1Contract('exportDashboardPDF', async (ctx, caller) => {
  ...
  try {
    ...
    return ehrValidationError(
      err instanceof Error ? err.message : 'Failed to retrieve dashboard data for export.',
    )
  }
  ...
  doc.text(`User: ${caller.user.id} | Role: ${caller.user.role}`, { align: 'center' })
  if (filter.timeRange) {
    doc.text(`Period: ${filter.timeRange.start} to ${filter.timeRange.end}`, { align: 'center' })
  }
  doc.moveDown(1)
  doc.fillColor('#000')
