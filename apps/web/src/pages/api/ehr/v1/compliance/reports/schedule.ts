...
export const POST = withV1Contract('createScheduledReport', async (ctx, caller) => {
  ...
  const { type, schedule, emailRecipients, format, dayOfMonth, triggerNow } = raw as {
    type?: string;
    schedule?: string;
  ...
  const validTypes: ReportType[] = [
    'hipaa_audit', 'soc2_security', 'soc2_availability',
    'consent_compliance', 'access_review',
  ];
  ...
  const validSchedules: ReportSchedule[] = ['monthly', 'quarterly', 'annual', 'ad-hoc'];
  ...
  const dom = typeof dayOfMonth === 'number' && dayOfMonth >= 1 && dayOfMonth <= 28
    ? dayOfMonth
    : 1;
...
export const GET = withV1Contract('listScheduledReports', async (ctx, caller) => {
  const tenantId = resolveTenantId(caller.user.accountId);
  if (!tenantId) {
    return ehrValidationError('Tenant association required for EHR access.');
  }
  ...
  const limit = sanitizeLimitParam(Number(url.searchParams.get('limit') ?? 50));
  const offset = sanitizeOffsetParam(Number(url.searchParams.get('offset') ?? 0));
...
export const DELETE = withV1Contract('deleteScheduledReport', async (ctx, caller) => {
  const tenantId = resolveTenantId(caller.user.accountId);
  if (!tenantId) {
    return ehrValidationError('Tenant association required for EHR access.');
  }
