...
/**
 * POST /api/v1/admin/state-consent-rules/{ruleId}/archive
 * Archive a rule (from any non-active status).
 * Body: { notes? }
 */
export const POST = async ({ request, cookies, params }: BaseAPIContext) => {
  ...
  try {
    ...
    const rule = await stateConsentRulesRepository.archive(ruleId, actor, notes)
    if (!rule) {
      return new Response(
        JSON.stringify({ error: 'Rule not found or cannot be archived' }),
        {
          status: 404,
          headers: JSON_HEADERS,
        },
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to archive rule', detail: message }),
      {
        status: 500,
        headers: JSON_HEADERS,
      },
