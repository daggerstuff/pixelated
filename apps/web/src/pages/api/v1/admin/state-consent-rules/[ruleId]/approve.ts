...
/**
 * POST /api/v1/admin/state-consent-rules/{ruleId}/approve
 * Transition a rule from review to approved status.
 * Body: { notes? }
 */
export const POST = async ({ request, cookies, params }: BaseAPIContext) => {
  ...
  try {
    ...
    const rule = await stateConsentRulesRepository.approve(ruleId, actor, notes)
    if (!rule) {
      return new Response(
        JSON.stringify({ error: 'Rule not found or not in review status' }),
        {
          status: 404,
          headers: JSON_HEADERS,
        },
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to approve rule', detail: message }),
      {
        status: 500,
        headers: JSON_HEADERS,
      },
