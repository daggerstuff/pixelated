;(function (global, factory) {
  ...
  if (support.arrayBuffer) {
    ...
    isArrayBufferView = ArrayBuffer.isView
      ...
      : function (obj) {
          return (
            obj && viewClasses.includes(Object.prototype.toString.call(obj))
          )
        }
  }
  ...
  const normalizeName = function (name) {
    ...
    if (/[^a-z0-9\-#$%&'*+.^_`|~!]/i.test(stringName) || stringName === '') {
      throw new TypeError(
        `Invalid character in header field name: "${stringName}"`,
      )
    }
  ...
  Headers.prototype.append = function (name, value) {
    ...
    this.map[normalizedName] = oldValue
  ...
  const Request = function (input, options) {
    ...
    this.credentials =
      requestOptions.credentials ?? this.credentials ?? 'same-origin'
  ...
  const Response = function (bodyInit, options) {
    ...
    this.type = 'default'
    this.status =
      responseOptions.status === undefined ? 200 : responseOptions.status
    ...
    this.statusText =
      responseOptions.statusText === undefined
        ? ''
...
!(function () {
  try {
    ...
    n &&
      ...
      (e._sentryDebugIds[n] = 'e4c59632-941c-5fa2-b90f-a1952bb6b3c4'))
  } catch (e) {}
})()
