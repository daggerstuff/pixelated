...
- Wrap third-party errors at the boundary and re-raise a domain-specific class
  so callers don't leak vendor specifics.
- Use `from exc` to preserve the traceback.
- Log once at the layer that handles the error. Nested try/except ladders create
  double-log noise.
