...
- **Dense** catches paraphrase, concept overlap, multilingual query/doc drift.
...
- **Weighted linear** only works when scores are calibrated; normalize per-query
  first.
...
- Duplicate docs (same chunk ingested twice) corrupt both recall and reranker
  training.
- Filter pushdown before retrieval, not after — post-filtering discards quality
  candidates.
