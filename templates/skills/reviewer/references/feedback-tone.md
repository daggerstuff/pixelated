...
- Lead with the problem, not the fix. "This unbounded loop can OOM on large
  inputs" beats "Change to `for i, item in enumerate(batch[:100]):`".
- Offer an alternative when you reject an approach.
- Use "we" when the convention is shared; "I'd suggest" when it's personal
  taste.
