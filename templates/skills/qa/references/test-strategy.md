...
- **Integration** (`tests/integration/`) — multiple modules, may touch disk or
  the task server.
- **Contract** (`tests/protocol/`) — verifies API schema stability.
- **Pentest** (`tests/pentest/`) — adversarial, verifies no prompt injection or
  credential leakage.

## Running

- `uv run python scripts/run_tests.py -x` — isolated per file, prevents memory
  leaks.
...
- Flaky tests get fixed, not retried. Quarantine to `tests/quarantine/` if you
  need to ship first, then file a follow-up.
