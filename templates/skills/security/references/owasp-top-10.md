...
Walk through every item when reviewing new code paths. Each finding gets a
severity tag and a concrete fix recommendation.
...
7. **Authentication Failures** — credential stuffing, missing MFA, weak session
   handling, JWT `alg=none`.
8. **Software and Data Integrity Failures** — unsigned releases, supply chain
   (package substitution, dependency confusion).
9. **Logging and Monitoring Failures** — missing audit trail, PII in logs.
10. **Server-Side Request Forgery** — unrestricted fetches to internal hosts,
    metadata endpoints.
