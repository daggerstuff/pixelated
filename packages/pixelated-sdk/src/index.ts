...
export class PixelatedClient {
  ...
  get user() {
    return {
      ...
      getPreferences: async (): Promise<UserPreferences> => {
        const response = await this.request<{ preferences: UserPreferences }>("/preferences");
  ...
  get search() {
    return {
      ...
      query: async (
        ...
        const response = await this.request<{ results: SearchResult[] }>(`/search?${params}`);
  ...
  get apiKeys() {
    return {
      ...
      > => {
        const response = await this.request<{ keys: any[] }>("/developer/api-keys");
      ...
      create: async (name: string, scopes?: string[]): Promise<{ key: string; id: string }> => {
        const response = await this.request<{ key: string; id: string }>("/developer/api-keys", {
          method: "POST",
