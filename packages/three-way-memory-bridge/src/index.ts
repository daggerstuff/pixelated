...
export class MemoryBridge {
   ...
   */
  async getContext(query: string, threadId?: string): Promise<ThreeWayMemoryContext> {
    ...
    if (this.env.FORESIGHT_API_URL) {
      try {
        const res = await fetch(`${this.env.FORESIGHT_API_URL}/api/memories/search`, {
          method: "POST",
        ...
        console.error("[MemoryBridge] Foresight search failed:", err);
      }
    }
...
`.trim();
  ...
  async saveMemory(
    ...
    // Save durable memory to Foresight
    if (this.env.FORESIGHT_API_URL) {
      try {
        await fetch(`${this.env.FORESIGHT_API_URL}/api/memories`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            content,
        ...
        console.error("[MemoryBridge] Foresight store failed:", err);
        success = false;
      }
    }
