...
test.describe("Theme Accessibility Tests", () => {
  ...
  test.describe("Color Contrast Analysis", () => {
    test("light theme should meet WCAG contrast requirements", async () => {
      ...
      await page.evaluate(() => {
        ...
        document.documentElement.classList.remove("dark", "system", "enhanced-dark-theme");
      ...
      const contrastRatio = getContrastRatio(textContrast.textColor, textContrast.bgColor);
      expect(contrastRatio).toBeGreaterThanOrEqual(accessibilityTests.contrastRatios.normal);
    });
    ...
    test("accent colors should have sufficient contrast", async () => {
      ...
      for (const theme of themes) {
        ...
        const contrastRatio = getContrastRatio(accentContrast.accentColor, accentContrast.bgColor);
        expect(contrastRatio).toBeGreaterThanOrEqual(accessibilityTests.contrastRatios.ui);
      }
    });
  });
  ...
  test.describe("Keyboard Navigation", () => {
    test("theme toggle should be keyboard accessible", async () => {
      ...
      const themeAfterEnter = await page.evaluate(() => localStorage.getItem("theme"));
    ...
    test("focus should be visible and high contrast", async () => {
      ...
      const focusContrast = await page.evaluate(() => {
        ...
        const focusColor = window.getComputedStyle(focusedElement, ":focus-visible").outlineColor;
        const bgColor = window.getComputedStyle(document.body).backgroundColor;
        return { focusColor, bgColor };
      });

      if (focusContrast) {
        const contrastRatio = getContrastRatio(focusContrast.focusColor, focusContrast.bgColor);
  ...
  test.describe("Semantic HTML", () => {
    ...
    test("should have proper heading hierarchy", async () => {
      ...
      for (let i = 1; i < headingStructure.length; i++) {
        const levelDiff = Math.abs(headingStructure[i].level - headingStructure[i - 1].level);
