...
const testConfig = {
  ...
  devices: ["Desktop Chrome", "Desktop Firefox", "Desktop Safari", "iPhone 12", "iPad"],
  viewportSizes: [
...
test.describe("Theme Compatibility Tests", () => {
  ...
  test.describe("Cross-Browser Theme Rendering", () => {
    test("should render light theme correctly across browsers", async ({ browserName }) => {
      // Set light theme
      await page.evaluate(() => {
    ...
    test("should render dark theme correctly across browsers", async ({ browserName }) => {
      // Set dark theme
      await page.evaluate(() => {
    ...
    test("should handle system theme preference", async ({ browserName: _browserName }) => {
      ...
      const hasDarkClass = await page.evaluate(() => {
        return document.documentElement.classList.contains("enhanced-dark-theme");
      });
  ...
  test.describe("Responsive Design", () => {
    for (const viewport of testConfig.viewportSizes) {
      test(`should render correctly on ${viewport.name} viewport`, async ({ browserName }) => {
  ...
  test.describe("Performance", () => {
    ...
    test("should not cause layout thrashing", async () => {
      ...
      const measure = await page.evaluate(() => {
        return performance.measure("theme-switch", "theme-switch-start", "theme-switch-end");
      });
  ...
  test.describe("Browser-Specific Issues", () => {
    test("should handle Safari backdrop-filter gracefully", async ({ browserName }) => {
    ...
    test("should handle Firefox color-scheme correctly", async ({ browserName }) => {
    ...
    test("should handle Chrome CSS custom properties", async ({ browserName }) => {
