...
test("login page has correct form elements", async ({ page }) => {
  ...
  await expect(page.locator("label").filter({ hasText: /remember me/i })).toBeVisible();
});
...
test("login page has proper transitions", async ({ page }) => {
  ...
  const passwordResetButton = page.locator('[data-testid="forgot-password-button"]').first();
  ...
  const resetPasswordHeading = page.locator('[data-testid="reset-password-heading"]').first();
