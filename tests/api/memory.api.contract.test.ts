...
test.describe("Memory API Contract Tests", () => {
  ...
  test.describe("Get Memory by ID (GET /api/memory/:memoryId)", () => {
    test("should retrieve a specific memory by ID", async () => {
      ...
      // Retrieve the memory
      const response = await makeRequest(request, "GET", `/api/memory/${memoryId}`);
  ...
  test.describe("Update Memory (PUT /api/memory/:memoryId)", () => {
    test("should update an existing memory", async () => {
      ...
      // Update the memory
      const response = await makeRequest(request, "PUT", `/api/memory/${memoryId}`, {
        data: {
  ...
  test.describe("Delete Memory (DELETE /api/memory/:memoryId)", () => {
    test("should delete an existing memory", async () => {
      ...
      // Delete the memory
      const response = await makeRequest(request, "DELETE", `/api/memory/${memoryId}`);

      expect(response.status()).toBe(204);

      // Verify it's gone
      const getResponse = await makeRequest(request, "GET", `/api/memory/${memoryId}`);
