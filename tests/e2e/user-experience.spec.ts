...
declare global {
  interface Window {
    _checkTransition: (element: Element) => void;
...
test.describe("User Experience Features", () => {
  ...
  test("page transitions work correctly between routes", async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      window._checkTransition = function (element: any) {
        if (
  ...
  test("forms provide proper interactive feedback", async ({ page }) => {
    ...
    const emailInputClassAfterFocus = await emailInput.evaluate((el) => el.className);
