...
export async function login(
  ...
  // the server-issued cookie and leaves /admin routes unauthenticated). The
  // signin endpoint compares the password against the per-run E2E_TEST_TOKEN
  // generated by the workflow, so send that instead of the local default.
  if (process.env['E2E_TEST_AUTH'] === '1') {
    const e2ePassword = process.env['E2E_TEST_TOKEN'] ?? password
