...
test.describe("Astro SSR Functionality Tests", () => {
  ...
  test("Admin dashboard renders static HTML (SSR, no interactive islands)", async ({ page }) => {
  ...
  test("Astro View Transitions work correctly", async ({ page }) => {
    ...
    const transitionsEnabled = await page.evaluate(() => {
      const meta = document.querySelector('meta[name="astro-view-transitions-enabled"]');
