...
mobileDevices.forEach((device: { name?: string; viewport?: { width: number; height: number } }) => {
  test.describe(`Mobile Responsiveness - ${device.name ?? "unknown"}`, () => {
    test.use({ ...(device as any) });
    ...
    test("Demo components are responsive on mobile", async ({ page }) => {
      ...
      await test.step("Validation Mobile UI", async () => {
        ...
        expect(textAreaBox?.width).toBeLessThanOrEqual((viewport?.width ?? 0) - 40); // Account for padding
        ...
        if (validationCardCount > 1) {
          const firstValidationCard = await validationCards.first().boundingBox();
          const secondValidationCard = await validationCards.nth(1).boundingBox();

          if (firstValidationCard && secondValidationCard) {
            expect(secondValidationCard.y).toBeGreaterThan(firstValidationCard.y);
          }
        }
      });
      ...
      await test.step("Category Balancing Mobile UI", async () => {
        ...
        if (categoryCount > 0) {
          ...
          for (let i = 0; i < Math.min(categoryCount, 3); i++) {
            ...
            expect(cardBox?.width).toBeLessThanOrEqual((viewport?.width ?? 0) - 20);
          }
        }
      ...
      await test.step("Export Mobile UI", async () => {
        ...
        if (formatCount > 0) {
          ...
          for (let i = 0; i < Math.min(formatCount, 3); i++) {
            ...
            expect(optionBox?.width).toBeLessThanOrEqual((viewport?.width ?? 0) - 20);
          }
        }
