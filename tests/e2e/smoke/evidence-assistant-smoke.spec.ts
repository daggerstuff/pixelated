...
test.describe("Evidence Assistant admin page", () => {
  test("page is accessible and includes the admin UI shell", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "AI Evidence Assistant" })).toBeVisible();
    await expect(page.getByText("Internal evidence-backed assistant for product")).toBeVisible();
  });
