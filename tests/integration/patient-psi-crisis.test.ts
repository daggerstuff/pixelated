...
describe("Patient-Psi Crisis Integration Tests", () => {
  ...
  describe("End-to-End Crisis Detection and Response", () => {
    it("should detect crisis and provide contextual intervention", async () => {
      ...
      expect(result.recommendations).toContain("Activate crisis intervention protocol");
    ...
    it("should track therapeutic progress over multiple sessions", async () => {
      const mockProfile: PatientPsiProfile = {
        ...
        therapeuticGoals: ["improve emotional regulation", "build coping skills"],
        culturalFactors: {
      ...
      const result = await contextualEnhancer.enhanceContext(
        ...
        currentEmotion,
        mockProfile,
        ["I used the breathing technique we practiced", "It helped me stay calmer"],
      );
  ...
  describe("Performance Benchmarks", () => {
    it("should complete contextual analysis within 100ms", async () => {
      ...
      await contextualEnhancer.enhanceContext("perf-test-session", emotion, mockProfile, [
    ...
    it("should handle large conversation histories efficiently", async () => {
      ...
      const largeHistory = Array.from(
        { length: 100 },
        (_, i) => `Message ${i + 1}: This is a test conversation line for performance testing`,
      );
  ...
  describe("Clinical Accuracy Validation", () => {
    it("should maintain high accuracy for crisis detection", async () => {
      const testCases = [
        {
          ...
          conversation: ["I feel like giving up", "Life has no meaning anymore"],
          expectedCrisis: true,
        },
        {
          emotion: {
      ...
      for (const testCase of testCases) {
        ...
        expect(result.interventionTiming.shouldIntervene).toBe(testCase.expectedCrisis);
      }
    });
  });
});
