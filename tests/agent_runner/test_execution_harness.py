...
from tools.agent_runner.models import AgentConfig, AgentRole, ExecutionResult, LinearIssue, RunnerConfig, VerificationConfig
...
def test_harness_anti_hollow_gate_catches_fake_mocks():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        exec_res = ExecutionResult(agent_name="opencode", ticket_identifier="PIX-9999", success=True, output="Implemented metrics", stderr="")
        gates, files = harness._run_quality_gauntlet(tmpdir, exec_res, issue)
...
def test_harness_anti_suppression_gate_catches_tags():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        exec_res = ExecutionResult(agent_name="opencode", ticket_identifier="PIX-9998", success=True, output="Fixed types", stderr="")
        gates, files = harness._run_quality_gauntlet(tmpdir, exec_res, issue)
