...
const CONSOLIDATION_URL = process.env["DREAM_CONSOLIDATION_URL"] ?? "http://localhost:5000";
...
async function startWorker(): Promise<void> {
  ...
  if (USER_WHITELIST.length === 0) {
    logger.warn("DREAM_USER_WHITELIST not set — worker will idle without processing");
  } else {
...
async function runConsolidationCycle(): Promise<void> {
  const targets = USER_WHITELIST.length > 0 ? USER_WHITELIST : await fetchActiveUsers();
  if (targets.length === 0) {
  ...
  for (const userId of targets) {
    try {
      ...
      const response = await fetch(`${CONSOLIDATION_URL}/api/dream/consolidate`, {
        method: "POST",
