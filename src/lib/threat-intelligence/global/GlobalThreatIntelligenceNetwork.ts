  ...
  implements GlobalThreatIntelligenceNetwork
{
  ...
  private async initializeSubsystems(): Promise<void> {
    try {
      ...
      this.validationSystem = new ThreatValidationSystem(
  ...
  async processThreatIntelligence(
    ...
    try {
      ...
      const existingThreat =
        await this.checkExistingGlobalThreat(validatedData);
  ...
  private async validateThreatData(
    ...
    if (threatData.confidence < 0 || threatData.confidence > 1) {
      throw new Error(
        "Invalid threat data: confidence must be between 0 and 1",
      );
    }
