...
async function deriveKey(salt: Uint8Array): Promise<CryptoKey> {
  ...
  if (encryptionKey.length < MIN_PASSWORD_LENGTH) {
    throw new Error(`Encryption key must be at least ${MIN_PASSWORD_LENGTH} characters long`);
  }
...
export async function decrypt(encryptedDataStr: string): Promise<unknown> {
  try {
    const { iv, data, tag, salt } = JSON.parse(encryptedDataStr) as EncryptedData;
