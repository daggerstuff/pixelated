...
type ExportStatus = "pending" | "processing" | "completed" | "failed" | "expired" | "cancelled";
...
interface ExportDownloadErrorResponse {
  success: false;
  error: "not_found" | "unauthorized" | "not_ready" | "expired" | "internal_error";
  message?: string;
  status?: ExportStatus;
  progress?: number;
  estimatedCompletionTime?: Date;
...
type ExportDownloadResponse = ExportDownloadSuccessResponse | ExportDownloadErrorResponse;
 ...
 */
export async function createDataExportRequest(input: ExportRequestInput): Promise<ExportResponse> {
  try {
    ...
    const hasAccess = await verifyPatientDataAccess(input.patientId, input.requestedBy);
    if (!hasAccess) {
...
export async function getDataExportDetails(
  ...
  try {
    ...
    const estimatedCompletionTime = new Date(createdAt.getTime() + 5 * 60 * 1000); // 5 minutes from creation
    ...
    const expiresAt = completedAt ? new Date(completedAt.getTime() + 24 * 60 * 60 * 1000) : null; // 24 hours after completion
    ...
    const typedExportRequest = exportRequest as unknown as DataExportRequestWithFormat;
    ...
    return {
      ...
      completedAt: exportRequest.completedAt ? exportRequest.completedAt.toISOString() : undefined,
      downloadUrl: exportRequest.files?.find((f) => f.format === typedExportRequest.format)?.url,
 ...
 */
async function verifyPatientDataAccess(patientId: string, userId: string): Promise<boolean> {
  try {
 ...
 */
export async function getDataExportRequest(id: string): Promise<DataExportRequest | null> {
  try {
...
export async function downloadDataExport(
  ...
  try {
    ...
    if (
      ...
      const expirationDate = new Date(completedAt.getTime() + 24 * 60 * 60 * 1000); // 24 hours after completion
...
interface MockDb {
  ...
  providerPatientAccess: {
    findFirst: (params: { where: unknown }) => Promise<ProviderPatientAccess | null>;
  };
  user: {
...
const mockDb: MockDb = Object.assign(Object.create(db) as Record<string, unknown>, {
  ...
  providerPatientAccess: {
    findFirst: async (_params: { where: unknown }): Promise<ProviderPatientAccess | null> => {
      return Promise.resolve(null);
    },
  },
  user: {
