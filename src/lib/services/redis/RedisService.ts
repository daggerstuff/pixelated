...
export class RedisService extends EventEmitter implements IRedisService {
      ...
      if (redisPasswordFile && fs.existsSync(redisPasswordFile)) {
        try {
          ...
          if (password) {
            ...
            logger.info(`[RedisService] Password loaded from ${redisPasswordFile}`);
  ...
  async connect(): Promise<void> {
    try {
      ...
      if (!this.config.url && process.env["NODE_ENV"] === "development") {
        logger.warn("No Redis URL configured, skipping connection in development");
      ...
      if (process.env["NODE_ENV"] === "development") {
        logger.warn("Failed to connect to Redis in development, will use mock:", {
  ...
  private createCommandClient(rawClient: Redis): RedisClient {
    ...
    const createPipeline = () => {
      ...
      const pipeline = {
        ...
        sadd: (key: string, member: string) => addCommand("sadd", [key, member]),
        expire: (key: string, seconds: number) => addCommand("expire", [key, seconds]),
        del: (...keys: string[]) => {
          if (keys.length > 0) {
            addCommand("del", keys);
          }
    ...
    client = {
      ...
      sadd: async (key: string, member: string) => toNumber(await invoke("sadd", key, member)),
      srem: async (key: string, member: string) => toNumber(await invoke("srem", key, member)),
      ...
      hdel: async (key: string, field: string) => toNumber(await invoke("hdel", key, field)),
      ...
      zrem: async (key: string, member: string) => toNumber(await invoke("zrem", key, member)),
      zrange: async (key: string, start: number, stop: number, withScores?: "WITHSCORES") =>
        toStringArray(
          await invoke(
            "zrange",
            ...(withScores === "WITHSCORES" ? [key, start, stop, withScores] : [key, start, stop]),
          ),
        ),
  ...
  private createMockClient(): RedisMockClient {
    ...
    const createMockPipeline = () => {
      ...
      const pipeline = {
        setex: (key: string, _seconds: number, value: string) => addCommand("setex", [key, value]),
        sadd: (key: string, member: string) => addCommand("sadd", [key, member]),
          ...
          const commandsToResolve = commands.map(
            async (command): Promise<[Error | null, unknown]> => {
              try {
                ...
                if (command.cmd === "del") {
                  const keys = command.args.filter((arg): arg is string => typeof arg === "string");
                ...
                if (command.cmd === "get") {
                  ...
                  return [null, typeof key === "string" ? (store.get(key) ?? null) : null];
                }
    ...
    const mockClient: RedisMockClient = {
      ...
      deletePattern: async (pattern: string) => {
        const regex = new RegExp("^" + pattern.replace(/\*/g, ".*") + "$");
        const keysToDelete = Array.from(store.keys()).filter((key) => regex.test(key));
  ...
  async keys(pattern: string): Promise<string[]> {
    try {
      ...
      try {
        ...
        if (error instanceof Error && error.message.includes("ERR KEYS command is disabled")) {
          const results: string[] = [];
          let cursor = "0";

          do {
            const scanResult = await client.scan(cursor, "MATCH", pattern, "COUNT", 1000);
  ...
  async getPoolStats(): Promise<{
    ...
    try {
      ...
      stats.idleConnections = Math.max(0, stats.totalConnections - stats.activeConnections);
    ...
    try {
      ...
      if (withScores === "WITHSCORES") {
        ...
        for (let i = 0; i < result.length; i += 2) {
          if (typeof result[i] === "string" && typeof result[i + 1] === "string") {
            arr.push({
              value: result[i],
