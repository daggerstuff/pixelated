...
async function expectKeyExists(redisService: RedisService, key: string): Promise<void> {
  expect(await getRedisClientOrThrow(redisService).exists(key)).toBe(1);
}

async function expectKeyDoesNotExist(redisService: RedisService, key: string): Promise<void> {
  expect(await getRedisClientOrThrow(redisService).exists(key)).toBe(0);
}

async function getMatchingKeys(redisService: RedisService, pattern: string): Promise<string[]> {
...
const SKIP_REDIS_TESTS = process.env["SKIP_REDIS_TESTS"] === "true" || process.env["CI"] === "true";
...
const describeCacheInvalidation = shouldRunCacheInvalidationTests ? noopDescribe : describe;
...
describeCacheInvalidation("cacheInvalidation Integration", () => {
  ...
  describe("cache Pattern Invalidation", () => {
    ...
    it("should handle concurrent pattern invalidations", async () => {
      // Set up test data
      const patterns = Array.from({ length: 3 }, () => generateTestKey("concurrent"));
  ...
  describe("error Handling", () => {
    it("should handle Redis connection failures during invalidation", async () => {
      ...
      await expect(cacheInvalidation.invalidatePattern(`${pattern}:*`)).rejects.toThrow();
  ...
  describe("performance", () => {
    ...
    it("should maintain performance under concurrent load", async () => {
      ...
      const patterns = Array.from({ length: 10 }, (_, i) => `${basePattern}:${i}`);
