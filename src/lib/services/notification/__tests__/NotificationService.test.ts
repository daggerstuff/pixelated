...
describe("notificationService", () => {
  ...
  const hasClient = (service: NotificationService, userId: string): boolean => {
    return (service as unknown as NotificationServiceTestInterface).wsClients.has(userId);
  };
  ...
  beforeEach(() => {
    ...
    vi.spyOn(notificationService as unknown as NotificationServiceTestInterface, "deliverInApp");
  });
  ...
  describe("registerTemplate", () => {
    it("should register a template successfully", async () => {
      ...
      expect(
        (notificationService as unknown as NotificationServiceTestInterface).emailService
          .upsertTemplate,
    ...
    it("should validate template data", async () => {
      ...
      await expect(notificationService.registerTemplate(invalidTemplate)).rejects.toThrow();
    });
  });
  ...
  describe("queueNotification", () => {
    ...
    it("should validate notification data", async () => {
      const invalidNotification = {
        ...
        templateId: "test",
        data: null,
      } as unknown as Parameters<typeof notificationService.queueNotification>[0];

      await expect(notificationService.queueNotification(invalidNotification)).rejects.toThrow();
    });
    ...
    it("should throw error for non-existent template", async () => {
      ...
      await expect(notificationService.queueNotification(notification)).rejects.toThrow();
    });
  });
  ...
  describe("processQueue", () => {
    ...
    it("should handle delivery errors", async () => {
      ...
      vi.mocked(redis["rpoplpush"]).mockResolvedValueOnce(JSON.stringify(queueItem));
      vi.spyOn(
        notificationService as unknown as NotificationServiceTestInterface,
        "deliverPush",
  ...
  describe("markAsRead", () => {
    ...
    it("should throw error for non-existent notification", async () => {
      ...
      await expect(notificationService.markAsRead("test-user", "test-id")).rejects.toThrow();
    });
  });
  ...
  describe("getNotifications", () => {
    ...
    it("should handle pagination", async () => {
      ...
      const result = await notificationService.getNotifications("test-user", 5, 2);
  ...
  describe("webSocket integration", () => {
    ...
    it("should deliver in-app notifications via WebSocket", async () => {
      ...
      await (notificationService as unknown as NotificationServiceTestInterface).deliverInApp(
        notification,
      );

      expect(vi.spyOn(ws, "send")).toHaveBeenCalledWith(expect.stringContaining(notification.id));
    });
  });
});
