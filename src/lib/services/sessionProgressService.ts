...
import type { SessionProgressMetrics, TherapistSession } from "@/types/dashboard";
...
export async function getTherapistSkillProgress(
  ...
  try {
    ...
    const data = (await response.json()) as {
      skills: TherapistSkillProgressData[];
    };
    return data.skills ?? [];
  } catch (error: unknown) {
...
export async function saveSessionProgressBatch(session: TherapistSession): Promise<boolean> {
  try {
    ...
    if (session.progressMetrics) {
      ...
      if (session.progressMetrics.skillScores) {
        await saveSkillScores(session.id, session.therapistId, session.progressMetrics.skillScores);
      }
