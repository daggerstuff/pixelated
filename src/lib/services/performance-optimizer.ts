...
export class PerformanceOptimizer {
  ...
  set(key: string, value: unknown): void {
    ...
    // Evict expired entries - optimized to stop early since insertion order is temporal
    this.evictExpired();

    // If key already exists, delete it so we can re-insert at the end (for LRU/FIFO)
  ...
  get(key: string): unknown | null {
    ...
    // For LRU, move to the end of the Map to mark as most recently used
    if (this.config.cache.strategy === "LRU") {
      this.cache.delete(key);
      entry.timestamp = now; // Update timestamp for LRU and TTL
  ...
  private evictExpired() {
    ...
    // Optimization: Since we re-insert on access/update,
    // the first entries in the Map are the oldest by timestamp.
    for (const [key, entry] of this.cache.entries()) {
      if (now - entry.timestamp > ttl) {
        this.cache.delete(key);
      } else {
        // Stop early: subsequent entries are newer
        break;
      }
    }
  }
  ...
  private evictByStrategy() {
    ...
    switch (this.config.cache.strategy) {
      case "LRU":
      case "FIFO":
        // O(1) eviction for LRU/FIFO using Map insertion order
  ...
  private updateMetrics() {
    // Update cache hit rate - O(1) using counters
