...
export class ExternalThreatIntelligenceService extends EventEmitter {
  ...
  private async initializeServices(): Promise<void> {
    try {
      ...
      this.redis = new Redis(
        this.config.redisUrl ?? process.env["REDIS_URL"] ?? "redis://localhost:6379",
      );
  ...
  private transformIntelligenceItem(
    ...
    try {
      ...
      const severity = this.mapSeverity(String(data["severity"] ?? data["confidence"] ?? "medium"));
      ...
      return {
        ...
        attribution: data["attribution"]
          ? {
              ...
              campaign:
                ((data["attribution"] as Record<string, unknown>)["campaign"] as string) ||
                "unknown",
              family:
      ...
      if (intelligenceFindings.length > 0) {
        enrichedResponse.metadata = {
          ...
          externalIntelligence: {
            ...
            sources: Array.from(
              new Set(intelligenceFindings.flatMap((f) => f["sources"] as string[])),
            ),
          },
        };
      }
  ...
  async getStatistics(): Promise<{
    ...
    try {
      ...
      const [
        ...
        db.collection("threat_intelligence").countDocuments({
          $or: [{ expirationDate: { $exists: false } }, { expirationDate: { $gt: new Date() } }],
        }),
  ...
  private async getFeedStatistics(
    db: Db,
  ): Promise<Record<string, { total: number; active: number; lastUpdate: Date }>> {
