...
describe('Threat Hunting Service', () => {
  ...
  beforeEach(() => {
    ...
    mockBehavioralService = {
      analyzeUserBehavior:
        vi.fn<() => Promise<{ userId: string; riskScore: number }>>(),
      getBehavioralProfile:
        vi.fn<() => Promise<{ profile: unknown; lastUpdated: Date }>>(),
      detectAnomalies:
        vi.fn<() => Promise<{ anomalies: unknown[]; confidence: number }>>(),
    }

    mockPredictiveService = {
      predictThreats:
        vi.fn<() => Promise<{ threats: unknown[]; confidence: number }>>(),
      getThreatForecast:
        vi.fn<() => Promise<{ forecast: unknown[]; period: string }>>(),
      analyzeTrends:
        vi.fn<() => Promise<{ trends: unknown[]; patterns: string[] }>>(),
    }
