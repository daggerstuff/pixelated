...
describe("QueryAuditService", () => {
  ...
  describe("logQuery", () => {
    it("should log a query with all fields", () => {
      ...
      const entry = service.logQuery(query, "admin", "admin", result, 0.1, 42, false);
    ...
    it("should capture result shape with fields and data types", () => {
      const entry = service.logQuery(makeQuery(), "admin", "admin", makeResult(), 0.1, 42, false);
    ...
    it("should handle empty result data", () => {
      ...
      const entry = service.logQuery(makeQuery(), "admin", "admin", result, 0.1, 10, false);
    ...
    it("should log error status", () => {
      ...
      const entry = service.logQuery(makeQuery(), "admin", "admin", result, 0.1, 5, false);
    ...
    it("should log pending-approval status", () => {
      ...
      const entry = service.logQuery(makeQuery(), "admin", "admin", result, 0.1, 3, false);
    ...
    it("should record custom epsilon", () => {
      const entry = service.logQuery(makeQuery(), "admin", "admin", makeResult(), 0.5, 10, false);
  ...
  describe("FIFO eviction", () => {
    it("should evict oldest entries when maxEntries exceeded", () => {
      for (let i = 0; i < 10001; i++) {
        ...
        service.logQuery(query, "admin", "admin", makeResult({ queryId: query.id }), 0.1, 1, false);
      }
  ...
  describe("getAuditTrail", () => {
    it("should return entries sorted by timestamp descending", async () => {
      service.logQuery(makeQuery({ id: "q1" }), "admin", "admin", makeResult(), 0.1, 1, false);
      await new Promise((r) => setTimeout(r, 10));
      service.logQuery(makeQuery({ id: "q2" }), "admin", "admin", makeResult(), 0.1, 1, false);
    ...
    it("should filter by date range", () => {
      ...
      service.logQuery(makeQuery({ id: "q-old" }), "admin", "admin", makeResult(), 0.1, 1, false);
      const entries = (service as any).entries as QueryAuditEntry[];
      entries[0].timestamp = oldDate;

      service.logQuery(makeQuery({ id: "q-new" }), "admin", "admin", makeResult(), 0.1, 1, false);
  ...
  describe("clear", () => {
    it("should clear all entries", () => {
      service.logQuery(makeQuery(), "admin", "admin", makeResult(), 0.1, 10, false);
...
describe("QueryOutputFormatter", () => {
  ...
  beforeEach(async () => {
    const { getQueryOutputFormatter, resetQueryOutputFormatter } = await import(
      "../services/QueryOutputFormatter"
    );
...
describe("QueryDSL", () => {
  beforeEach(async () => {
    const { resetQueryAuditService } = await import("../services/QueryAuditService");
    resetQueryAuditService();
  });
  ...
  it("should convert DSL to SQL with filters", async () => {
    ...
    expect(sql).toContain("SELECT technique_type, AVG(emotion_score) AS avg_emotion");
  ...
  it("should use default values when not specified", async () => {
    ...
    const query = createQueryFromRequest({ description: "Minimal query" }, "admin");
