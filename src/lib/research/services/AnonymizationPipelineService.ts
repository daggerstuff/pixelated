...
export class AnonymizationPipelineService {
  ...
  async anonymizeTherapeuticData(
    ...
    try {
      ...
      const kAnonymizedData = await this.applyKAnonymity(preprocessedData, finalConfig.kAnonymity);
      ...
      const qualityMetrics = await this.assessAnonymizationQuality(rawData, finalAnonymizedData);
  ...
  private async applyTemporalObfuscation(
    ...
    return data.map((record) => {
      ...
      if (seasonalMasking && record["date"]) {
        (obfuscatedRecord as Record<string, unknown>)["temporal_period"] = this.getTemporalPeriod(
      ...
      if (record["session_sequence"]) {
        (obfuscatedRecord as Record<string, unknown>)["session_sequence_range"] =
    ...
    return data.map((record) => {
      ...
      return {
        ...
        qualityMetrics: {
          ...
          privacyBudgetUsed:
            ((record["differential_privacy"] as Record<string, unknown> | undefined)?.[
              "epsilon_used"
            ] as number) ?? 0,
        },
      } as AnonymizedRecord;
  ...
  private async assessAnonymizationQuality(
    ...
    // Assess privacy risk
    const minKAnonymity = Math.min(...anonymizedData.map((r) => r.qualityMetrics.kAnonymityLevel));
  ...
  private async mergeOrSuppressGroup(
    ...
    logger.warn(`Suppressing ${smallGroup.length} records that couldn't achieve k-anonymity`);
    return [];
  }
