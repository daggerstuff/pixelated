...
export class PatternDiscoveryService {
  ...
  async discoverCorrelations(
    ...
    return (result.data as Record<string, unknown>[]).map((row: Record<string, unknown>) => ({
      variables: [String(row["variable1"]), String(row["variable2"])],
      correlation: Number(row["correlation"]),
      pValue: Number(row["p_value"]),
      sampleSize: Number(row["sample_size"]),
      confidenceInterval: [Number(row["ci_lower"]), Number(row["ci_upper"])],
    }));
  }
   ...
   */
  async discoverTrends(metrics: string[], _request: DiscoveryRequest): Promise<TrendPattern[]> {
  ...
  private describePattern(
    pattern: CorrelationPattern | TrendPattern | AnomalyPattern | ClusterPattern,
  ): string {
