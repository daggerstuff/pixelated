...
function incompleteBeta(x: number, a: number, b: number): number {
  ...
  for (let iter = 0; iter < maxIter && Math.abs(delta) > epsilon; iter++) {
    ...
    const numerator2 = (-(a + m - 1) * (a + b + m - 1) * x) / ((a + d) * (a + d + 1));
...
function lnGamma(x: number): number {
  const cof = [
    76.1800917294715, -86.5053203294168, 24.0140982408309, -1.2317395724502, 0.1208650973866e-2,
    -0.5395239385e-5,
  ];
  let y = x;
  let tmp = x + 5.5;
...
function upperIncompleteGamma(a: number, x: number): number {
  ...
  // Continued fraction
  return Math.exp(-x + a * Math.log(x) - lnGamma(a)) * gammaContinuedFraction(a, x);
}
 ...
 */
export function welchTTest(group1: number[], group2: number[], alpha = 0.05): TTestResult {
 ...
 * @param alpha Significance level
 */
export function chiSquareIndependence(table: number[][], alpha = 0.05): ChiSquareResult {
 ...
 * @param alpha Significance level
 */
export function spearmanCorrelation(x: number[], y: number[], alpha = 0.05): CorrelationResult {
  const n = x.length;
  if (n !== y.length || n < 3) {
...
function normalQuantile(p: number): number {
  const a = [
    -3.969683028665376e1, 2.209460983245205e2, -2.759285104469687e2, 1.38357751867269e2,
    -3.066479806614716e1, 2.506628277459239, -3.969683028665376e1, 2.209460983245205e2,
    -2.759285104469687e2, 1.38357751867269e2, -3.066479806614716e1, 2.506628277459239,
    -3.969683028665376e1, 2.209460983245205e2, -2.759285104469687e2, 1.38357751867269e2,
    -3.066479806614716e1, 2.506628277459239, -3.969683028665376e1, 2.209460983245205e2,
    -2.759285104469687e2, 1.38357751867269e2, -3.066479806614716e1, 2.506628277459239,
    -3.969683028665376e1, 2.209460983245205e2, -2.759285104469687e2, 1.38357751867269e2,
