...
import { getDedicatedResourceHistory, getGenericResourceHistory } from '../repositories/index.js';
...
describe('getResourceHistory', () => {
  ...
  it('returns 200 with a history Bundle for a dedicated resource', async () => {
    const mockHistory = [
      { resource: { resourceType: 'Patient', id: 'p1' }, timestamp: '2024-01-01T00:00:00Z', action: 'create' },
      { resource: { resourceType: 'Patient', id: 'p1' }, timestamp: '2024-01-02T00:00:00Z', action: 'update' },
    ];
    vi.mocked(getDedicatedResourceHistory).mockResolvedValue(mockHistory);

    const result = await getResourceHistory('Patient', 'p1', mockContext, BASE_URL);
  ...
  it('returns 200 with a history Bundle for a generic resource', async () => {
    const mockHistory = [
      { resource: { resourceType: 'Condition', id: 'c1' }, timestamp: '2024-01-01T00:00:00Z', action: 'create' },
    ];
    vi.mocked(getGenericResourceHistory).mockResolvedValue(mockHistory);

    const result = await getResourceHistory('Condition', 'c1', mockContext, BASE_URL);
  ...
  it('returns 404 when no history exists', async () => {
    ...
    const result = await getResourceHistory('Patient', 'nonexistent', mockContext, BASE_URL);
  ...
  it('includes entry request method based on action', async () => {
    const mockHistory = [
      { resource: { resourceType: 'Patient', id: 'p1' }, timestamp: '2024-01-01T00:00:00Z', action: 'create' },
      { resource: { resourceType: 'Patient', id: 'p1' }, timestamp: '2024-01-03T00:00:00Z', action: 'delete' },
    ];
    vi.mocked(getDedicatedResourceHistory).mockResolvedValue(mockHistory);
  ...
  it('returns 500 on error', async () => {
    vi.mocked(getDedicatedResourceHistory).mockRejectedValue(new Error('DB error'));
