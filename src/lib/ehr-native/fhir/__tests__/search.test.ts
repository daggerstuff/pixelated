...
import { searchDedicatedResources, searchGenericResources } from '../repositories/index.js';
...
describe('searchResources', () => {
  ...
  it('returns 200 with a Bundle for a dedicated resource type', async () => {
    ...
    const result = await searchResources('Patient', params, mockContext, BASE_URL);
  ...
  it('returns 200 with a Bundle for a generic resource type', async () => {
    ...
    const params = new URLSearchParams();
    const result = await searchResources('Condition', params, mockContext, BASE_URL);
