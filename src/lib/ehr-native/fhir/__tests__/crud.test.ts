...
describe('createResource', () => {
  ...
  it('creates a valid Patient and returns 201 with Location header', async () => {
    vi.mocked(createDedicatedResource).mockResolvedValue({ ...validPatient, id: 'test-uuid-001' });
    vi.mocked(insertDedicatedResourceHistory).mockResolvedValue(undefined);

    const result = await createResource('Patient', validPatient, mockContext, BASE_URL);

    expect(result.status).toBe(201);
    expect(result.headers['Location']).toBe(`${BASE_URL}/Patient/${(result.body as Record<string, unknown>)['id']}`);
  ...
  it('creates a generic resource (Condition)', async () => {
    const condition = { resourceType: 'Condition', subject: { reference: 'Patient/p1' } };
    vi.mocked(createGenericResource).mockResolvedValue({ ...condition, id: 'cond-1' });
    vi.mocked(insertGenericResourceHistory).mockResolvedValue(undefined);

    const result = await createResource('Condition', condition, mockContext, BASE_URL);
  ...
  it('returns 422 for type mismatch', async () => {
    const wrongType = { resourceType: 'Practitioner', name: [{ family: 'Doe' }] };
    const result = await createResource('Patient', wrongType, mockContext, BASE_URL);
  ...
  it('returns 422 for validation failure', async () => {
    ...
    const result = await createResource('Patient', invalidPatient, mockContext, BASE_URL);
...
describe('updateResource', () => {
  ...
  it('updates an existing resource and returns 200', async () => {
    ...
    vi.mocked(updateDedicatedResource).mockResolvedValue({ ...validPatient, id: 'p1' });
    vi.mocked(insertDedicatedResourceHistory).mockResolvedValue(undefined);

    const result = await updateResource('Patient', 'p1', { ...validPatient, id: 'p1' }, mockContext, null);
  ...
  it('returns 422 for type mismatch', async () => {
    const wrongType = { resourceType: 'Practitioner', id: 'p1', name: [{ family: 'Doe' }] };
    const result = await updateResource('Patient', 'p1', wrongType, mockContext, null);
  ...
  it('returns 409 when body id does not match path id', async () => {
    ...
    const result = await updateResource('Patient', 'p1', { ...validPatient, id: 'different-id' }, mockContext, null);
  ...
  it('returns 412 when If-Match provided but resource does not exist', async () => {
    ...
    const result = await updateResource('Patient', 'nonexistent', { ...validPatient, id: 'nonexistent' }, mockContext, 'W/"1"');
