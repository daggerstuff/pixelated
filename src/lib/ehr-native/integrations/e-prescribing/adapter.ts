...
export interface EPrescribingAdapter {
  ...
  searchPharmacies(request: PharmacySearchRequest): Promise<PharmacySearchResponse>

  /** Verify controlled substance prescribing eligibility (PDMP + EPCS). */
  checkControlledSubstance(request: ControlledSubstanceCheckRequest): Promise<ControlledSubstanceCheckResult>

  /** Check for drug interactions with active medications. */
  checkDrugInteractions(request: DrugInteractionCheckRequest): Promise<DrugInteractionCheckResponse>

  /** Transmit a prescription to a pharmacy. */
  transmitPrescription(request: PrescriptionTransmissionRequest): Promise<PrescriptionTransmissionResponse>

  /** Check the status of a transmitted prescription. */
  checkPrescriptionStatus(request: PrescriptionStatusRequest): Promise<PrescriptionStatusResponse>

  /** Cancel a transmitted prescription. */
  cancelPrescription(request: PrescriptionCancelRequest): Promise<PrescriptionCancelResponse>
}
