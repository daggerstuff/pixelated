...
const validAppointment = {
  ...
  serviceType: [{ coding: [{ system: 'http://hl7.org/fhir/ValueSet/service-type', code: 'therapy' }] }],
  start: '2026-01-15T10:00:00Z',
  end: '2026-01-15T11:00:00Z',
  participant: [
    { actor: { reference: `Patient/${validPatientId}` }, status: 'accepted' as const },
    { actor: { reference: 'Practitioner/provider-001' }, status: 'accepted' as const },
  ],
}
...
describe('SchedulingService', () => {
  ...
  describe('createAppointment', () => {
    it('creates an appointment through the repository', async () => {
      ...
      const result = await service.createAppointment({ fhirResource: validAppointment })
  ...
  describe('cancelAppointment', () => {
    it('cancels an appointment with reason', async () => {
      const cancelled = { ...validAppointment, status: 'cancelled' as const, cancelationReason: 'Patient request' }
      mockAppointmentRepo.findById.mockResolvedValue(validAppointment)
      mockAppointmentRepo.update.mockResolvedValue(cancelled)
      const result = await service.cancelAppointment(validAppointmentId, 'Patient request')
  ...
  describe('getPatientAppointments', () => {
    it('returns appointments for a valid patient', async () => {
      ...
      const result = await service.getPatientAppointments(validPatientId, { limit: 10, offset: 0 })
      expect(result).toEqual([validAppointment])
    })
  })
  ...
  describe('getPractitionerSchedule', () => {
    it('returns appointments for a practitioner (non-upcoming)', async () => {
      mockAppointmentRepo.findByPractitioner.mockResolvedValue([validAppointment])
      const result = await service.getPractitionerSchedule(validAppointmentId, { upcomingOnly: false, limit: 20, offset: 0 })
      expect(result).toEqual([validAppointment])
    })

    it('returns upcoming appointments for a practitioner', async () => {
      mockAppointmentRepo.findUpcomingByPractitioner.mockResolvedValue([validAppointment])
      const result = await service.getPractitionerSchedule(validAppointmentId, { upcomingOnly: true, limit: 20, offset: 0 })
      expect(result).toEqual([validAppointment])
    })
  })
  ...
  describe('getAppointmentsByStatus', () => {
    it('returns appointments filtered by status', async () => {
      ...
      const result = await service.getAppointmentsByStatus('booked', { limit: 50, offset: 0 })
      expect(result).toEqual([validAppointment])
    })
  })
