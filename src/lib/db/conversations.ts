...
export async function updateConversation(
  ...
  // Log the event for HIPAA compliance
  await createAuditLog(AuditEventType.MODIFY, "conversation_updated", userId, "conversations", {
    conversationId: id,
    updates,
    ipAddress: getRequestHeader(request, "x-forwarded-for"),
    userAgent: getRequestHeader(request, "user-agent"),
...
export async function deleteConversation(
  ...
  // Log the event for HIPAA compliance
  await createAuditLog(AuditEventType.DELETE, "conversation_deleted", userId, "conversations", {
    conversationId: id,
    ipAddress: getRequestHeader(request, "x-forwarded-for"),
    userAgent: getRequestHeader(request, "user-agent"),
  });
