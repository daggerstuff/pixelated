...
const EDUCATIONAL_PATTERNS = {
  ...
  [EducationalType.EXPLANATION]: [/\b(?:how does|how do|how to|why|explain|explanation|work)\b/i],
  ...
  [EducationalType.RESEARCH]: [/\b(?:research|latest|studies|study|findings)\b/i],
};
...
export class EducationalContextRecognizer {
    ...
    try {
      ...
      return {
        ...
        metadata: {
          ...
          practicalApplications: Array.isArray(parsed.metadata?.practicalApplications)
  ...
  private validateResources(resources: unknown): ResourceType[] {
    ...
    return resources.filter((r) => Object.values(ResourceType).includes(r)).slice(0, 5); // Limit to 5 resources
  }
