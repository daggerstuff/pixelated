...
import { ObjectiveEvaluationResult, AlignmentEvaluationResult } from "./objective-interfaces";
import {
  ObjectiveDefinition,
  AlignmentContext,
  ObjectiveCriteria,
...
export class ObjectiveMetricsEngine {
  ...
  calculateObjectiveMetrics(
    ...
    const improvement = this.calculateImprovement(objective.id, evaluationResult.score);
  ...
  calculateAlignmentMetrics(
    ...
    const contextualAlignment = this.calculateOverallContextualAlignment(objectiveMetrics);
   ...
   */
  addEvaluation(evaluation: AlignmentEvaluationResult, objectives: ObjectiveDefinition[]): void {
    ...
    if (this.evaluationHistory.evaluations.length > this.config.historyWindow * 2) {
      this.evaluationHistory.evaluations = this.evaluationHistory.evaluations.slice(
  ...
  private calculateCriteriaMetrics(
    ...
    return criteria.map((criterion) => {
      ...
      const confidence = this.calculateCriterionConfidence(criterion, score, context);
  ...
  private calculateConfidence(
    ...
    const historicalConfidence = this.getHistoricalConfidence(evaluationResult.objectiveId);
  ...
  private calculateImprovement(objectiveId: string, currentScore: number): number {
  ...
  private calculateTrend(objectiveId: string): EvaluationTrend {
    const recentEvaluations = this.getRecentEvaluations(objectiveId, this.config.historyWindow);
    ...
    const meanY = sumY / n;
    const totalSumSquares = scores.reduce((sum, score) => sum + Math.pow(score - meanY, 2), 0);
  ...
  private calculateContextualFit(
    ...
    switch (context.detectedContext) {
      ...
      case "clinical_assessment":
        if (objective.id === "correctness" || objective.id === "professionalism") {
          fit += 0.4;
        }
        break;
      case "educational":
        if (objective.id === "informativeness" || objective.id === "correctness") {
          fit += 0.4;
        }
        break;
  ...
  private calculateBalanceScore(objectiveMetrics: Record<string, ObjectiveMetrics>): number {
  ...
  private calculateOverallConsistency(objectiveMetrics: Record<string, ObjectiveMetrics>): number {
    const consistencyScores = Object.values(objectiveMetrics).map((m) => m.consistency);
  ...
  private calculateOverallImprovement(objectiveMetrics: Record<string, ObjectiveMetrics>): number {
    const improvementScores = Object.values(objectiveMetrics).map((m) => m.improvement);

    if (improvementScores.length === 0) {
      return 0;
    }

    return improvementScores.reduce((sum, score) => sum + score, 0) / improvementScores.length;
  }

  private calculateOverallContextualAlignment(
    objectiveMetrics: Record<string, ObjectiveMetrics>,
  ): number {
    const alignmentScores = Object.values(objectiveMetrics).map((m) => m.contextualFit);
  ...
  private calculateQualityIndicators(
    ...
    const reliability = metrics.reduce((sum, m) => sum + m.reliability, 0) / metrics.length || 0;
  ...
  private generatePerformanceProfile(
    ...
    const opportunities = Object.entries(objectiveMetrics)
      .filter(([, metrics]) => metrics.improvement < 0 && metrics.trend.direction === "improving")
      .map(([id]) => id);

    const risks = Object.entries(objectiveMetrics)
      .filter(([, metrics]) => metrics.score < 0.5 && metrics.trend.direction === "declining")
      .map(([id]) => id);
  ...
  private updateTrends() {
    ...
    this.evaluationHistory.evaluations.forEach((e) => {
      Object.keys(e.evaluation.objectiveResults).forEach((id) => objectiveIds.add(id));
    });

    objectiveIds.forEach((objectiveId) => {
      this.evaluationHistory.trends[objectiveId] = this.calculateTrend(objectiveId);
    });
  }
  ...
  private getRecentEvaluations(objectiveId: string, count: number): TimestampedEvaluation[] {
  ...
  private generateEvidence(criterion: ObjectiveCriteria, score: number): string[] {
  ...
  private getLatestOverallScore(): number {
    const latest =
      this.evaluationHistory.evaluations[this.evaluationHistory.evaluations.length - 1];
  ...
  private calculateAchievementRate(threshold: number): number {
    ...
    const achievements = recentScores.filter((score) => score >= threshold).length;
