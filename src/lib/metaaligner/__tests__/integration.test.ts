...
const createMockAIService = (enhancedResponse?: string): AIService => {
  const mockService: Partial<AIService> = {
    createChatCompletion: vi.fn<() => Promise<AICompletion>>().mockResolvedValue({
      id: "test-response",
      created: Date.now(),
      model: "test-model",
      choices: [
...
describe("MetaAligner Integration Tests", () => {
  ...
  describe("Complete Multi-Objective Analysis Workflow", () => {
    it("should perform end-to-end evaluation workflow for crisis context", async () => {
      ...
      const originalResponse = "That sounds difficult. Have you considered talking to someone?";
    ...
    it("should handle educational context workflow", async () => {
      const userQuery = "What is cognitive behavioral therapy and how does it work?";
      const originalResponse = "CBT is a type of therapy that focuses on thoughts and behaviors.";
      ...
      const informativenessResult = evaluation.evaluation.objectiveResults["informativeness"];
      const correctnessResult = evaluation.evaluation.objectiveResults["correctness"];
    ...
    it("should handle support context with empathy prioritization", async () => {
      ...
      const originalResponse = "Work stress is common. Try to manage your time better.";
      ...
      if (empathyResult) {
        ...
        expect(empathyResult.criteriaScores).toHaveProperty("emotional_validation");
        expect(empathyResult.criteriaScores).toHaveProperty("understanding_demonstration");
  ...
  describe("Integrated AI Service Workflow", () => {
    it("should automatically evaluate and enhance responses", async () => {
      const baseAIService = createMockAIService("Original response that may need improvement");
      const integratedService = new IntegratedAIService(baseAIService, metaAligner);
    ...
    it("should handle conversation history in context detection", async () => {
      ...
      expect(response.alignment?.evaluation?.evaluationContext?.conversationHistory).toBeDefined();
      expect(
        response.alignment?.evaluation?.evaluationContext?.conversationHistory?.length,
      ).toBeGreaterThan(0);
    });
  });
  ...
  describe("Enhancement Pipeline Integration", () => {
    ...
    it("should handle enhancement failures gracefully", async () => {
      ...
      vi.mocked(failingAIService.createChatCompletion).mockRejectedValue(new Error("API Error"));
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    it("should validate objective consistency", async () => {
      ...
      for (const [objectiveId, result] of Object.entries(evaluation.evaluation.objectiveResults)) {
...
describe("AdaptiveSelector Workflow Integration Tests", () => {
  ...
  test("should process general input, detect context, and select objectives with default weights", async () => {
    ...
    expect(result1.contextDetectionResult.detectedContext).toBe(ContextType.GENERAL);
    ...
    expect(result2.contextDetectionResult.detectedContext).toBe(ContextType.GENERAL);
  ...
  test("should apply user preferences if WeightingStrategy is USER_PREFERENCE_ADJUSTED", async () => {
    ...
    DEFAULT_WEIGHT_ADJUSTMENT_PARAMS.strategy = WeightingStrategy.USER_PREFERENCE_ADJUSTED;
    ...
    const baseWeightsForGeneralContext = getContextualObjectiveWeights(ContextType.GENERAL);
    const expectedEmpathyRatio =
      (baseWeightsForGeneralContext.empathy * 2.0) / baseWeightsForGeneralContext.correctness;
