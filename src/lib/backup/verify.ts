...
export class BackupVerificationService extends EventEmitter {
  ...
  private async verifyBackup(filename: string): Promise<BackupVerificationResult> {
  ...
  private async verifyBackupContents(backupPath: string): Promise<void> {
    try {
      ...
      const backupData = JSON.parse(backupFileData.toString()) as Record<string, unknown>;
  ...
  private async verifyRestoredData(redis: RedisService, data: unknown): Promise<void> {
  ...
  private async getBackupMetadata(backupFile: string): Promise<Record<string, unknown> | null> {
    try {
  ...
  private async markBackupVerified(backupFile: string, metadata: BackupMetadata): Promise<void> {
  ...
  private async cleanupOldBackups(): Promise<void> {
    ...
    for (const backup of backups) {
      ...
      if (metadata && now - (metadata as { timestamp: number }).timestamp > retentionMs) {
