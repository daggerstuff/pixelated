...
export class S3StorageProvider implements StorageProvider {
  ...
  async storeFile(key: string, data: Uint8Array): Promise<void> {
    ...
    try {
      ...
      logger.error(`Failed to store file ${key} to AWS S3:`, error)
      throw new Error(
  ...
  async getFile(key: string): Promise<Uint8Array> {
    ...
    try {
      ...
      logger.error(`Failed to get file ${key} from AWS S3:`, error)
      throw new Error(
  ...
  async deleteFile(key: string): Promise<void> {
    ...
    try {
      ...
      logger.error(`Failed to delete file ${key} from AWS S3:`, error)
      throw new Error(
