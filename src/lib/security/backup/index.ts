  ...
  if (isBrowser) {
    return {
      encrypt: async (
        ...
        const encrypted = await (subtle.encrypt as any)({ name: "AES-GCM", iv }, importedKey, data);
    ...
    return {
      ...
      decrypt: async (
        ...
        const decipher: import("crypto").DecipherGCM = nodeCrypto.createDecipheriv(
          "aes-256-gcm",
          key,
          iv,
        );
...
export class BackupSecurityManager {
   ...
   */
  public static getInstance(config?: Partial<BackupConfig>): BackupSecurityManager {
  ...
  public async createBackup(type: BackupType): Promise<string> {
    try {
      ...
      logAuditEvent(AuditEventType.CREATE, "backup_create", "system", metadata.id, {
        type: metadata.type,
        size: metadata.size,
  ...
  private async encrypt(data: Uint8Array): Promise<EncryptedBackupData> {
    ...
    const { encryptedData, authTag } = await crypto.encrypt(data, this.encryptionKey, iv);
  ...
  private async decrypt(
    ...
    try {
      const crypto = await getCrypto();
      return await crypto.decrypt(encryptedData, this.encryptionKey, iv, authTag);
    } catch (error: unknown) {
  ...
  private async storeBackup(
    ...
    try {
      ...
      const provider = this.storageProviders.get(metadata.location as StorageLocation);
      if (!provider) {
        throw new Error(`Storage provider not found for location: ${metadata.location}`);
      }
      ...
      await provider.storeFile(metadataKey, new TextEncoder().encode(JSON.stringify(metadata)));

      logger.info(`Successfully stored backup ${metadata.id} in ${metadata.location}`);

      // Log storage as an audit event
      logAuditEvent(AuditEventType.SECURITY, "BACKUP_STORED", "system", metadata.id, {
        location: metadata.location,
        size: metadata.size,
        path: backupKey,
      });
  ...
  async verifyBackup(backupId: string): Promise<boolean> {
    ...
    try {
      ...
      logAuditEvent(AuditEventType.SECURITY, "backup_verify", "system", metadata.id, {
        isValid: true,
      ...
      logAuditEvent(AuditEventType.SECURITY, "backup_verify", "system", backupId, {
  ...
  private async getBackupMetadata(backupId: string): Promise<BackupMetadata | null> {
    ...
    for (const [location, provider] of storageEntries) {
      ...
      try {
        ...
        const files = await provider.listFiles(`backups/*/*/*/*/${backupId}.meta.json`);

        if (files && files.length > 0 && files[0]) {
          // Read the metadata file
          const metadataBuffer = await provider.getFile(files[0]);
          return JSON.parse(new TextDecoder().decode(metadataBuffer)) as BackupMetadata;
        }
      } catch (error: unknown) {
        logger.error(
  ...
  private async getDataForBackup(type: BackupType): Promise<Uint8Array> {
    let appDataJson =
      '{"timestamp":"' + new Date().toISOString() + '","type":"' + type + '","data":{';
  ...
  public async restoreBackup(backupId: string): Promise<boolean> {
    try {
      ...
      if (metadata.verificationStatus !== "verified") {
        throw new Error(`Cannot restore from unverified backup: ${metadata.verificationStatus}`);
      }
      ...
      // Log audit event
      logAuditEvent(AuditEventType.SECURITY, "backup_restore_completed", "system", backupId, {
      ...
      // Log audit event
      logAuditEvent(AuditEventType.SECURITY, "backup_restore_failed", "system", backupId, {
        error: error instanceof Error ? String(error) : String(error),
      });

      logger.error(`Restore failed: ${error instanceof Error ? String(error) : String(error)}`);
      throw error;
    }
  }
  ...
  getStorageProvider(location: StorageLocation): StorageProvider {
    ...
    if (!provider) {
      throw new Error(`No storage provider configured for location: ${location}`);
    }
  ...
  private async loadStorageProviders(): Promise<void> {
    ...
    for (const [location, locationConfig] of Object.entries(this.config.storageLocations)) {
