...
export class EdgeThreatDetectionSystem extends EventEmitter {
  ...
  constructor(
    private readonly _config: EdgeDetectionConfig,
    redis: Redis,
  ) {
    super();
    this.redis = redis;
  ...
  private async loadModel(config: ModelConfig): Promise<tf.LayersModel> {
    try {
      ...
      if (config.output_shape.length === 0) {
        throw new Error("output_shape must have at least one element");
      }
  ...
  async processDetectionRequest(request: {
    ...
    try {
      ...
      const finalResult = await this.generateFinalResult(request.input, modelResults, biasAnalysis);
    ...
    for (const [modelName, model] of this.models) {
      try {
        ...
        results[modelName] = {
          ...
          version:
            this.config.model_config[modelName as keyof typeof this.config.model_config].version,
  ...
  private async handleModelUpdate(update: { model_name: string; version: string }): Promise<void> {
    try {
      ...
      const modelConfig =
        this.config.model_config[update.model_name as keyof typeof this.config.model_config];
      if (modelConfig) {
