...
export function createMemoryTransport(): InternalMemoryServiceClientLike {
  const transportType = process.env["MEMORY_SERVICE_TRANSPORT"] ?? "http-loopback";

  switch (transportType) {
    case "mcp": {
      const launcherPath =
        process.env["FORESIGHT_MCP_LAUNCHER"] ?? "scripts/memory/foresight-mcp-server.sh";
    ...
    case "http-loopback": {
      ...
      const legacyClient = new InternalMemoryServiceClient(resolveInternalMemoryServiceConfig());
