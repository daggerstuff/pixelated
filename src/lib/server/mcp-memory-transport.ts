...
export class McpMemoryTransport implements McpMemoryTransportLike {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new InternalMemoryServiceError("MCP transport connection failed", 503, {
  ...
  private async callTool<T>(toolName: string, args: Record<string, unknown>): Promise<T> {
    ...
    if (!client || !tools) {
      throw new InternalMemoryServiceError("MCP transport failed to initialize", 500);
    }

    if (!tools[toolName]) {
      throw new InternalMemoryServiceError(`MCP tool ${toolName} not available`, 500);
    }
    ...
    try {
      ...
      if (content.length === 0) {
        throw new InternalMemoryServiceError("Empty response from MCP tool", 500);
      }

      const textContent = content[0];
      if (textContent?.type !== "text" || typeof textContent?.text !== "string") {
        throw new InternalMemoryServiceError("Unexpected MCP response format", 500);
      }
  ...
  async getMemory(input: {
    ...
    try {
      ...
      if (err instanceof InternalMemoryServiceError && err.message.includes("not found")) {
        return null;
      }
      throw err;
    }
