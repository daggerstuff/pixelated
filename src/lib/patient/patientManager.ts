...
class PatientManager {
  ...
  async searchPatients(
    ...
    if (criteria.treatmentStatus) {
      const statusMatches =
        this.searchIndex.get("treatmentStatus") || new Set();
      matchingPatients =
        matchingPatients.size === 0
          ? statusMatches
          : new Set(
              [...matchingPatients].filter((id) => statusMatches.has(id)),
            );
    }
  ...
  getPatientStatistics(): {
    ...
    patients.forEach((patient) => {
      byRiskLevel[patient.riskLevel] =
        (byRiskLevel[patient.riskLevel] || 0) + 1;
    });
