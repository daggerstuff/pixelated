...
import { hasPermission, PERMISSION_DEFINITIONS, ROLE_DEFINITIONS } from "../roles";
...
describe("researcher — training permissions", () => {
  ...
  it('is denied "read:own_training_sessions"', () => {
    expect(hasPermission("researcher", "read:own_training_sessions")).toBe(false);
  });
...
describe.each([
  ...
  "support",
  "guest",
] as UserRole[])("%s — no training permissions", (role) => {
  for (const perm of TRAINING_PERMISSIONS) {
    it(`is denied "${perm}"`, () => {
