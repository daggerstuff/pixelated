...
async function getAuth0UserInfo(accessToken: string): Promise<Auth0UserInfoResponse> {
  if (!auth0UserInfo) {
...
function decodeJwtPayloadSegment(segment: string): unknown {
  ...
  const paddedSegment =
    padding === 0 ? sanitizedSegment : `${sanitizedSegment}${"=".repeat(4 - padding)}`;
...
export async function validateToken(
  ...
  try {
    ...
    if (!auth0Authentication || !auth0UserInfo) {
      throw new AuthenticationError("Auth0 authentication clients not initialized");
    }
    ...
    const accountId =
      ...
      (userInfoData.user_metadata as Record<string, unknown> | undefined)?.accountId ??
      (userInfoData.app_metadata as Record<string, unknown> | undefined)?.accountId;
    const workspaceId =
      (userInfoData as Record<string, unknown>).workspaceId ??
      (userInfoData.user_metadata as Record<string, unknown> | undefined)?.workspaceId ??
      (userInfoData.app_metadata as Record<string, unknown> | undefined)?.workspaceId;
...
export async function refreshAccessToken(
  ...
  try {
    ...
    const accessTokenId = typeof userPayload.jti === "string" ? userPayload.jti : undefined;
    ...
    logSecurityEvent(SecurityEventType.TOKEN_REFRESHED, null, {
      ...
      newRefreshTokenId: tokenResponseData?.refresh_token ? "present" : "not_provided",
    });
    ...
    return {
      ...
      tokenType: "Bearer",
      expiresIn:
        typeof tokenResponseData?.expires_in === "number" ? tokenResponseData.expires_in : 3600,
