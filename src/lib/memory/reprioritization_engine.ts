...
import { evaluateReflectionOutcome, type EvaluationResult } from "./reflection/outcome-evaluator";
...
export function defaultConfig(overrides?: Partial<ReprioritizationConfig>): ReprioritizationConfig {
  return {
...
async function generateItemId(patternId: string, domain: UpstreamDomain): Promise<string> {
...
function generateTitle(point: EvidencePoint, accumulation: EvidenceAccumulation): string {
  const domainLabel = point.domain.charAt(0).toUpperCase() + point.domain.slice(1);
...
function generateDescription(point: EvidencePoint, accumulation: EvidenceAccumulation): string {
  return [
    `**Domain**: ${point.domain}`,
...
function generateValidationCriteria(
  ...
  const typeCriteria: Record<InterventionType, string[] | undefined> = {
    ...
    [InterventionType.PRIORITY_CHANGE]: ["Intervention validated against evidence"],
  };
  criteria.push(...(typeCriteria[interventionType] ?? ["Intervention validated against evidence"]));
  return criteria;
}
...
function generateChangeReason(
  ...
  const direction = newScore > existing.priorityScore ? "increased" : "decreased";
  const evidenceCount = accumulation.evidencePoints.length;
  return (
...
export class EvidenceAccumulator {
  ...
  private parseFeedbackReport(report: Record<string, unknown>): EvidencePoint[] {
  ...
  private addEvidenceToAccumulation(
    ...
    accumulation.isActionable = accumulation.totalWeight >= accumulation.actionThreshold;
  }
  ...
  private pruneAccumulationsIfNeeded(): void {
    ...
    const inactive = Array.from(this.accumulations.entries())
      ...
      .sort((a, b) =>
        (a[1].lastSeen || a[1].firstSeen).localeCompare(b[1].lastSeen || b[1].firstSeen),
      );
  ...
  summary(): Record<string, unknown> {
    const actionable = Array.from(this.accumulations.values()).filter((a) => a.isActionable);
...
export class PriorityCalculator {
  ...
  calculatePriority(
    ...
    const urgencyScore =
      severityWeight(severity) * (PriorityCalculator.DOMAIN_URGENCY[domain] ?? 1.0);
    ...
    const priorityScore =
      Math.round((evidenceComponent + urgencyComponent + coverageComponent) * 10_000) / 10_000;
  ...
  calculateInterventionType(
    ...
    if (domain === UpstreamDomain.ACQUISITION) return InterventionType.SOURCE_INTAKE;
    if (severity === EvidenceSeverity.CRITICAL && !(patternType in patternInterventionMap)) {
      return InterventionType.THRESHOLD_ADJUSTMENT;
    }
    return patternInterventionMap[patternType] ?? InterventionType.PRIORITY_CHANGE;
  }
...
export class ReprioritizationEngine {
  ...
  ingestReflectionContext(
    ...
    const evidence: EvidencePoint = {
      ...
      rootCauseHypothesis: context.cognitivePatterns.join("; ") || evaluation.rationale,
  ...
  async runReprioritization(): Promise<ReprioritizationReport> {
    ...
    for (const accumulation of actionable) {
      ...
      const latestPoint = accumulation.evidencePoints[accumulation.evidencePoints.length - 1]!;
      ...
      const itemId = await generateItemId(accumulation.patternId, latestPoint.domain);
      const title = generateTitle(latestPoint, accumulation);
      const description = generateDescription(latestPoint, accumulation);
      const validationCriteria = generateValidationCriteria(latestPoint, interventionType);
    ...
    const sortByScore = (a: BacklogItem, b: BacklogItem) => b.priorityScore - a.priorityScore;
    newItems.sort(sortByScore);
    reprioritized.sort(sortByScore);
    unchanged.sort(sortByScore);

    const byDomain = this.buildDomainSummary(newItems, reprioritized, unchanged, allAccumulations);
  ...
  private shouldReprioritize(
    ...
    const scoreChange = Math.abs(newScore - existing.priorityScore) / existing.priorityScore;
  ...
  getBacklog(): BacklogItem[] {
    return Array.from(this.backlog.values()).sort((a, b) => b.priorityScore - a.priorityScore);
  }
...
export function createEngine(config?: ReprioritizationConfig): ReprioritizationEngine {
