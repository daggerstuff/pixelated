...
const BEHAVIORAL_NUDGES: Record<string, string> = {
  ...
  despair:
    'assess for crisis indicators; validate the weight without dismissing',
  ...
  relief:
    'acknowledge relief; explore what shifted without assuming permanence',
  pride: 'recognize earned pride; connect effort to outcome',
  confusion:
    'normalize confusion as part of integration; do not resolve prematurely',
  loneliness:
    'validate loneliness; explore connection patterns without prescribing',
...
const VALIDATION_PATTERNS: Record<string, string> = {
  ...
  anger:
    'Anger often protects more vulnerable emotions beneath; validate the protective layer',
  despair:
  ...
  shame:
    'Shame thrives in isolation; validate through externalizing and contextualizing',
  relief:
    'Relief indicates shifting internal conditions; validate without assuming permanence',
  pride:
    'Pride in small steps builds self-efficacy; validate the process not just outcome',
  confusion:
    'Confusion often precedes integration; validate the discomfort of not-knowing',
  loneliness:
    'Loneliness signals unmet connection needs; validate without pathologizing',
  acceptance:
    'Acceptance is not passivity; validate the active process of reckoning',
  ...
  curiosity:
    'Curiosity is self-directed healing; validate the inner drive toward growth',
}

// ─── Default fallbacks ────────────────────────────────────────────────

const DEFAULT_NUDGE =
  'respond with heightened emotional attunement; let the conversation breathe'
const DEFAULT_VALIDATION =
  'validate the emotional experience without assuming its cause'
 ...
 */
function deriveBehavioralNudge(
  categories: string[],
 ...
 */
function deriveRelationalPattern(
...
export class LatentSurfacer {
  ...
  decay(reverie: ReverieVector, messagesSinceTrigger: number): ReverieVector {
    ...
    const decayFactor = Math.pow(
      0.5,
      messagesSinceTrigger / reverie.decayHalfLife,
    )
  ...
  isFaded(reverie: ReverieVector): boolean {
    return (
      reverie.phase === 'fading' &&
      reverie.resonanceScore < this.config.fadingThreshold
    )
  }
}
