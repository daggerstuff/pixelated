...
import { ReverieVector, ReveriePhase, ReverieConfig } from "../../../types/reverie";
...
export class SoftInjector {
  ...
  apply(newReveries: ReverieVector[], messageCount: number): InjectionResult {
    ...
    const decayed = this.activeReveries.map((r) => this.applyDecay(r, messageCount));
    ...
    if (this.activeReveries.length > this.config.maxActiveReveries) {
      ...
      this.activeReveries = [resolution.winner, ...resolution.suppressed]
        ...
        .filter((r) => this.currentInfluence(r, messageCount) > this.config.fadingThreshold);
    }
    ...
    for (const nr of newReveries) {
      ...
      if (prev) {
        ...
        const blended: ReverieVector = {
          ...
          phase: this.phaseFromResonance(0.4 * prev.resonanceScore + 0.6 * nr.resonanceScore),
        };
   ...
   */
  private applyDecay(reverie: ReverieVector, messageCount: number): ReverieVector {
   ...
   */
  private assemblePrompt(reveries: ReverieVector[], messageCount: number): string {
    ...
    for (const reverie of reveries) {
      ...
      const intensity = influence > 0.5 ? "strong" : influence > 0.25 ? "moderate" : "subtle";
