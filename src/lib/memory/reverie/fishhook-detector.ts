...
export class FishhookDetector {
  ...
  detect(
    ...
    for (const latent of latentPool) {
      ...
      const patternScore = categoryOverlap(
        currentEmotions.categories,
      ...
      const surpriseScore = bayesianSurprise(
        currentEmotions,
        latent.emotions,
        expectedVariance,
      )
   ...
   */
  private computeCompositeResonance(
    scores: { type: FishhookMatchType; score: number }[],
  ): number {
    let weighted = 0
    let totalWeight = 0
