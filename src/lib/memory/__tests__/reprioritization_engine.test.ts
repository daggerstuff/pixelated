...
describe('ReprioritizationEngine TS', () => {
  ...
  describe('createEngine factory', () => {
    ...
    it('creates engine with custom config', () => {
      const customCfg: ReprioritizationConfig = { actionThreshold: 0.9, churnPreventionWindowDays: 14, evidenceDecayRate: 0.1, maxTrackedPatterns: 500, maxEvidenceAgeDays: 60, urgentThreshold: 4.0, highThreshold: 3.0, mediumThreshold: 2.0, lowThreshold: 1.0, reprioritizeScoreDeltaRatio: 0.3 };
  ...
  describe('EvidenceAccumulator', () => {
    ...
    it('non-actionable evidence below threshold', () => {
      const acc = new EvidenceAccumulator(defaultConfig({ actionThreshold: 10.0 }));
      const point: EvidencePoint = {
        patternId: 'p2',
    ...
    it('getActionablePatterns returns only actionable accumulations', () => {
      ...
      expect(actionable.some(a => a.patternId === 'not_actionable1')).toBe(false);
    });
  ...
  describe('ReprioritizationEngine', () => {
    function makeFeedbackReport(patternId: string, severity: string, frequency: number) {
      return {
        failure_patterns: [
          {
    ...
    it('runReprioritization creates backlog item for actionable pattern', async () => {
      const engine = new ReprioritizationEngine();
      engine.loadFeedbackDict(makeFeedbackReport('critical_gap', 'critical', 1.0));
    ...
    it('CRITICAL severity produces higher score than MEDIUM', async () => {
      ...
      criticalEngine.loadFeedbackDict(makeFeedbackReport('crit1', 'critical', 1.0));
    ...
    it('getBacklog returns items sorted by priority score', async () => {
      ...
      for (let i = 1; i < backlog.length; i++) {
        expect(backlog[i - 1]!.priorityScore).toBeGreaterThanOrEqual(backlog[i]!.priorityScore);
      }
    });

    it('getBacklogByDomain filters correctly', async () => {
      const engine = new ReprioritizationEngine();
      engine.loadFeedbackDict(makeFeedbackReport('privacy_issue', 'critical', 1.0));
      await engine.runReprioritization();
      const privacyItems = engine.getBacklogByDomain(UpstreamDomain.PRIVACY);
      expect(privacyItems.every(i => i.domain === UpstreamDomain.PRIVACY)).toBe(true);
    });
