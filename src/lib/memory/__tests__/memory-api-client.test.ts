...
function createMockClient(mockFetch: MockFetch) {
  return new MemoryApiClient({ baseUrl: 'http://test:8000', fetchFn: mockFetch as unknown as typeof fetch })
}
...
describe('health()', () => {
  it('returns health status', async () => {
    const mockFetch = jest.fn().mockResolvedValue(
      mockJsonResponse({ status: 'ok', memory_count: 42, scorer_latency_ms: 0.01, classifier_latency_ms: 0.02 }),
    )
...
describe('create()', () => {
  it('creates a memory block', async () => {
    const mockBlock = {
      id: 'mem_123', tenantId: 't1', sessionId: 's1',
      content: 'Test memory', timestamp: Date.now(),
      importance: { raw: 0.5, recency: 0.9, relevance: 0.5, emotionalWeight: 1, actionability: 0.5 },
      emotions: { valence: 0.5, arousal: 0.5, categories: ['joy'] },
      gating: { piiStatus: 'absent', crisisFlag: false, traumaIndicators: [], consentGate: 'open' },
      consolidation: { phase: 'raw', lastProcessed: Date.now(), remCycles: 3, schemaReferences: [] },
    }
    const mockFetch = jest.fn().mockResolvedValue(mockJsonResponse(mockBlock, 201))
    const client = createMockClient(mockFetch)

    const result = await client.create({ tenantId: 't1', sessionId: 's1', content: 'Test memory' })
...
describe('search()', () => {
  it('builds correct query params', async () => {
    ...
    await client.search({ tenantId: 't1', minImportance: 0.5, crisisOnly: true, limit: 10, offset: 5 })
...
describe('get()', () => {
  it('fetches a single memory by id', async () => {
    const mockBlock = { id: 'mem_123', tenantId: 't1', sessionId: 's1', content: 'Test', timestamp: 0,
      importance: { raw: 0.5, recency: 0, relevance: 0, emotionalWeight: 1, actionability: 0.5 },
      emotions: { valence: 0.5, arousal: 0.5, categories: [] },
      gating: { piiStatus: 'absent', crisisFlag: false, traumaIndicators: [], consentGate: 'open' },
      consolidation: { phase: 'raw', lastProcessed: 0, remCycles: 0, schemaReferences: [] },
    }
...
describe('delete()', () => {
  ...
  it('throws MemoryApiError on 404', async () => {
    const mockFetch = jest.fn().mockResolvedValue(mockError(404, { detail: 'Not found' }))
    const client = createMockClient(mockFetch)

    await expect(client.delete('nonexistent', 't1')).rejects.toThrow(MemoryApiError)
  })
})
...
describe('score()', () => {
  it('returns score response', async () => {
    const mockScore = {
      id: 'mem_123',
      importance: { raw: 0.72, recency: 0.85, relevance: 0.91, emotionalWeight: 2.0, actionability: 0.5 },
      components: { recency: 0.85, relevance: 0.91, emotionalWeight: 2.0, actionability: 0.5 },
    }
