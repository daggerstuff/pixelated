...
describe("create()", () => {
  it("posts to /api/v1/memory and returns the v1 envelope", async () => {
    const mockFetch = vi.fn().mockResolvedValue(mockJsonResponse({ data: sampleMemory }, 201));
...
describe("update()", () => {
  it("patches /api/v1/memory/:id", async () => {
    const mockFetch = vi.fn().mockResolvedValue(mockJsonResponse({ data: sampleMemory }));
    const client = createMockClient(mockFetch);
...
describe("delete()", () => {
  ...
  it("throws MemoryApiClientError on 404", async () => {
    const mockFetch = vi
      .fn()
      .mockResolvedValue(mockError(404, { error: "not_found", message: "Not found" }));
    const client = createMockClient(mockFetch);

    await expect(client.delete(sampleMemory.id)).rejects.toThrow(MemoryApiClientError);
  });
});
...
describe("health()", () => {
  it("gets /api/ingestion/gate/health", async () => {
    const mockFetch = vi.fn().mockResolvedValue(mockJsonResponse({ status: "healthy" }));
    const client = new IngestionGateClient({
      fetchFn: mockFetch as unknown as typeof fetch,
    });
