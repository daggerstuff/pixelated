...
export class SemanticDeduplicator {
  ...
  deduplicate(memories: MemoryBlock[]): DedupResult {
    ...
    const reductionPct = ((memories.length - unique.length) / memories.length) * 100;
  ...
  private mergeCluster(cluster: DedupCluster): MemoryBlock {
    ...
    const maxArousal = Math.max(...cluster.members.map((m) => m.emotions.arousal));
    const maxEmotional = Math.max(...cluster.members.map((m) => m.importance.emotionalWeight));
    const allIndicators = [...new Set(cluster.members.flatMap((m) => m.gating.traumaIndicators))];
    const maxRemCycles = Math.max(...cluster.members.map((m) => m.consolidation.remCycles));
