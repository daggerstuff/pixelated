...
export interface ConsolidationReport {
  /** IDs of memories merged into duplicates (survivors kept) */
  mergedIds: string[]
  /** IDs of memories archived during forgetting phase */
  archivedIds: string[]
  /** IDs of memories deleted during forgetting phase */
  deletedIds: string[]
  /** Cross-links discovered during REM dreaming */
  crossLinks: CrossLink[]
  /** Schema patterns extracted during REM dreaming */
...
export class ConsolidationPipeline {
  ...
  private extractSchemas(memories: UnifiedMemory[]): SchemaPattern[] {
    ...
    for (const [category, group] of categoryGroups) {
      ...
      const avgCatImportance =
        group.reduce((s, m) => s + m.importance, 0) / group.length
      schemas.push({
  ...
  private evaluateForgetting(
    ...
    nowMs: number,
  ): { archiveIds: string[]; deleteIds: string[]; deleteCount: number } {
