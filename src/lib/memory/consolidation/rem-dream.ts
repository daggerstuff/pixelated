...
export class RemDreamScheduler {
  ...
  private crosslinkThreshold: number;
    ...
    for (let i = 0; i < memories.length; i++) {
      for (let j = i + 1; j < memories.length; j++) {
        ...
        if (sim >= this.crosslinkThreshold) {
          const sameEmotion =
            memories[i].emotions.categories.some((c) =>
  ...
  private static defaultSummarizer(memories: MemoryBlock[]): string {
    ...
    const top = [...memories]
      .sort((a, b) => b.importance.raw - a.importance.raw)
      .slice(0, 3);
    const topics = new Set(top.flatMap((m) => m.emotions.categories.length > 0 ? m.emotions.categories : ['general']));
