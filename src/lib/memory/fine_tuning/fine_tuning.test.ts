...
describe('DatasetPreparator', () => {
  test('prepares dataset with splits', () => {
    ...
    const memories = [
      makeMemory({ emotions: { valence: -0.5, arousal: 0.5, categories: ['anxiety'] } }),
      makeMemory({ emotions: { valence: 0.5, arousal: 0.5, categories: ['joy'] } }),
      makeMemory({ emotions: { valence: 0.0, arousal: 0.5, categories: ['neutral'] } }),
    ];
    const [split, stats] = preparator.prepare(memories);
    expect(stats.totalExamples).toBeGreaterThanOrEqual(3);
    expect(split.train.length + split.val.length + split.test.length).toBe(stats.totalExamples);
  ...
  test('balances valence distribution', () => {
    ...
    const memories = [
      makeMemory({ emotions: { valence: -0.5, arousal: 0.5, categories: ['anxiety'] } }),
      makeMemory({ emotions: { valence: -0.6, arousal: 0.5, categories: ['fear'] } }),
      makeMemory({ emotions: { valence: 0.5, arousal: 0.5, categories: ['joy'] } }),
    ];
    const [split, stats] = preparator.prepare(memories);
    expect(stats.totalExamples).toBeGreaterThanOrEqual(3);
...
describe('MemorySystemEvaluator', () => {
  test('evaluates retrieval quality', () => {
    ...
    const memories = [
      makeMemory({ content: 'anxiety about work stress', emotions: { valence: -0.5, arousal: 0.7, categories: ['anxiety'] } }),
      makeMemory({ content: 'happy day at the park', emotions: { valence: 0.8, arousal: 0.4, categories: ['joy'] } }),
      makeMemory({ content: 'anxiety about health', emotions: { valence: -0.3, arousal: 0.6, categories: ['anxiety'] } }),
    ];
  ...
  test('evaluates safety metrics', () => {
    ...
    const memories = [
      makeMemory({ gating: { crisisFlag: true, piiStatus: 'absent', traumaIndicators: [], consentGate: 'open' } }),
      makeMemory({ gating: { crisisFlag: false, piiStatus: 'absent', traumaIndicators: [], consentGate: 'open' } }),
      makeMemory({ gating: { crisisFlag: false, piiStatus: 'absent', traumaIndicators: [], consentGate: 'open' } }),
    ];
  ...
  test('full evaluation report', () => {
    ...
    const memories = Array.from({ length: 30 }, (_, i) =>
      makeMemory({
        ...
        emotions: { valence: -0.5 + i * 0.04, arousal: 0.5, categories: ['anxiety', 'joy', 'fear', 'hope', 'sadness'][i % 5] },
        gating: { crisisFlag: i % 15 === 0, piiStatus: 'absent', traumaIndicators: [], consentGate: 'open' },
      })
    );
