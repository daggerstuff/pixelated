...
export class DatasetPreparator {
  ...
  private extractExamples(memories: MemoryBlock[]): TrainingExample[] {
    ...
    for (const [sessionId, sessionMemories] of Object.entries(sessionGroups)) {
      for (let i = 0; i < sessionMemories.length; i++) {
        ...
        const priorContext = sessionMemories.slice(0, i).map((m) => m.content).join('\n');
        examples.push({
  ...
  private balanceValence(examples: TrainingExample[]): TrainingExample[] {
    const buckets: Record<string, TrainingExample[]> = { negative: [], neutral: [], positive: [] };
    ...
    for (const bucket of Object.values(buckets)) {
      ...
      if (bucket.length >= maxSize) {
        ...
        const repeats = Array.from({ length: Math.ceil(maxSize / bucket.length) }, () => bucket).flat();
  ...
  private computeStats(split: DatasetSplit, piiLeak: boolean): DatasetStats {
    ...
    return {
      ...
      avgValence: valences.length > 0 ? Math.round((valences.reduce((s, v) => s + v, 0) / valences.length) * 1000) / 1000 : 0,
      crisisRatio: all.length > 0 ? Math.round((crisisCount / all.length) * 1000) / 1000 : 0,
      piiLeakDetected: piiLeak,
    };
  }

  private static generateResponseTemplate(memory: MemoryBlock): string {
    const emotionLabels = memory.emotions.categories.length > 0 ? memory.emotions.categories.join(', ') : 'general';
    const valenceDesc =
      memory.emotions.valence < -0.2 ? 'negative emotional state'
        : memory.emotions.valence > 0.2 ? 'positive emotional state'
