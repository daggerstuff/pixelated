...
export class MemorySystemEvaluator {
  ...
  evaluateRetrieval(memories: MemoryBlock[], testQueries?: string[]): RetrievalResult {
    if (memories.length === 0) return { precisionAtK: 0, recallAtK: 0, mrr: 0, k: this.k };

    const queries = testQueries ?? memories.slice(0, 20).map((m) => m.content.slice(0, 50));
    ...
    for (const query of queries) {
      ...
      const firstRelevantRank = retrieved.findIndex((m) => relevantIds.has(m.id));
      rrScores.push(1 / (firstRelevantRank >= 0 ? firstRelevantRank + 1 : retrieved.length + 1));
    }

    return {
      precisionAtK: precisions.length > 0 ? Math.round((precisions.reduce((s, v) => s + v, 0) / precisions.length) * 1000) / 1000 : 0,
      recallAtK: recalls.length > 0 ? Math.round((recalls.reduce((s, v) => s + v, 0) / recalls.length) * 1000) / 1000 : 0,
      mrr: rrScores.length > 0 ? Math.round((rrScores.reduce((s, v) => s + v, 0) / rrScores.length) * 1000) / 1000 : 0,
      k: this.k,
    };
  }

  evaluateResponse(memories: MemoryBlock[]): ResponseResult {
    if (memories.length === 0) return { appropriatenessScore: 0, personalizationScore: 0, continuityScore: 0 };
    ...
    return {
      appropriatenessScore: Math.round((scores.reduce((s, v) => s + v[0], 0) / scores.length) * 1000) / 1000,
      personalizationScore: Math.round((scores.reduce((s, v) => s + v[1], 0) / scores.length) * 1000) / 1000,
      continuityScore: Math.round((scores.reduce((s, v) => s + v[2], 0) / scores.length) * 1000) / 1000,
    };
  }
  ...
  evaluateSafety(memories: MemoryBlock[]): SafetyResult {
    ...
    const crisisDetected = crisisMemories.filter((m) => m.gating.crisisFlag).length;
    const crisisSensitivity = crisisMemories.length > 0 ? crisisDetected / crisisMemories.length : 1;

    const falsePositives = nonCrisis.filter((m) => m.gating.crisisFlag).length;
    const crisisSpecificity = nonCrisis.length > 0 ? 1 - falsePositives / nonCrisis.length : 1;
  ...
  evaluatePerformance(memories: MemoryBlock[]): PerformanceResult {
    if (memories.length === 0) return { p50LatencyMs: 0, p95LatencyMs: 0, p99LatencyMs: 0, throughputPerSec: 0, peakMemoryMb: 0 };
    ...
    return {
      ...
      throughputPerSec: Math.round((n / (latencies.reduce((s, v) => s + v, 0) / 1000)) * 10) / 10,
      peakMemoryMb: 0,
    };
  }
  ...
  private retrieve(memories: MemoryBlock[], query: string, k: number): MemoryBlock[] {
    const queryTerms = new Set(query.toLowerCase().split(' '));
  ...
  private static checkPiiLeak(memories: MemoryBlock[]): number {
    ...
    const leakCount = memories.filter((m) => patterns.some((p) => p.test(m.content))).length;
