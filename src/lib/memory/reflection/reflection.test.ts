...
function makeMemory(overrides: Partial<MemoryBlock> = {}): MemoryBlock {
  return {
    ...
    importance: overrides.importance ?? {
      raw: 0.5, recency: 0.5, relevance: 0.5, emotionalWeight: 1.0, actionability: 0.3,
    },
    emotions: overrides.emotions ?? {
      valence: 0.0, arousal: 0.5, categories: [],
    },
...
describe('SessionConsolidator', () => {
  test('consolidates improving session', () => {
    ...
    const memories = [
      makeMemory({ sessionId: 's1', emotions: { valence: -0.5, arousal: 0.5, categories: ['anxiety'] }, timestamp: 1000 }),
      makeMemory({ sessionId: 's1', emotions: { valence: -0.1, arousal: 0.5, categories: ['neutral'] }, timestamp: 2000 }),
      makeMemory({ sessionId: 's1', emotions: { valence: 0.5, arousal: 0.5, categories: ['hope'] }, timestamp: 3000 }),
    ];
  ...
  test('identifies unresolved crisis topics', () => {
    const consolidator = new SessionConsolidator();
    const memories = [
      makeMemory({ sessionId: 's1', gating: { crisisFlag: true, piiStatus: 'absent', traumaIndicators: [], consentGate: 'open' } }),
    ];
    const summary = consolidator.consolidate(memories);
    expect(summary.unresolvedTopics).toContain('crisis_content_requires_followup');
  });
});
...
describe('PatternDetector', () => {
  test('detects recurring themes', () => {
    ...
    const sessions: SessionSummary[] = [
      { sessionId: 's1', tenantId: 't1', themes: ['anxiety'], emotionalArc: { startValence: 0, endValence: 0, minValence: 0, maxValence: 0, avgValence: 0, trend: 'stable', volatility: 0 }, unresolvedTopics: [], summaryText: '', memoryCount: 1, timestampMs: 1000 },
      { sessionId: 's2', tenantId: 't1', themes: ['anxiety'], emotionalArc: { startValence: 0, endValence: 0, minValence: 0, maxValence: 0, avgValence: 0, trend: 'stable', volatility: 0 }, unresolvedTopics: [], summaryText: '', memoryCount: 1, timestampMs: 2000 },
    ];
...
describe('DreamReflectionIntegrator', () => {
  test('integrates dream and session insights', () => {
    ...
    const dreamResult = {
      schemas: [{ schemaId: 's1', title: 'test', generalization: 'test pattern', sourceMemoryIds: ['m1'], confidence: 0.7 }],
      crossLinks: [],
    };
    const summary: SessionSummary = {
      sessionId: 's1', tenantId: 't1', themes: ['anxiety'],
...
describe('ActionPipeline', () => {
  test('generates crisis notifications', () => {
    ...
    const summary: SessionSummary = {
      sessionId: 's1', tenantId: 't1', themes: ['crisis'],
      emotionalArc: { startValence: -0.5, endValence: -0.8, minValence: -0.8, maxValence: -0.5, avgValence: -0.65, trend: 'declining', volatility: 0.2 },
      unresolvedTopics: ['crisis_content_requires_followup'], summaryText: 'crisis', memoryCount: 2, timestampMs: Date.now(),
    };
    const result = pipeline.execute(summary);
    expect(result.notifications.some(n => n.severity === 'critical')).toBe(true);
  });
  ...
  test('generates recommendations for unresolved topics', () => {
    ...
    const summary: SessionSummary = {
      sessionId: 's1', tenantId: 't1', themes: ['anxiety'],
      emotionalArc: { startValence: 0, endValence: 0, minValence: 0, maxValence: 0, avgValence: 0, trend: 'stable', volatility: 0 },
      unresolvedTopics: ['high_arousal:anxiety'], summaryText: 'test', memoryCount: 2, timestampMs: Date.now(),
    };
    const result = pipeline.execute(summary);
