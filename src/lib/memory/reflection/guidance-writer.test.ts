...
function makeInsight(
  overrides: Partial<
    Pick<
      ReflectionInsight,
      "summary" | "insightType" | "confidence" | "recommendedAction" | "evidenceIds" | "metadata"
    >
  > = {},
): Pick<ReflectionInsight, "summary" | "insightType" | "confidence" | "recommendedAction"> {
  return {
...
describe("proposeGuidanceUpdate", () => {
  ...
  describe("formatted guidance text", () => {
    test("includes ISO date prefix, insight type, and confidence percentage", () => {
      ...
      expect(result.guidanceText).toContain("Prioritize empathetic tone over clinical jargon.");
    });
    ...
    test("appends recommendedAction when present", () => {
      ...
      expect(result.guidanceText).toContain("Add a post-session cue review step");
    });
  ...
  describe("NoopGuidanceWriter", () => {
    test("writes succeed silently (no throw)", async () => {
      ...
      await expect(writer.writeGuidanceToBlock("guidance", "test")).resolves.toBeUndefined();
    });
  });
