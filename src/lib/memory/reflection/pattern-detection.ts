...
export class PatternDetector {
  ...
  analyze(sessions: SessionSummary[], rawMemories?: MemoryBlock[]): PatternReport {
  ...
  private detectRecurringThemes(sessions: SessionSummary[]): RecurringTheme[] {
    ...
    for (const [theme, sessionList] of Object.entries(themeSessions)) {
      ...
      if (valences.length >= 2) {
        ...
        const secondHalf = valences.slice(mid).reduce((s, v) => s + v, 0) / (valences.length - mid);
      ...
      results.push({
        ...
        avgValence: Math.round((valences.reduce((s, v) => s + v, 0) / valences.length) * 1000) / 1000,
        trend,
      });
    }
        ...
        if (confidence >= this.minConfidence) {
          patterns.push({
            ...
            exampleSessions: sessions
              .filter((s) => s.themes.includes(theme) && crisisSessions.has(s.sessionId))
              .slice(0, 3)
              .map((s) => s.sessionId),
          });
        }
  ...
  private analyzeInterventions(sessions: SessionSummary[]): InterventionResult[] {
    if (sessions.length < 2) return [];
    ...
    const earlyValence = early.reduce((s, session) => s + session.emotionalArc.avgValence, 0) / early.length;
    const lateValence = late.reduce((s, session) => s + session.emotionalArc.avgValence, 0) / late.length;
  ...
  private linearSlope(values: number[]): number {
    ...
    const denominator = Array.from({ length: n }, (_, i) => (i - xMean) ** 2).reduce(
      (s, v) => s + v,
      0,
    );
