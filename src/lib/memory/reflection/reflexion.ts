...
export class ReflexionEngine {
  ...
  private static parseReflection(
    ...
    for (const line of lines) {
      ...
      if (lower.includes('went well') || lower.includes('success') || lower.includes('positive')) {
        section = 'well';
        continue;
      }
      if (lower.includes('went wrong') || lower.includes('fail') || lower.includes('negative')) {
        section = 'wrong';
        continue;
      }
      if (lower.includes('change') || lower.includes('differently') || lower.includes('improve')) {
        section = 'change';
        continue;
      }
    ...
    const failures = pairs.filter((p) => p.feedbackType === FeedbackType.FAILURE).length;
    const confidence = pairs.length > 0 ? 1 - failures / pairs.length : 0.5;

    return {
      reflectionId: `reflexion_${Date.now()}`,
      whatWentWell: wentWell.length > 0 ? wentWell : ['Trajectory completed without major issues'],
  ...
  private static defaultGenerator(pairs: ActionFeedbackPair[]): string {
    const successes = pairs.filter((p) => p.feedbackType === FeedbackType.SUCCESS);
    const failures = pairs.filter((p) => p.feedbackType === FeedbackType.FAILURE);
    const partials = pairs.filter((p) => p.feedbackType === FeedbackType.PARTIAL);
    ...
    for (const p of partials) lines.push(`- Refine: ${p.action} (was ${p.feedback})`);
