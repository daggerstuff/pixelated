...
export class SessionConsolidator {
  ...
  private computeEmotionalArc(memories: MemoryBlock[]): EmotionalArc {
    ...
    const volatility = Math.sqrt(valences.reduce((s, v) => s + (v - avg) ** 2, 0) / valences.length);
  ...
  private identifyUnresolved(memories: MemoryBlock[]): string[] {
    ...
    if (crisisMemories.length > 0) unresolved.push('crisis_content_requires_followup');

    if (highArousal.length > 0) {
      const topics = new Set(highArousal.flatMap((m) => m.emotions.categories.length > 0 ? m.emotions.categories : ['high_arousal']));
      for (const t of topics) unresolved.push(`high_arousal:${t}`);
    }

    if (negativeEnd.length > 0) {
      const topics = new Set(negativeEnd.flatMap((m) => m.emotions.categories.length > 0 ? m.emotions.categories : ['negative_ending']));
