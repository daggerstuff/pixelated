...
export function jsonError(mapping: { status: number; code: string; message: string }): Response {
...
export function parseJson<T>(schema: z.ZodType<T>, raw: unknown): ValidationResult<T> {
...
export function parseSearchParams<T>(schema: z.ZodType<T>, url: URL): ValidationResult<T> {
...
export function toPublicMemory(record: ProductMemoryRecord): z.infer<typeof PublicMemory> {
