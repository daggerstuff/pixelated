...
export class ConsentGate {
  ...
  checkConsent(userId: string, memoryId?: string): ConsentGateResult {
    ...
    if (!record) {
      return this.buildResult(userId, auditMemoryId, false, 'blocked', 'No consent record found', false);
    }

    if (record.revoked) {
      return this.buildResult(userId, auditMemoryId, false, record.consentType, 'Consent has been revoked', false);
    }
    ...
    if (expired) {
      ...
      const result = this.buildResult(userId, auditMemoryId, false, record.consentType, 'Consent has expired', true);
      result.auditEntry.details = `${result.auditEntry.details}; expiration_audit=${expireEntry.timestamp}`;
      return result;
    }

    if (record.consentType === 'open') {
      return this.buildResult(userId, auditMemoryId, true, record.consentType, 'Consent granted', false);
    }

    if (record.consentType === 'restricted') {
      return this.buildResult(userId, auditMemoryId, true, record.consentType, 'Consent granted with restrictions', false);
    }

    return this.buildResult(userId, auditMemoryId, false, 'blocked', 'Consent tier is blocked', false);
  }
  ...
  evaluate(userId: string, memoryId?: string): ConsentGateEvaluation {
    ...
    return {
      gate: GATE_NAME,
      decision: 'pass',
      reason: result.consentTier === 'restricted' ? 'Consent granted with restrictions' : 'Consent granted',
      confidence: 1.0,
    };
  }
  ...
  private recordAudit(entry: Omit<ConsentAuditEntry, 'timestamp'>): ConsentAuditEntry {
