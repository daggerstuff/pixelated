...
const TEMPORAL_PATTERN = /\b(used to|previously|in the past|used to be|was feeling|had been)\b/i;
...
export class CrisisDetector {
  ...
  private hasNegatedEvidence(content: string, matches: KeywordMatch[]): boolean {
  ...
  private hasTemporalContext(content: string, matches: KeywordMatch[]): boolean {
  ...
  private recommendations(
    ...
    if (tier === 'critical') {
      recommendations.add('Block deletion and initiate immediate crisis workflow');
    } else if (tier === 'high') {
    ...
    if (historical) {
      recommendations.add('Crisis language appears historical; reduced confidence');
    }
