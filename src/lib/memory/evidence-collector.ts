...
import { EvidencePoint, EvidenceSeverity, UpstreamDomain } from "./reprioritization_engine";
...
export class EvidenceCollector {
  ...
  async collectClinicalAlerts(): Promise<EvidencePoint[]> {
    try {
      ...
      cutoffDate.setDate(cutoffDate.getDate() - this.config.collectionWindowDays);
  ...
  async collectInteractionFrequency(): Promise<EvidencePoint[]> {
    logger.warn("Interaction frequency collection not implemented - schema needs to be confirmed", {
  ...
  async collectFailureRates(): Promise<EvidencePoint[]> {
    logger.warn("Failure rates collection not implemented - schema needs to be confirmed", {
      collectionWindowDays: this.config.collectionWindowDays,
    });
    // Return empty array as per requirements when data source schema is unclear
    return [];
  ...
  async collectAll(): Promise<EvidencePoint[]> {
    ...
    try {
      ...
      const [clinicalAlerts, interactionFrequency, failureRates] = await Promise.all([
      ...
      const allEvidence = [...clinicalAlerts, ...interactionFrequency, ...failureRates];
