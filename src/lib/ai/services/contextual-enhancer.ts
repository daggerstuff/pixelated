...
export class ContextualEnhancer {
  ...
  async enhanceContext(
    ...
    try {
      ...
      const interventionTiming = this.calculateInterventionTiming(context, progress);
      ...
      throw new Error(`Failed to enhance context for session ${sessionId}: ${message}`);
    }
  }
  ...
  private async buildContext(
    ...
    return {
      ...
      crisisIndicators: this.identifyCrisisIndicators(currentState, conversationHistory),
  ...
  private calculateInterventionTiming(
    ...
    const crisisLevel = this.assessCrisisLevel(context.currentState, context.crisisIndicators);
   ...
   */
  private calculateConfidence(context: SessionContext, progress: TherapeuticProgress): number {
    const factors = [
  ...
  private createBaselineProgress(patientProfile: PatientPsiProfile): TherapeuticProgress {
    return {
  ...
  private calculateAllianceProgress(history: SessionContext[]): number {
    return Math.min(1.0, 0.5 + history.filter((h) => h.therapeuticAlliance > 0.7).length * 0.1);
  }
  ...
  private calculateCrisisFrequency(history: SessionContext[]): number {
    const crisisSessions = history.filter((h) => h.crisisIndicators.length > 0).length;
    return Math.max(0, crisisSessions / Math.max(history.length, 1));
  }

  private calculateGoalProgress(history: SessionContext[], goals: string[]): number {
  ...
  private assessTherapeuticAlliance(history: SessionContext[]): number {
    return history.length > 0
      ? history.reduce((sum, h) => sum + (h.therapeuticAlliance || 0.5), 0) / history.length
      : 0.5;
  }

  private identifyCrisisIndicators(state: EmotionState, conversation: string[]): string[] {
  ...
  private determineSessionPhase(duration: number): "early" | "mid_session" | "late_session" {
