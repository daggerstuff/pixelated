...
export class PromptOptimizerService {
  ...
  private generateInsights(context: ConversationContext): string[] {
    ...
    const patientMessages = context.messages.filter(
      (m) => m.role === "patient",
    );
  ...
  private identifyRiskFactors(
    ...
    if (finalEmotion.dimensions.dominance < -0.6) {
      riskFactors.push(
        "Low sense of control/power - may indicate helplessness",
      );
    }
  ...
  private generateOptimizationReasoning(
    ...
    if (
      ...
      reasoning.push(
        "Reduced pressure and time constraints to improve comfort",
      );
    }
