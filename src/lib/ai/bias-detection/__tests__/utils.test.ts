...
describe("Demographic Processing", () => {
  ...
  describe("extractDemographicGroups", () => {
    it("should extract all demographic groups", () => {
      ...
      expect(groups.find((g) => g["type"] === "language")?.["value"]).toBe(
        "en",
      );
...
describe("HIPAA Compliance", () => {
  ...
  describe("requiresAdditionalAuth", () => {
    it("should require auth for high sensitivity data", () => {
      expect(requiresAdditionalAuth("session-data", "admin", "high")).toBe(
        true,
      );
    ...
    it("should require auth for demographics for non-admin", () => {
      expect(requiresAdditionalAuth("demographics", "viewer", "low")).toBe(
        true,
      );
      expect(requiresAdditionalAuth("demographics", "admin", "low")).toBe(
        false,
      );
    });

    it("should require auth for session data for viewers", () => {
      expect(requiresAdditionalAuth("session-data", "viewer", "low")).toBe(
        true,
      );
