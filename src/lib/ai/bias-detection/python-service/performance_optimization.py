...
class BatchProcessor:
    ...
    def _should_process_batch(self) -> bool:
        ...
        return len(self.current_batch) >= self.config.max_batch_size // 2
...
class MemoryManager:
    ...
    def track_session_processing(self) -> None:
        ...
        if self.session_counter % 100 == 0 and self.should_cleanup():
...
class PerformanceOptimizedBiasDetector:
    ...
    async def analyze_session_optimized(
        self, session_data: SessionData, user_id: str, use_cache: bool = True
    ) -> dict[str, Any]:
    ...
    async def analyze_batch_optimized(
        self, sessions: list[SessionData], user_id: str, use_parallel: bool = True
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if cache_misses:
                ...
                # Cache results
                if self.config.enable_caching:
                    for session, result in zip(cache_misses, missed_results, strict=False):
                        cache_key = self._generate_cache_key(session)
                        self.cache.set(cache_key, result)
            else:
                missed_results = []
    ...
    async def process_large_dataset(
        self, sessions: list[SessionData], user_id: str, chunk_size: int = 100
    ) -> dict[str, Any]:
...
# Global instance
performance_optimizer: PerformanceOptimizedBiasDetector | None = None
...
async def analyze_session_with_performance(
    session_data: SessionData, user_id: str, use_cache: bool = True
) -> dict[str, Any]:
...
async def analyze_batch_with_performance(
    sessions: list[SessionData], user_id: str, use_parallel: bool = True
) -> list[dict[str, Any]]:
...
if __name__ == "__main__":
    ...
    async def example():
        ...
        await optimizer.analyze_batch_optimized(test_sessions, "test_user")
