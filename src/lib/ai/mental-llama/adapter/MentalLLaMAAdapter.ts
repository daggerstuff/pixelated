...
import { specializedPrompts, buildGeneralAnalysisPrompt } from "../prompts/prompt-templates.ts";
import type {
...
export class MentalLLaMAAdapter {
  ...
  public async analyzeMentalHealth(
    ...
    const finalResult: MentalHealthAnalysisResult = {
      hasMentalHealthIssue:
        hasMentalHealthIssue ??
        (llmCategory !== "none" && llmCategory !== "wellness" && llmCategory !== "unknown"),
  ...
  private async performLLMAnalysis({
    ...
    let promptBuilder =
      ((specializedPrompts as Record<string, unknown>)[categoryForPrompt] as (params: {
        text: string;
        categoryHint: string;
      }) => Message[]) || buildGeneralAnalysisPrompt;
    ...
    try {
      ...
      try {
        const parsedLlmResponse = JSON.parse(llmResponseRaw.content) as {
