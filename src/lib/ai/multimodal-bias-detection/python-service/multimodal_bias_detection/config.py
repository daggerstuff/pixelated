...
class Settings(BaseSettings):
    ...
    database_url: PostgresDsn | str = Field(
        default="postgresql://user:password@localhost:5432/multimodal_bias_detection", validation_alias="DATABASE_URL"
    )
    ...
    allowed_image_formats: list = Field(
        default=["jpg", "jpeg", "png", "gif", "bmp", "webp"], validation_alias="ALLOWED_IMAGE_FORMATS"
    )
    allowed_audio_formats: list = Field(
        default=["mp3", "wav", "flac", "m4a", "ogg"], validation_alias="ALLOWED_AUDIO_FORMATS"
    )
    allowed_video_formats: list = Field(
        default=["mp4", "avi", "mov", "mkv", "webm"], validation_alias="ALLOWED_VIDEO_FORMATS"
    )

    # Logging settings
    log_level: str = Field(default="INFO", validation_alias="LOG_LEVEL")
    log_format: str = Field(default="json", validation_alias="LOG_FORMAT", description="Log format: json, text")
