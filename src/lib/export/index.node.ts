    ...
    try {
      ...
      switch (exportOptions.format) {
        ...
        case ExportFormat.ENCRYPTED_ARCHIVE: {
          const archive = await this.createEncryptedArchive(messages, exportOptions);
      ...
      if (exportOptions.includeVerificationToken) {
        verificationToken = await this.createVerificationToken(exportData, exportId, timestamp);
      }
  ...
  private async createPDFExport(
    ...
    try {
      ...
      if (!Array.isArray(messages) || messages.length === 0) {
        throw new Error("Invalid or empty messages array provided for PDF export");
      }
        ...
        try {
          ...
          doc
            ...
            .text(watermarkText, 0, 0, {
              align: "center",
              width: Math.sqrt(Math.pow(doc.page.width, 2) + Math.pow(doc.page.height, 2)),
            });
          doc.restore();
        } catch (err: unknown) {
      ...
      for (const [index, msg] of messages.entries()) {
        try {
          ...
          const contentLines = this.wrapText(sanitizedContent, doc.page.width - 100, 11);
