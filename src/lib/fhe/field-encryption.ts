...
function isEncryptedField(value: unknown): value is EncryptedField {
  return (
    typeof value === "object" && value !== null && "fhe" in value && (value as EncryptedField).fhe
  );
}
 ...
 */
export async function encryptField(field: string, value: unknown): Promise<EncryptedField> {
 ...
 */
export async function decryptField<T = string>(encryptedField: unknown): Promise<T | null> {
  ...
  try {
    ...
    if (encryptedField.originalType === "object" || encryptedField.originalType === "array") {
