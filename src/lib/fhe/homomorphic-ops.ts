...
function isSerializedCiphertextInput(value: unknown): value is SerializedCiphertextInput {
  return (
    isRecord(value) &&
...
function parseSerializedCiphertextInput(encryptedData: string): SerializedCiphertextInput {
  try {
    const parsed: unknown = JSON.parse(encryptedData);
    return isSerializedCiphertextInput(parsed) ? parsed : { serializedCiphertext: encryptedData };
  } catch {
...
function isSerializedSealObject(value: unknown): value is SerializedSealObject {
  return (
    isRecord(value) && typeof value["save"] === "function" && typeof value["delete"] === "function"
  );
}
...
function normalizeOptionalRecordToStringArray(value: unknown): Record<string, string[]> {
  if (!isRecord(value)) {
    return {};
  }
...
function getNumericArray(value: unknown, fallback: OptionalNumericArray = null): number[] {
...
function getStringArray(value: unknown): string[] | undefined {
  ...
  return value.every((token): token is string => typeof token === "string") ? value : undefined;
}
...
export class FHEOperationError extends Error {
  ...
  constructor(message: string, operation: FHEOperation | string, code = "OPERATION_ERROR") {
    super(message);
...
const SENTIMENT_WORDS = {
  ...
  neutral: ["maybe", "possibly", "perhaps", "okay", "fine", "average", "neutral", "unclear"],
};
...
export class HomomorphicOperations {
  ...
  public async processEncrypted(
    ...
    try {
      ...
      try {
        ...
        switch (operation) {
          ...
          case FHEOperation.CATEGORIZE:
            ...
            categoryResult = await this.simulateCategorization(serializedCiphertext, params);
            result = JSON.stringify({
              serializedCiphertext: categoryResult,
              operation: "categorize",
              timestamp: Date.now(),
            });
            metadata["categories"] = normalizeOptionalRecordToStringArray(params?.["categories"]);
            break;
      ...
      logger.error(`Failed to process encrypted data with operation ${operation}`, { error });
      return {
        success: false,
  ...
  private async simulateOperation(
      ...
      try {
        switch (operation) {
          case FHEOperation.SENTIMENT:
            ...
            break;
          case FHEOperation.CATEGORIZE:
            encResult = await textProcessor.encryptedCategorize(plaintextForEncoding);
            break;
          case FHEOperation.WORD_COUNT:
        ...
        if (encResult) {
          logger.info(`Operation ${operation} performed fully homomorphically`, {
    ...
    switch (operation) {
      ...
      case FHEOperation.SUMMARIZE:
        result = await this.summarizeText(decodedData, getNumericValue(params?.["maxLength"], 100));
      ...
      case FHEOperation.FILTER:
        result = await this.filterText(decodedData, getStringArray(params?.["filterTerms"]));
      ...
      case FHEOperation.CUSTOM:
        {
          ...
          result = await this.performCustomOperation(decodedData, customOperation, params);
      ...
      case FHEOperation.READING_LEVEL: {
        ...
        const sentenceCount = Math.max(decodedData.split(/[.!?]+/).filter(Boolean).length, 1);
   ...
   */
  private async filterText(text: string, filterTerms?: string[]): Promise<string> {
