...
export class FHECacheService {
  ...
  public async cacheResult(
    ...
    // Check size limit
    const dataSize =
      typeof encryptedResult.data === "string" ? Buffer.byteLength(encryptedResult.data) : 0;
   ...
   */
  public async generateInputHash(operation: FHEOperation, inputData: unknown): Promise<string> {
  ...
  public async prewarmCache(): Promise<void> {
    ...
    const commonPatterns = ["hello", "test message", "emotional data", "sentiment analysis"];
    ...
    for (const pattern of commonPatterns) {
      ...
      await this.cacheResult(FHEOperation.SENTIMENT, hash, encrypted, {
        ...
        outputSize: typeof encrypted.data === "string" ? encrypted.data.length : 0,
        durationMs: 0,
      });
    }
