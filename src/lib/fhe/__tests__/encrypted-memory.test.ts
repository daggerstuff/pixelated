...
describe('EncryptedMemoryService', () => {
  ...
  describe('session management', () => {
    ...
    it('should update session with new messages', async () => {
      await encryptedMemory.createSession(
        'test-session-456',
        'therapist-789',
        [],
      )

      const updated = await encryptedMemory.updateSession(
        'test-session-456',
        [],
      )
  ...
  describe('emotional state tracking', () => {
    ...
    it('should retrieve emotional trajectory', async () => {
      ...
      const trajectory =
        encryptedMemory.getEmotionalTrajectory('test-session-101')
    ...
    it('should analyze emotional trajectory', async () => {
      ...
      const analysis =
        await encryptedMemory.analyzeEmotionalTrajectory('test-session-102')
