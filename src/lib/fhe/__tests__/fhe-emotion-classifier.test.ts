...
import { FHEEmotionClassifier, createEmotionClassifierFHEService } from "../fhe-emotion-classifier";
...
describe("FHEEmotionClassifier", () => {
  ...
  it("should classify batch of texts", async () => {
    const texts = ["I am very happy today!", "I am very sad today!", "I am very angry today!"];
  ...
  it("should compute session trajectory", async () => {
    const texts = ["I am very happy today!", "I am a bit sad now.", "I am very angry today!"];
    const results = await classifier.classifyBatch(texts);
...
describe("FHEEmotionClassifier error paths", () => {
  it("should throw when FHE service lacks processEncrypted", async () => {
    ...
    await expect(c.classify("hello")).rejects.toThrow("does not support encrypted processing");
  });
