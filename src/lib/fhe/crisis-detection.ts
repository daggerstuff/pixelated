...
export class EncryptedCrisisDetectionService {
  ...
  public static getInstance(): EncryptedCrisisDetectionService {
    EncryptedCrisisDetectionService.instance ??= new EncryptedCrisisDetectionService();
  ...
  public async analyzeMessages(
    ...
    for (const message of messages) {
      ...
      for (const [patternName, patternVector] of this.riskPatterns) {
        const score = await this.homomorphicMatch(encryptedMsg, patternVector, message.content);
        encryptedKeywordScores.push(score);

        // Update indicators based on thresholds
        if (score > this.thresholds[`${patternName}Threshold` as keyof DetectionThresholds]) {
    ...
    const assessment: EncryptedCrisisAssessment = {
      ...
      recommendations: this.generateRecommendations(encryptedIndicators, riskLevel),
    };
  ...
  private keywordMatchScore(text: string, patternVector: number[]): number {
    ...
    const keywordGroups = [
      ["knife", "cut", "wound", "razor", "blade", "hurt", "harm", "damage", "injure", "bleed"],
      ["suicide", "kill", "die", "death", "end", "done", "finish", "dead", "dying"],
      ["hurt", "harm", "attack", "kill", "murder", "violent", "weapon", "gun", "fight"],
      ["abuse", "assault", "rape", "sexual", "domestic", "partner", "child", "vulnerable"],
      ["overwhelmed", "can't", "couldn't", "hopeless", "despair", "panic", "anxiety"],
   ...
   */
  private updateIndicator(indicators: CrisisIndicators, patternName: string): void {
    switch (patternName) {
      case "self_harm":
   ...
   */
  private generateRecommendations(indicators: CrisisIndicators, riskLevel: CrisisLevel): string[] {
   ...
   */
  private async encryptRiskKeywords(messages: ChatMessage[]): Promise<EncryptedData[]> {
   ...
   */
  public async monitorSession(sessionId: string): Promise<EncryptedCrisisAssessment | null> {
   ...
   */
  public async getAssessmentHistory(patientId: string): Promise<EncryptedCrisisAssessment[]> {
   ...
   */
  public async alertOnCriticalRisk(assessment: EncryptedCrisisAssessment): Promise<void> {
