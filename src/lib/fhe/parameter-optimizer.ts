...
export class FHEParameterOptimizer {
  ...
  public getOptimizedParametersForOperations(
    ...
    const maxComplexity = Math.max(...operations.map((op) => OPERATION_COMPLEXITY[op] || 5));
    ...
    const params = this.createCustomParameters(basePreset, maxComplexity, scheme);

    // Calculate estimates for the result
    const estimatedSecurity = this.estimateSecurityBits(params);
    const estimatedPerformance = this.estimatePerformanceScore(params, operations);
  ...
  public optimizeForContext(
    ...
    const adjustedComplexity = Math.min(10, Math.max(1, baseComplexity * complexityModifier));
  ...
  private adaptiveOptimization(
    ...
    const relevantMetrics = this.performanceHistory.filter((m) => m.operation === operation);

    // If we don't have enough data, fall back to balanced approach
    if (relevantMetrics.length < 3) {
      logger.debug("Insufficient history for adaptive optimization, using balanced approach");
    ...
    if (avgDuration > 500) {
      ...
      logger.debug("Operations have reasonable performance, using balanced approach");
      return this.balancedOptimization(
        scheme === SealSchemeType.CKKS
          ? "ckks-default"
          : scheme === SealSchemeType.BGV
   ...
   */
  private getComplexityModifierFromContext(context: FHEOperationContext): number {
    // Default modifier
    let modifier = 1.0;
    ...
    if (context.parameters) {
      // Check for data size
      const dataSize =
        typeof context.parameters["dataSize"] === "number" ? context.parameters["dataSize"] : 0;
      if (dataSize > 1000000) {
      ...
      const batchSize =
        typeof context.parameters["batchSize"] === "number" ? context.parameters["batchSize"] : 0;
      if (batchSize > 10) {
  ...
  private estimatePerformanceScore(
    ...
    const avgComplexity =
      operations.reduce((sum, op) => sum + (OPERATION_COMPLEXITY[op] || 5), 0) / operations.length;
