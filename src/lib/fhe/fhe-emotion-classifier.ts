...
export class FHEEmotionClassifier {
  ...
  async classifyBatch(
    ...
    for (const batch of batches) {
      const batchResults = await Promise.all(
        batch.map(async (encryptedStr) => {
          ...
          return this.fheService.processEncrypted(
            encryptedStr,
            FHEOperation.EMOTION_CLASSIFY,
            {
              multiLabel,
    ...
    for (const result of encryptedResults) {
      if (!result.success) {
        throw new Error(
          `FHE emotion classification failed: ${result.error ?? 'Unknown error'}`,
        )
      }
      if (!result.result) {
    ...
    const decryptedStrings =
      await this.fheService.decryptBatch<string>(encryptedDataResults)
