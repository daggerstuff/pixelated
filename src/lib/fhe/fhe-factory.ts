...
if (mockFHEService && typeof mockFHEService.scheme === "object" && mockFHEService.scheme !== null) {
  ...
  console.warn("[fhe-factory] mockFHEService.scheme is not an object, cannot set version.");
}
...
const sealFHEService: FHEService = {
  ...
  async decrypt<T>(encryptedData: EncryptedData, _options?: unknown): Promise<T> {
    try {
      // Extract serializedCiphertext from metadata if available
      const ciphertext =
        encryptedData.metadata?.["serializedCiphertext"] ?? (encryptedData.data as string);
      ...
      sealCipherText.load(context, ciphertext);
  ...
  async add(a: EncryptedData, b: EncryptedData | number): Promise<EncryptedData> {
    try {
      ...
      // Extract serializedCiphertext from metadata if available
      const aCiphertextStr = a.metadata?.["serializedCiphertext"] ?? (a.data as string);
      const bCiphertextStr = b.metadata?.["serializedCiphertext"] ?? (b.data as string);
      ...
      aCiphertext.load(context, aCiphertextStr);

      const bCiphertext = seal.CipherText();
      bCiphertext.load(context, bCiphertextStr);
  ...
  async subtract(a: EncryptedData, b: EncryptedData | number): Promise<EncryptedData> {
    try {
  ...
  async multiply(a: EncryptedData, b: EncryptedData | number): Promise<EncryptedData> {
    try {
      ...
      const result = await sealOperations.multiply(aCiphertext as string, bCiphertext as string);
  ...
  async negate(value: EncryptedData): Promise<EncryptedData> {
    try {
      // Extract serializedCiphertext from metadata if available
      const ciphertext = value.metadata?.["serializedCiphertext"] ?? (value.data as string);
  ...
  async applyPolynomial(value: EncryptedData, coefficients: Polynomial): Promise<EncryptedData> {
    try {
      // Extract serializedCiphertext from metadata if available
      const ciphertext = value.metadata?.["serializedCiphertext"] ?? (value.data as string);
  ...
  async rotate(vector: EncryptedData, steps: number): Promise<EncryptedData> {
    try {
      // Extract serializedCiphertext from metadata if available
      const ciphertext = vector.metadata?.["serializedCiphertext"] ?? (vector.data as string);
 ...
 */
function createEncryptedData(serializedCiphertext: string, originalData?: number[]): EncryptedData {
  return {
 ...
 */
export async function getFHEService(options: FHEFactoryOptions = {}): Promise<FHEService> {
  const {
    ...
    case FHEImplementation.Mock:
      ...
      if (requiredOperations.length > 0) {
        ...
        if (unsupportedOps.length > 0) {
          logger.warn(`Mock FHE service does not support operations: ${unsupportedOps.join(", ")}`);
        }
      }
...
export async function getTenantFHEService(
  ...
  if (!tenant) {
    logger.warn(`Tenant ${tenantId} not registered, registering with default configuration`);
  ...
  if (tenant?.isolationLevel === "dedicated") {
    ...
    const tenantService: FHEService = {
      ...
      initialize: async (config?: FHEConfig): Promise<void> => {
        ...
        logger.info(`Tenant ${tenantId} FHE service initialized with dedicated resources`);
      },
    };
  ...
  const tenantService: FHEService = {
    ...
    decrypt: async <T>(encryptedData: EncryptedData, options?: unknown): Promise<T> => {
      ...
      if (
        ...
        throw new Error("Access denied: cannot decrypt data from another tenant");
      }
