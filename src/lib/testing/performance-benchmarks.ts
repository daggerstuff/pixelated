...
export class PerformanceBenchmarkRunner {
  ...
  async runFullBenchmark(): Promise<PerformanceResults[]> {
    ...
    for (const [_key, scenario] of Object.entries(performanceScenarios)) {
      ...
      for (const concurrentUsers of benchmarkConfig.concurrentUsers) {
        ...
        logger.info(`   ✅ Success Rate: ${(result.errorRate * 100).toFixed(2)}%`);
        logger.info(`   ⚡ Avg Response Time: ${result.averageResponseTime.toFixed(2)}ms`);
  ...
  generateReport(results: PerformanceResults[]): string {
    ...
    const avgThroughput =
      allResults.reduce((sum, r) => sum + r.requestsPerSecond, 0) / allResults.length;
  ...
  async runMemoryLeakTest(): Promise<{
    ...
    logger.info(`   Initial Memory: ${(initialMemory / 1024 / 1024).toFixed(2)} MB`);
...
if ((require as NodeRequire).main === module) {
  ...
  runner
    ...
    .then((memoryResults) => {
      ...
      logger.info(`   Leak Detected: ${memoryResults.leakDetected ? "YES" : "NO"}`);
