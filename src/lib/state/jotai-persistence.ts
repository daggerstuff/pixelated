    ...
    try {
      ...
      if (this.options.encrypt || serialized.startsWith("enc:")) {
        try {
          ...
          logger.warn("Failed to decrypt stored state, treating as plain text:", error);
          // Continue with original string - might be legacy unencrypted data
        }
      }

      const storedState: StoredState<Value> = JSON.parse(decrypted) as unknown as any;
      ...
      // Handle migration
      if (this.options.migration && storedState.metadata.version < this.options.version) {
        logger.debug(
          `Migrating state for ${this.key} from version ${storedState.metadata.version} to ${this.options.version}`,
        );
        const migratedData = this.options.migration(storedState.data, storedState.metadata.version);
...
export function atomWithEnhancedStorage<Value>(
  ...
  return atomWithStorage(
    ...
    createJSONStorage(
      () =>
        storage as unknown as Parameters<typeof createJSONStorage>[0] extends () => infer T
          ? T
          : never,
    ),
  );
...
export class StatePersistenceManager {
  ...
  async clearAllPersistedState(): Promise<void> {
    ...
    const keys = Object.keys(localStorage).filter((key) => key.startsWith("jotai_"));
  ...
  getStorageStats(): { totalKeys: number; totalSize: number; keys: string[] } {
    ...
    const jotaiKeys = Object.keys(localStorage).filter((key) => key.startsWith("jotai_"));
