...
export class DeploymentOrchestrator extends EventEmitter {
   ...
   */
  private createExecution(planId: string, _regions: RegionConfig[]): DeploymentExecution {
    return {
    ...
    try {
      ...
      for (const validationStep of plan.validationSteps) {
        ...
        try {
          ...
          validationResults.push(`✗ ${validationStep.name}: ${error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error"}`);
          validationFailed = true;
        }
      }
      ...
      return { success: false, errors: [error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error"] };
    }
  }
