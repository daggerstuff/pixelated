...
export class HealthMonitor extends EventEmitter {
  ...
  private initializeHealthData(): void {
    for (const region of this.regions) {
      ...
      this.healthScores.set(
        region.id,
        this.createInitialHealthScore(region.id),
      );
    }
  }
  ...
  private calculatePerformanceScore(
    ...
    if (current.responseTime > this.config.thresholds.responseTime) {
      ...
      score -= Math.min(
        30,
        (excess / this.config.thresholds.responseTime) * 30,
      );
    }
  ...
  private calculateAvailabilityScore(
    ...
    if (current.availability < this.config.thresholds.availability) {
      const deficit =
        this.config.thresholds.availability - current.availability;
      score -= Math.min(
        50,
  ...
  private calculateCapacityScore(
    ...
    if (current.diskUtilization > this.config.thresholds.disk) {
      ...
      score -= Math.min(
        40,
        (excess / (100 - this.config.thresholds.disk)) * 40,
      );
    }
