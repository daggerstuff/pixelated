...
describe("GovernanceBridge", () => {
  ...
  describe("compliance integration", () => {
    ...
    it("records compliance failure event with reasons", async () => {
      await bridge.recordComplianceDecision("access_phi", false, ["FHE encryption required"]);
  ...
  describe("Slack alert wiring", () => {
    ...
    it("sends alert via Slack when threshold breached", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        await bridge.recordComplianceDecision("test_op", false, ["FHE encryption required"]);
      }
