...
const getDocumentationSystemInstance = async (): Promise<DocumentationSystem> => {
  if (!documentationSystemInstance) {
    ...
    const aiService = {
      ...
      createStreamingChatCompletion: llmService.createStreamingChatCompletion.bind(llmService),
  ...
  const saveDocumentation = useCallback(
    async (updatedDocumentation: SessionDocumentation): Promise<boolean> => {
      try {
        ...
        const success = await documentationSystem.saveDocumentation(sessionId, docToSave);
  ...
  const exportToEHR = useCallback(
    async (options: EHRExportOptions): Promise<EHRExportResult> => {
      ...
      try {
        ...
        const documentationSystem = await getDocumentationSystemInstance();
        const rawResult = (await documentationSystem.exportToEHR(sessionId, options)) as {
          success?: boolean;
          format?: string;
          metadata?: unknown;
          data?: unknown;
        ...
        const result: EHRExportResult = {
          ...
          format:
            typeof rawResult.format === "string"
              ? (rawResult.format as "fhir" | "ccda" | "pdf")
              : "fhir",
          metadata:
          ...
          ...(rawResult.data != null ? { data: rawResult.data as Record<string, unknown> } : {}),
          ...(Array.isArray(rawResult.errors) ? { errors: rawResult.errors.map(String) } : {}),
        };

        safeSetState(setExportResult, result);

        if (result.success) {
          toast.success(`Documentation exported successfully to ${options.format.toUpperCase()}`);
        } else {
          const errorMsg =
