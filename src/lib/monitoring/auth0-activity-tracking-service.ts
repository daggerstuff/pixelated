...
export class Auth0ActivityTrackingService {
  ...
  private async fetchAndStoreRecentActivities(): Promise<void> {
    try {
      ...
      const lastLog = logs[logs.length - 1] as { log_id: string };
      ...
      for (const log of logs) {
        ...
        switch (log.type) {
          ...
          case "s": // Success login
            ...
            logSecurityEvent(SecurityEventType.AUTHENTICATION_SUCCESS, log.user_id, {
              logId: log.log_id,
              ipAddress: log.ip,
              userAgent: log.user_agent,
        ...
        if (securityEvent) {
          ...
          logSecurityEvent(securityEvent.type, securityEvent.userId ?? null, {
  ...
  async getUserActivitySummary(userId: string): Promise<ActivitySummary | null> {
    try {
      ...
      const activeDays = new Set(activities.map((a) => a.timestamp.toISOString().split("T")[0]))
        .size;
  ...
  async getRealTimeActivityStream(limit: number = 50): Promise<UserActivity[]> {
    try {
      ...
      const activities = await collection.find({}).sort({ timestamp: -1 }).limit(limit).toArray();
