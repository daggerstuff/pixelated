...
const DEFAULT_LEDGER_FILE = path.resolve(
  process.cwd(),
  'data',
  'agent-note-collab',
  'turns.json',
...
async function readStoredTurns(
  filePath: string,
): Promise<StoredTurnsReadResult> {
  try {
...
export class PersistentTurnLedger {
  ...
  constructor(options?: PersistentTurnLedgerOptions) {
    ...
    const configuredThreshold =
      process.env['AGENT_NOTE_COLLAB_LEDGER_COMPACT_AFTER_BYTES']
    const fallbackValue = COMPACTION_AFTER_BYTES
    const parsedThreshold = configuredThreshold
      ? Number.parseInt(configuredThreshold, 10)
      : fallbackValue
    this.compactAfterBytes =
  ...
  private async loadFromDisk(): Promise<void> {
    ...
    const { turns: storedTurns, hasValidSnapshot } = await readStoredTurns(
      this.storagePath,
    )
