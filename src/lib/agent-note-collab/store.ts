...
export class InMemoryTurnLedger implements TurnLedger {
  ...
  list(query: TurnQuery = {}): NoteTurn[] {
    ...
    const normalizedAction = query.requestedAction
      ? normalizeRequestedAction(query.requestedAction)
      : undefined
    ...
    const filtered = sourceTurns.filter((turn) => {
      ...
      if (normalizedAction && turn.requestedAction !== normalizedAction)
        return false
      return true
    })
  ...
  openQuestionCount(artifactId: string): number {
    return (this.turnsByArtifact.get(artifactId.trim()) ?? []).reduce(
      (count, turn) => {
  ...
  submitTurn(
    input: Parameters<typeof createTurnRecord>[0],
  ): TurnSubmissionResult | TurnSubmissionFailure {
...
export function formatTurnError(error: unknown): TurnSubmissionFailure {
  ...
  return {
    ...
    errors: [
      {
        code: 'UNKNOWN_ERROR',
        message: 'An unknown error occurred while submitting the turn.',
      },
    ],
  }
}
