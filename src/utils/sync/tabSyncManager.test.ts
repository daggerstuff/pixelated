...
describe("MessagingTransport", () => {
  ...
  it("should catch and log errors during BroadcastChannel initialization", () => {
    ...
    expect(consoleErrorSpy.mock.calls[0][1].message).toBe("Simulated BroadcastChannel failure")
  })
  ...
  it("should handle BroadcastChannel missing support", () => {
    ...
    expect(success).toBe(false)
    expect(consoleWarnSpy).toHaveBeenCalledWith("MessagingTransport: BroadcastChannel not supported")
