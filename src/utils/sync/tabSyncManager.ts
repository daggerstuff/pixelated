  ...
  resolve<T>(key: string, localValue: T, remoteValue: T): T {
    switch (this.config.strategy) {
      ...
      case "manual":
        ...
        throw new Error("Manual conflict resolution requires an onConflict handler");
...
export class StorageAdapter {
   ...
   */
  get<T>(key: string, defaultValue?: T, options?: Partial<StorageConfig>): T | undefined {
...
export class MessagingTransport {
  ...
  private messageHandlers = new Map<string, Set<(message: SyncMessage) => void>>();
...
export class SyncOrchestrator {
  ...
  validateStateUpdate(
    ...
    if (checksum && computedChecksum !== null && computedChecksum !== checksum) {
      console.warn(`SyncOrchestrator: Checksum mismatch for key ${key}, rejecting update`);
...
class TabSyncManager {
  ...
  constructor(config: TabSyncConfig = {}) {
    ...
    this.orchestrator = new SyncOrchestrator(this.config.enableVersioning, this.config.maxVersions);
  }
   ...
   */
  syncState(key: string, value: any, sourceId?: string, serializedValue?: string): boolean {
   ...
   */
  getState<T>(key: string, defaultValue?: T, options?: Partial<StorageConfig>): T | undefined {
