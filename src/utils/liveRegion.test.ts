...
import { announceStatus, announceAlert, log, announceProgress } from './liveRegion';
...
describe('liveRegion utilities', () => {
  ...
  describe('announceStatus', () => {
    it('uses window.LiveRegionSystem if available', () => {
      ...
      expect(window.LiveRegionSystem.announceStatus).toHaveBeenCalledWith('Test status', 1000);
    });
