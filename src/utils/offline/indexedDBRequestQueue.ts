  ...
  private async loadFromStorage(): Promise<void> {
    ...
    try {
      ...
      if (result && Array.isArray(result.value)) {
        ...
        const now = Date.now();
        this.queue = stored.filter((req: QueuedRequest) => now - req.timestamp < maxAge);
      } else {
        this.queue = [];
      }
    } catch (error) {
      ...
      while (this.queue.length > 0) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (this.options.enablePersistence) {
              this.saveToStorage().catch((err) => {
                console.warn("Failed to persist request queue after processing:", err);
              });
            }
          ...
          if (request.retryCount >= request.maxRetries) {
            ...
            if (this.options.enablePersistence) {
              this.saveToStorage().catch((err) => {
                console.warn("Failed to persist request queue after max retries:", err);
              });
            }
          } else {
