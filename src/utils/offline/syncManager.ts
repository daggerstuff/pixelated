...
class SyncManager {
  ...
  private async attemptSync(): Promise<void> {
    ...
    try {
      const queueLengthBefore = indexedDBRequestQueue.hasPendingRequests() ? 1 : 0; // We don't have a direct way to get length without exposing it, but we can use hasPendingRequests as a boolean
      // We'll call processQueue and then check if the queue is still pending
      await indexedDBRequestQueue.processQueue();
      const queueLengthAfter = indexedDBRequestQueue.hasPendingRequests() ? 1 : 0;
  ...
  private scheduleNextSync(): void {
    this.clearSyncTimeout();
    const delay = Math.min(this.options.baseDelay * this.backoffMultiplier, this.options.maxDelay);
