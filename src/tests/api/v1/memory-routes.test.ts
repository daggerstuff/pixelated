...
import { GET as listMemories, POST as createMemory } from "../../../pages/api/v1/memory/index";
import {
  GET as getMemoryById,
  PATCH as patchMemoryById,
  DELETE as deleteMemoryById,
} from "../../../pages/api/v1/memory/[memoryId]";
import { GET as searchGet, POST as searchPost } from "../../../pages/api/v1/memory/search";
...
describe("GET /api/v1/memory", () => {
  ...
  it("returns 502 when gateway throws 502", async () => {
    gateway.listMemories.mockRejectedValue(new ProductMemoryGatewayError("upstream error", 502));
...
describe("DELETE /api/v1/memory/:memoryId", () => {
  ...
  it("returns 200 with deleted id", async () => {
    ...
    expect(body.data.id).toBe(id);
    expect(gateway.deleteMemory).toHaveBeenCalledWith(expect.objectContaining({ memoryId: id }));
  });
...
describe("GET /api/v1/memory/search", () => {
  ...
  it("returns 502 when gateway errors", async () => {
    gateway.searchMemories.mockRejectedValue(new ProductMemoryGatewayError("search failed", 502));
    const response = await searchGet({
      request: makeRequest("http://localhost/api/v1/memory/search?q=test"),
    });
    expect(response.status).toBe(502);
