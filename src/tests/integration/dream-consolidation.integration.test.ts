...
import type { RunResult } from "@/services/dream-scheduler";
...
describe("DreamScheduler", () => {
  ...
  describe("runOnce()", () => {
    it("processes one user successfully", async () => {
      ...
      fetchSpy.mockResolvedValueOnce(mockJsonResponse({ dream_id: "dream_abc123" }));
    ...
    it("skips when cycle is already in progress", async () => {
      ...
      fetchSpy.mockReset();
      fetchSpy.mockResolvedValueOnce(mockJsonResponse({ dream_id: "dream_cancel" }));
    });

    it("marks user as failed on HTTP error", async () => {
      const fetchSpy = vi.spyOn(globalThis, "fetch");
      fetchSpy.mockResolvedValueOnce(new Response("", { status: 502, statusText: "Bad Gateway" }));
    ...
    it("fetches active users when none specified", async () => {
      const fetchSpy = vi.spyOn(globalThis, "fetch");
      fetchSpy
        .mockResolvedValueOnce(mockJsonResponse({ users: ["active-1", "active-2"] }))
    ...
    it("records duration after completion", async () => {
      const fetchSpy = vi.spyOn(globalThis, "fetch");
      fetchSpy.mockResolvedValueOnce(mockJsonResponse({ dream_id: "dream_xyz" }));
  ...
  describe("isRunning guard", () => {
    it("sets isRunning to false after normal completion", async () => {
      const fetchSpy = vi.spyOn(globalThis, "fetch");
      fetchSpy.mockResolvedValueOnce(mockJsonResponse({ dream_id: "dream_normal" }));

      await scheduler.runOnce(["user-normal"]);
      expect(scheduler.active).toBe(false);

      fetchSpy.mockReset();
      fetchSpy.mockResolvedValueOnce(mockJsonResponse({ dream_id: "dream_normal2" }));
...
describe("FlaskBlueprint contract", () => {
  ...
  it("scheduler GET /users returns user list for auto-discovery", async () => {
    ...
    fetchSpy
      .mockResolvedValueOnce(mockJsonResponse({ success: true, users: mockUsers }))
  ...
  it("scheduler handles duplicate consolidation gracefully (idempotent Flask)", async () => {
    ...
    fetchSpy
      .mockResolvedValueOnce(mockJsonResponse({ success: true, dream_id: sharedDreamId }))
      .mockResolvedValueOnce(mockJsonResponse({ success: true, dream_id: sharedDreamId }));
  ...
  it("scheduler continues to next user after failure (does not halt)", async () => {
    ...
    fetchSpy
      ...
      .mockResolvedValueOnce(mockJsonResponse({ success: true, dream_id: "dream_ok" }));
