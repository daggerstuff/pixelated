...
describe("System Health Dashboard Page", () => {
  ...
  it("fetches and displays health data", async () => {
    ...
    await waitFor(() => {
      ...
      // Check CPU info
      expect(screen.getByText(/CPU: Intel\(R\) Core\(TM\) i7-10700K \(8 cores\)/)).toBeTruthy();

      // Check load average
      expect(screen.getByText(/Load Average: 1.50 \(1m\), 1.20 \(5m\), 0.90 \(15m\)/)).toBeTruthy();
    });
  });
  ...
  it("handles unhealthy status correctly", async () => {
    ...
    await waitFor(() => {
      ...
      // Check error message
      expect(screen.getByText(/Details: Database connection failed/)).toBeTruthy();
    });
  });
});
