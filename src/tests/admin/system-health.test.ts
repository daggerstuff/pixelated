...
async function renderMockComponent(
  ...
  const mockHtml = `
    <div>
      ...
      <div>API status: ${isHealthy ? "healthy" : "unhealthy"}</div>
      ${!isHealthy ? "<div>Database status: unhealthy</div><div>Details: Database connection failed</div>" : ""}
      <div>50%</div>
...
describe("System Health Dashboard Page", () => {
  ...
  it("handles unhealthy status correctly", async () => {
    ...
    await renderMockComponent("unhealthy");
