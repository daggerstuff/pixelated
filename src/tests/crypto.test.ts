...
class KeyRotationManager {
  ...
  private readonly keys: Map<string, KeyMetadata> = new Map<string, KeyMetadata>();
...
describe("createCryptoSystem", () => {
  const createCryptoSystemMock = (options: CryptoSystemTestOptions): ExtendedCryptoSystem => {
    ...
    return {
      ...
      encrypt: async (_data: string, _key: string) => `v1:${_key ?? "default"}:${_data}`,
      ...
      verify: async (data: string, signature: string) => signature === `sig-${data}`,
      ...
      encryptWithKeyManagement: async (data: string, _purpose?: string) => `enc-${data}`,
      decryptWithKeyManagement: async (encryptedData: string) => encryptedData.replace("enc-", ""),
      close: async () => {},
    };
  };
  ...
  beforeEach(() => {
    vi.spyOn(cryptoModule, "createCryptoSystem").mockImplementation(
      (options?: CryptoSystemOptions) =>
        createCryptoSystemMock(options ?? { namespace: "test" }) as unknown as CryptoSystem,
    );
  });
...
describe("Fully Homomorphic Encryption Integration Tests", () => {
  ...
  it.skipIf(SKIP_FHE_TESTS)("should process data securely with FHE", async () => {
  ...
  it.skipIf(SKIP_FHE_TESTS)("should verify sender identity securely", async () => {
  ...
  it.skipIf(SKIP_FHE_TESTS)("should encrypt and decrypt data securely", async () => {
    const data = {
