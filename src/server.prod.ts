...
const redis = new Redis(productionConfig.redis.url, {
  lazyConnect: true,
  tls: productionConfig.redis.url.startsWith("rediss://") ? productionConfig.redis.tls : undefined,
} as RedisOptions);
...
const startServer = () => {
  server.listen(PORT, () => {
    ...
    console.log(`🔧 Health check: http${isProduction ? "s" : ""}://localhost:${PORT}/health`);
  });
};

// Only start the server if this file is run directly
const isMain =
  process.argv[1]?.includes("server.prod.js") ?? process.argv[1]?.includes("server.prod.ts");
if (isMain) {
  startServer();
}
