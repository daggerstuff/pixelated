...
describe("errorResponse", () => {
  ...
  it("includes details verbatim when present (object, array, string)", async () => {
    ...
    const arrayRes = errorResponse(400, "Bad Request", "Invalid", ["field-a", "field-b"]);
    const arrayData = await arrayRes.json();
    expect(arrayData["details"]).toEqual(["field-a", "field-b"]);

    const stringRes = errorResponse(400, "Bad Request", "Invalid", "single-line detail");
  ...
  it("preserves HTTP status codes 404 and 500 end-to-end", async () => {
    ...
    const serverError = errorResponse(500, "Internal Server Error", "Unexpected failure");
