  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!WEBHOOK_SECRET) {
      ...
      res.status(501).json({
        ...
        message: "LINEAR_DASHBOARD_WEBHOOK_SECRET environment variable is not set",
      });
      return;
    }
    ...
    if (eventType !== "Issue" && eventType !== "Project") {
      res.status(200).json({ status: "ignored", reason: `Unwatched event type: ${eventType}` });
      return;
    }
    ...
    console.log("[webhook] Change detected in Enterprise Readiness Program — refreshing dashboard");
