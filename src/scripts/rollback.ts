...
async function getLastStableVersion(
  ...
  try {
    ...
    const tagResult = spawnSync("git", ["tag", "-l", "production-*", "--sort=-committerdate"]);
    ...
    if (result.status !== 0) {
      throw new Error(`Failed to get deployment history: ${result.stderr.toString()}`);
    }
    ...
    const lastStable = deployments.find((d) => d.state === "READY" && !d.meta?.rollback);

    if (!lastStable) {
      // If no stable version found and a fallback branch is specified, use i
      if (options.fallbackBranch && options.fallbackBranch !== "use-current-tag") {
        console.log(
...
async function performRollback(options: RollbackOptions): Promise<boolean> {
  try {
    ...
    const version = options.version ?? (await getLastStableVersion(options.environment, options));
    ...
    if (
      ...
      if (checkoutResult.status !== 0) {
        throw new Error(`Failed to checkout ${version}: ${checkoutResult.stderr.toString()}`);
      }

      // Deploy from the checked out version
      console.log("Deploying from the previous version...");
      const deployResult = spawnSync("pnpm", ["vercel", "deploy", "--prod", "--yes"]);
    ...
    const verify = spawnSync("pnpm", ["tsx", "src/scripts/verify-deployment.ts"]);
