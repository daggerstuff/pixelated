...
program.description("Test the security breach notification system").version("1.0.0");
...
program
  ...
  .action(async (_options: unknown) => {
    ...
    try {
      ...
      const isBreachType = (value: string): value is BreachType => breachTypeSet.has(value);
      ...
      if (severity === "high" || severity === "critical") {
        console.log(chalk.yellow("⚠️  Note: This will send actual test notifications to Slack"));
...
program
  ...
  .option("-d, --description <text>", "Breach description", "Security breach detected")
  .option("-a, --affected-data <items>", "Affected data (comma separated)", "user_data")
  .option("-u, --users <ids>", "Affected user IDs (comma separated)", "user_1,user_2")
  .option("-m, --detection <method>", "Detection method", "security monitoring")
  .option("-r, --remediation <steps>", "Remediation steps", "System secured, investigation ongoing")
  .action(
    async (options: {
      type: string;
      severity: string;
      ...
      try {
        ...
        console.log(chalk.yellow("- Slack channels (configured via SLACK_WEBHOOK)"));
        console.log(chalk.yellow("- Email recipients defined in SECURITY_STAKEHOLDERS"));
        console.log(chalk.yellow("- Affected users (if they exist in the system)"));
        ...
        const confirm = await promptForConfirmation("Are you ABSOLUTELY SURE you want to proceed?");
        ...
        if (options.severity === "critical") {
          ...
          console.log(chalk.red("This will initiate emergency response protocols"));
  ...
  .action(async (_options: unknown) => {
    try {
      ...
      breaches.slice(0, limit).forEach((breach, index) => {
        ...
        console.log(`   Time: ${chalk.gray(new Date(breach.timestamp).toLocaleString())}`);
        console.log(`   Affected Users: ${chalk.yellow(breach.affectedUsers.length.toString())}`);
        console.log(
...
program
  ...
  .action(async (_id: unknown) => {
    try {
      ...
      console.log(`Affected Data: ${chalk.yellow(breach.affectedData.join(", "))}`);
