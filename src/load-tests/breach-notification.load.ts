...
const BREACH_TYPES = ["unauthorized_access", "data_leak", "system_compromise", "other"];
...
function generateBreachDetails() {
  ...
  const affectedData = Array.from({ length: dataTypeCount })
    .fill("")
    .map(() => AFFECTED_DATA_TYPES[Math.floor(Math.random() * AFFECTED_DATA_TYPES.length)])
...
export default function () {
  ...
  const createResponse = http.post("/api/security/breaches", JSON.stringify(breachDetails), {
    headers: {
      "Content-Type": "application/json",
  ...
  const success = check(createResponse, {
    "breach created successfully": (r: unknown) => (r as any).status === 200,
    "has breach ID": (r: unknown) => (r as any).json("id") !== undefined,
  });

  if (!success) {
    breachCreationErrors.add(1);
    console.error(`Failed to create breach: ${createResponse.status} ${createResponse.body}`);
    return;
  }
  ...
  if (completed) {
    const notificationResponse = http.get(`/api/security/breaches/${breachId}/notifications`);

    check(notificationResponse, {
      "notifications sent successfully": (r: unknown) => (r as any).status === 200,
      "all notifications delivered": (r: unknown) => {
        const data = (r as any).json() as NotificationResponse;
        return (
...
export function setup() {
  ...
  check(response, {
    "test environment setup successfully": (r: unknown) => (r as any).status === 200,
  });
...
export function teardown(data: { testRunId: string }): void {
  ...
  check(response, {
    "test environment cleaned up successfully": (r: unknown) => (r as any).status === 200,
  });
}
