...
export function useMemory(options: UseMemoryOptions = {}): UseMemoryReturn {
  ...
  const handleError = useCallback((err: unknown) => {
    const errorMessage =
      err instanceof Error ? err?.message || String(err) : "An unknown error occurred";
  ...
  const addMemory = useCallback(
    async (content: string, metadata?: MemoryEntry["metadata"]): Promise<string> => {
      setError(null);
  ...
  const searchMemories = useCallback(
    async (query: string, searchOptions?: Partial<SearchOptions>): Promise<MemoryEntry[]> => {
      setError(null);
...
export function useUserPreferences(userId: string): UseUserPreferencesReturn {
  ...
  const getPreference = useCallback(
    (key: string): string | number | boolean | object | null => {
      const prefMemory = memory.memories.find((m) => m.content.includes(`User preference: ${key}`));

      if (prefMemory) {
        try {
          const match = prefMemory.content.match(/= (.+)$/);
          return match?.[1]
            ? (JSON.parse(match[1]) as string | number | boolean | object | null)
            : null;
        } catch {
          return null;
        }
