...
describe("useEmotionDetection", () => {
  ...
  const mockStreamResponse = new ReadableStream({
    start(controller): void {
      controller.enqueue(new TextEncoder().encode(JSON.stringify(mockAIResponse)));
      controller.close();
    },
  });
  ...
  it("should detect emotions from text content", async () => {
    ...
    const analysis = await result.current.detectEmotions("I am feeling really happy today!");
