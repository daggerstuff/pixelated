...
const DATABASE_URL =
  process.env["DATABASE_URL"] ?? "postgresql://postgres:postgres@localhost:5432/pixelated";
...
if (typeof redis.connect === "function") {
  redis.connect().catch((err) => {
    if (process.env["NODE_ENV"] === "development") {
      ...
      redis = redisMock;
      setRedisClient(redisMock as unknown as Parameters<typeof setRedisClient>[0]);
    } else {
  ...
  if (process.env["NODE_ENV"] === "development") {
    console.warn("Redis client does not expose connect(), skipping eager connection fallback.");
  }
}
