...
export function createFileRoutes(db: Pool) {
  ...
  // Upload new file
  router.post("/upload", uploadConfig.businessFiles.single("file"), async (req, res) => {
    try {
      if (!req.file) {
  ...
  router.post("/presigned-upload", async (req, res) => {
    try {
      ...
      const presignedUrl = await fileStorage.getPresignedUploadUrl(fileName, mimeType, userId, {
        folder,
        isPublic,
      });
  ...
  router.get("/:fileId/versions/:version", async (req, res) => {
    try {
      ...
      const versionRecord = await versioningService.getFileVersion(fileId, versionNumber);
      if (!versionRecord) {
  ...
  router.post("/:fileId/versions/:version/rollback", async (req, res) => {
    try {
      ...
      const newVersion = await versioningService.rollbackToVersion(fileId, versionNumber, userId);
  ...
  router.delete("/:fileId/versions/:version", async (req, res) => {
    try {
      ...
      const fileResult = await db.query("SELECT uploaded_by FROM files WHERE id = $1", [fileId]);
  ...
  router.post("/folders", async (req, res) => {
    try {
      ...
      const folderId = await versioningService.createFolder(name, userId, parentId);
