...
export class FileStorageService {
  ...
  async uploadFile(
    ...
    const key = config.folder ? `${config.folder}/${fileName}` : `uploads/${userId}/${fileName}`;
  ...
  async getPresignedDownloadUrl(fileKey: string, expiresIn = 3600): Promise<string> {
    ...
    return getSignedUrl(this.s3Client as unknown as Parameters<typeof getSignedUrl>[0], command, {
      expiresIn,
    });
  }
  ...
  async createFolder(folderName: string, userId: string, parentFolder?: string): Promise<string> {
