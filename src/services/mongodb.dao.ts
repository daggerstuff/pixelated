...
export class DataExportDAO {
  ...
  async update(id: string, updates: Partial<DataExport>): Promise<DataExport | null> {
    const collection = await this.getCollection();
...
export class TodoDAO {
  ...
  async create(todo: Omit<Todo, "_id" | "createdAt" | "updatedAt">): Promise<Todo> {
    const collection = await this.getCollection();
...
export class AIMetricsDAO {
  private async getCollection(): Promise<MongoCollection<AIMetrics>> {
    ...
    await initializeDependencies();
    mongoLogger.info(
      "Dependencies initialized. MongoDB client status: " + (mongodb ? "defined" : "undefined"),
    );
    if (!mongodb) {
...
export class TreatmentPlanDAO {
  ...
  async findByClientId(clientId: string, filters?: { status?: string }): Promise<TreatmentPlan[]> {
    const collection = await this.getCollection();
    ...
    const plans = await collection.find(filter).sort({ createdAt: -1 }).toArray();
  ...
  async findByUserId(userId: string): Promise<TreatmentPlan[]> {
    const collection = await this.getCollection();
    const plans = await collection.find({ clientId: userId }).sort({ createdAt: -1 }).toArray();

    return plans.map((plan) => ({ ...plan, id: plan._id?.toString() }));
  }

  async update(id: string, updates: Partial<TreatmentPlan>): Promise<TreatmentPlan | null> {
    const collection = await this.getCollection();
    const { _id, id: _, ...safeUpdates } = updates;
  ...
  async updateGoal(
    ...
    goalId: string,
    goalUpdates: Partial<
      Omit<import("../types/mongodb.types").TreatmentPlanGoal, "id" | "milestones" | "metrics">
    > & {
  ...
  async updateMilestone(
    ...
    goalId: string,
    milestoneId: string,
    milestoneUpdates: Partial<Omit<import("../types/mongodb.types").TreatmentPlanMilestone, "id">>,
  ): Promise<TreatmentPlan | null> {
    const collection = await this.getCollection();
    const plan = await collection.findOne({ _id: new ObjectId!(planId) });
...
export class CrisisSessionFlagDAO {
  ...
  async create(flag: Omit<CrisisSessionFlag, "_id" | "createdAt">): Promise<CrisisSessionFlag> {
    const collection = await this.getCollection();
  ...
  async findActiveFlags(userId?: string): Promise<CrisisSessionFlag[]> {
    ...
    const flags = await collection.find(filter).sort({ createdAt: -1 }).toArray();

    return flags.map((flag) => ({ ...flag, id: flag._id?.toString() }));
  }

  async resolveFlag(id: string, resolvedBy: string): Promise<CrisisSessionFlag | null> {
    const collection = await this.getCollection();
...
export class ConsentManagementDAO {
  ...
  async create(consent: Omit<ConsentManagement, "_id">): Promise<ConsentManagement> {
    const collection = await this.getCollection();
...
export class AgentActivityDAO {
  ...
  async saveActivities(record: AgentActivityRecord): Promise<void> {
    const collection = await this.getCollection();
    await collection.updateOne({ turnId: record.turnId }, { $set: record }, { upsert: true });
  }
