...
describe('scenarios utility functions', () => {
  describe('getRecommendedScenario', () => {
    it('returns null if all available scenarios are completed', async () => {
      // Fetch all available scenarios dynamically so the test is robust against data changes in the actual implementation.
    ...
    it('returns any available scenario if no matching difficulty scenario is available', async () => {
      const allScenarios = await getScenarios()
      const nonIntermediateScenarios = allScenarios.filter((s) => s.difficulty !== ScenarioDifficulty.INTERMEDIATE)
      // Add an explicit assertion to verify that at least one non-intermediate scenario exists
      expect(nonIntermediateScenarios.length).toBeGreaterThan(0)
      if (nonIntermediateScenarios.length === 0) {
        throw new Error('No non-intermediate scenarios found. Cannot run this test.')
      }
