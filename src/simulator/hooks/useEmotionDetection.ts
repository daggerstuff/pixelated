...
/**
 * Provides a helper to detect emotional states from text input.
 *
 * Uses the PAD (Valence-Arousal-Dominance) emotional state model to map
 * categorical emotions from the AI provider into continuous dimensional
 * coordinates (valence, arousal, dominance). Note: the simulator codebase
 * refers to arousal as "energy" in the state (e.g., updateEmotionState uses
 * an `energy` property) for consistency with the EmotionDimensions type.
 * Can be called repeatedly for near-real-time emotion tracking.
 */
  ...
  const detectEmotions = useCallback(
    async (text: string): Promise<EmotionAnalysis | null> => {
      try {
        ...
        // Convert emotions object to array for processing
        const emotionsArray = Object.values(analysis.emotions)
        ...
        if (emotionsArray.length > 0) {
          ...
          const valence = emotionsArray.reduce((sum, emotion) => {
          ...
          }, 0) / emotionsArray.length

          // Calculate energy/arousal level
          const energy = emotionsArray.reduce((sum, emotion) => {
          ...
          // Calculate dominance
          const dominance = emotionsArray.reduce((sum, emotion) => {
