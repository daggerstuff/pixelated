...
const DemographicBalancingDisplay: FC<DemographicBalancingDisplayProps> = ({
  ...
  useEffect(() => {
    const stats: DemographicData[] = [
      ...
      ...Object.entries(demographicTargets.age).map(([category, target]) => ({
        ...
        target,
        percentage: (currentStats.age[category as keyof typeof currentStats.age] / target) * 100,
      })),
      // Gender demographics
      ...Object.entries(demographicTargets.gender).map(([category, target]) => ({
        category: "Gender",
        subcategory: category,
        current: currentStats.gender[category as keyof typeof currentStats.gender],
        target,
        percentage:
          (currentStats.gender[category as keyof typeof currentStats.gender] / target) * 100,
      })),
      // Occupation demographics
      ...Object.entries(demographicTargets.occupation).map(([category, target]) => ({
        category: "Occupation",
        subcategory: category,
        current: currentStats.occupation[category as keyof typeof currentStats.occupation],
        target,
        percentage:
          (currentStats.occupation[category as keyof typeof currentStats.occupation] / target) *
          100,
      })),
      // Background demographics
      ...Object.entries(demographicTargets.background).map(([category, target]) => ({
        category: "Background",
        subcategory: category,
        current: currentStats.background[category as keyof typeof currentStats.background],
        target,
        percentage:
          (currentStats.background[category as keyof typeof currentStats.background] / target) *
          100,
      })),
    ];
  ...
  const getCurrentProfileHighlight = (category: string, subcategory: string) => {
  ...
  const groupedStats = demographicStats.reduce<Record<string, DemographicData[]>>((acc, stat) => {
    const { category } = stat;
    acc[category] ??= [];
  ...
  return (
    <div className="bg-gray-100 rounded-lg p-4">
      <div className="mb-4 flex items-center justify-between">
        <h4 className="text-gray-800 text-xl font-bold">Dataset Demographic Balance</h4>
        <div className="flex items-center gap-2">
          <span className="text-gray-600 text-sm font-medium">Overall Balance:</span>
          <span
      ...
      <div className="bg-neutral-100 border-neutral-300 mb-6 rounded-lg border-l-4 p-4">
        <h5 className="text-neutral-800 mb-2 font-medium">Current Profile Classification</h5>
        <div className="text-neutral-700 grid grid-cols-2 gap-4 text-sm">
          <div>
          ...
          <div>
            <strong>Occupation Type:</strong> {getOccupationCategory(currentProfile.occupation)}
          </div>
          <div>
            <strong>Background:</strong> {getBackgroundCategory(currentProfile.background)}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {Object.entries(groupedStats).map(([category, stats]) => (
          <div key={category} className="rounded-lg border p-4">
            <h5 className="text-gray-700 mb-3 font-medium">{category} Distribution</h5>
            <div className="space-y-3">
              {stats.map((stat) => {
                const isCurrentProfile = getCurrentProfileHighlight(category, stat.subcategory);
                return (
                  <div
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {stat.subcategory}
                        {isCurrentProfile && <span className="ml-1 text-xs">(Current)</span>}
                      </span>
                    </div>
      ...
      <div className="bg-indigo-50 mt-6 rounded-lg p-4">
        <h5 className="text-indigo-800 mb-3 font-medium">Balance Recommendations</h5>
        <div className="text-indigo-700 space-y-2 text-sm">
          ...
          {demographicStats.filter((stat) => stat.percentage < 75 || stat.percentage > 125)
            .length === 0 && (
            <p className="text-indigo-600">✓ All demographic categories are well-balanced</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-gray-50 mt-4 rounded-lg p-3">
        <h6 className="text-gray-700 mb-2 text-sm font-medium">Balance Indicators</h6>
