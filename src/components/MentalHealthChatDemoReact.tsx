...
interface EnhancedMentalHealthAnalysis extends Omit<MentalHealthAnalysis, 'scores' | 'summary' | 'riskLevel'> {
...
export default function MentalHealthChatDemoReact({
  ...
  const [mentalHealthChat, setMentalHealthChat] = useState<ReturnType<typeof createMentalHealthChat> | null>(null);
    ...
    return messages
      ...
      .map((m) => {
        ...
        const enhancedAnalysis: EnhancedMentalHealthAnalysis = {
          ...
          expertExplanation: analysis.expertGuided ? analysis.explanation : undefined,
        };
      ...
      if (processedMessage.mentalHealthAnalysis) {
        setMessages((prev: ChatMessage[]) =>
          prev.map((m: ChatMessage) =>
            m.id === userMessage.id
              ? { ...m, mentalHealthAnalysis: processedMessage.mentalHealthAnalysis || undefined }
              : m
          )
        );
      }
    ...
    <div className="flex w-full flex-col gap-4 md:flex-row">
      ...
      {settings.showAnalysisPanel && (
        <div className="flex-1">
          <Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="h-[600px]">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="analysis">Analysis</TabsTrigger>
              {showSettingsPanel && <TabsTrigger value="settings">Settings</TabsTrigger>}
            </TabsList>
            <TabsContent value="analysis" className="h-[calc(100%-45px)] overflow-hidden">
              <Card className="h-full">
                <CardContent className="h-full overflow-auto p-4">
                  <div className="space-y-4">
                    <h3 className="text-lg font-medium">Mental Health Insights</h3>
                    ...
                    <div className="mt-6 h-[200px]">
                      <h3 className="mb-2 text-lg font-medium">Pattern Analysis</h3>
                      <MentalHealthHistoryChart analysisHistory={analysisHistory} />
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            {showSettingsPanel && (
              <TabsContent value="settings" className="h-[calc(100%-45px)] overflow-hidden">
                <Card className="h-full">
                  <CardContent className="h-full overflow-auto p-4">
                    <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <div>
                            <Label htmlFor="enable-analysis" className="text-base font-medium">
                              Enable Analysis
                            </Label>
                            <p className="text-muted-foreground text-sm">Analyze messages for mental health indicators</p>
                          </div>
                          <Switch
                            id="enable-analysis"
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            <Label htmlFor="expert-guidance" className="text-base font-medium">
                              Expert Guidance
                            </Label>
                            <p className="text-muted-foreground text-sm">Use clinician-validated interpretation</p>
                          </div>
                          <Switch
                            id="expert-guidance"
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            <Label htmlFor="show-analysis" className="text-base font-medium">
                              Show Analysis Panel
                            </Label>
                            <p className="text-muted-foreground text-sm">Display the analysis sidebar</p>
                          </div>
                          <Switch
                            id="show-analysis"
