...
async function runDeviceChecks(): Promise<DeviceCheckResult> {
  ...
  if (microphoneAvailable) {
    ...
    errors.push('No microphone device found. Connect a microphone to join the session.');
  }
...
export function PreCallCheck({ onComplete, onCancel }: PreCallCheckProps) {
  ...
  return (
    <div
      className="flex min-h-[400px] flex-col items-center justify-center gap-6 rounded-lg border border-gray-200 bg-white p-8 shadow-sm"
      role="dialog"
      aria-label="Pre-call device check"
    >
      <div className="text-center">
        <h2 className="text-xl font-semibold text-gray-900">Device Check</h2>
        ...
        <>
          <div className="w-full max-w-md space-y-3">
            ...
            <div className="flex items-center justify-between rounded-md border px-4 py-3">
              ...
              >
                {state.result.microphoneAvailable ? '✓ Available' : '✗ Unavailable'}
              </span>
            </div>
          </div>
          ...
          {state.result.errors.length > 0 && (
            <div
              className="w-full max-w-md rounded-md border border-amber-200 bg-amber-50 p-4"
              role="alert"
            >
