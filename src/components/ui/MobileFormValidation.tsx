...
export function MobileFormValidation({
  ...
  const validateForm = (): Record<string, string> => {
    ...
    inputs.forEach((input) => {
      ...
      const { value } = input as HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement;
      const error = validateField(name, value);
  ...
  const enhancedForm = React.Children.map(children, (child) => {
    if (React.isValidElement(child) && child.type === "form") {
      ...
      type FormSubmitEvent =
        NonNullable<React.ComponentProps<"form">["onSubmit"]> extends (event: infer T) => unknown
          ? T
          : never;
  ...
  return (
    <>
      ...
      <div id="validation-error-summary" className="sr-only" aria-live="assertive"></div>
...
export const ValidationRules = {
  ...
  match: (fieldName: string, message: string): ValidationRule => ({
    test: (value) => {
      const matchField = document.querySelector(`[name="${fieldName}"]`) as HTMLInputElement | null;
