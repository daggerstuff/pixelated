...
): child is React.ReactElement<React.FormHTMLAttributes<HTMLFormElement>, "form"> {
...
export function MobileFormValidation({
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      if (!isValid) {
        ...
        if (focusFirstInvalidField && formRef.current) {
          ...
          if (firstErrorName) {
            const firstErrorField = formRef.current.querySelector(`[name="${firstErrorName}"]`);
    ...
    [validateOnSubmit, validateForm, focusFirstInvalidField, isMobile, onValidationChange],
  );
  ...
  const enhancedForm = React.Children.map(children, (child): React.ReactNode => {
  ...
  return (
    <>
      ...
      <div id="validation-error-summary" className="sr-only" aria-live="assertive" />
...
export const ValidationRules = {
  ...
  match: (fieldName: string, message: string): ValidationRule => ({
    test: (value) => {
      const matchField = document.querySelector<HTMLInputElement>(`[name="${fieldName}"]`);
