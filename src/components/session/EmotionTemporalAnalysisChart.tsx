...
const readTooltipField = (
  ...
  return Object.entries(payload).reduce<unknown>((found, [entryKey, entryValue]) => {
    if (found !== undefined) {
      return found
    }
...
export default function EmotionTemporalAnalysisChart({
  ...
  const prepareProgressionData = () => {
    ...
    if (data.progression && typeof data.progression === 'object') {
      ...
      if (
        ...
        progression.positiveEmotionChange = data.progression.positiveEmotionChange
      }
      if (
        'negativeEmotionChange' in data.progression &&
        typeof data.progression.negativeEmotionChange === 'number'
      ) {
        progression.negativeEmotionChange = data.progression.negativeEmotionChange
      }
    }
    ...
    return [
      ...
      },
      {
        ...
        fill:
          progression.stabilityChange >= 0 ? '#3b82f6' : '#f97316',
      },
      {
  ...
  const prepareTransitionsData = () => {
    ...
    return (data.transitions ?? []).slice(0, 10).map((transition: TemporalTransition) => ({
