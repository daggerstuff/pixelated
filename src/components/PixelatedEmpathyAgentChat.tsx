...
const isTherapeuticScenario = (value: unknown): value is TherapeuticScenario => {
  if (!isRecord(value)) {
    return false;
  }
...
const isBiasAnalysis = (value: unknown): value is BiasAnalysis => {
  ...
  return value["detected_biases"].every((item) => {
    ...
    return isBiasSeverity(item["severity"]) && biasSeverityValues.includes(item["severity"]);
  });
};
...
export const PixelatedEmpathyAgentChat: FC<AgentChatProps> = ({
  ...
  const handleResponseMetadata = (response: AgentResponse, currentContext: AgentContext): void => {
    if (
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (response.success && response.response) {
        ...
        const errorMessage = createErrorMessage(`Sorry, I encountered an error: ${response.error}`);
  ...
  const quickActions: {
    ...
    },
    {
      ...
      action: () => setInput("Provide training recommendations based on my performance"),
    ...
    >
      ...
      <div className="border-gray-200 bg-blue-50 flex items-center justify-between border-b p-4">
        <div className="flex items-center space-x-3">
          ...
          <h3 className="text-gray-800 text-lg font-semibold">Pixelated Empathy AI Assistant</h3>
        </div>
        <select
          value={context}
          onChange={(e) => {
...
function extractPopulation(
  input: string,
): "adolescent" | "adult" | "geriatric" | "cultural_minority" | "lgbtq" | "veteran" | undefined {
  const lower = input.toLowerCase();
