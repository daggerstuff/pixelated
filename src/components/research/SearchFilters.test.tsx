...
describe("SearchFilters", () => {
  ...
  async function renderFilters(onChange = vi.fn(), onClose = vi.fn()) {
    ...
    act(() => {
      root.render(<SearchFilters filters={defaultFilters} onChange={onChange} onClose={onClose} />);
    });
  ...
  it("reflects active sort state", async () => {
    ...
    try {
      const relevanceOption = Array.from(container.querySelectorAll("option")).find((option) =>
        option.textContent?.includes("Relevance"),
      );
