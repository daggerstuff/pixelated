...
vi.mock("recharts", () => {
  const MockChart = ({ children, ...props }: { children?: React.ReactNode }) => (
...
describe("TherapyProgressCharts", () => {
  ...
  it("renders chart container with therapy-progress-charts class", () => {
    render(<TherapyProgressCharts data={mockData} />);
    const container = screen.getByTestId("mock-chart").closest(".therapy-progress-charts");
    expect(container).toBeInTheDocument();
  });

  it("shows fallback message when data is null", () => {
    render(<TherapyProgressCharts data={null} />);
    expect(screen.getByText("No therapy progress data available")).toBeInTheDocument();
  });
