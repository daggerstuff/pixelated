...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    const response = {
      data: dimensionalData.map((item: DimensionalEmotion) => ({
        ...
        mappedEmotion: item.valence >= 0 ? "positive" : "negative",
      })),
      meta: {
    ...
    return new Response(JSON.stringify({ error: "Failed to fetch emotion data" }), {
      status: 500,
      headers: {
        "Content-Type": "application/json",
