...
export const GET = withV1Contract('listPatients', async (ctx, caller) => {
  ...
  const limitNum = sanitizeLimitParam(limit ? parseInt(limit, 10) : 50)
  const offsetNum = sanitizeOffsetParam(offset ? parseInt(offset, 10) : 0)
  const sanitizedQuery = searchQuery
    ? sanitizeSearchParam(searchQuery)
    : undefined
