...
export const GET = withV1Contract('getClaim', async (ctx, caller) => {
  ...
  if (!claimJson) {
    return ehrValidationError(
      'Pass the claim object as a `claim` query parameter (URL-encoded JSON) for stateless validation.',
    )
  }
  ...
  try {
    ...
    return ehrValidationError(
      err instanceof Error ? err.message : 'Invalid claim JSON.',
    )
  }
})
...
export const PATCH = withV1Contract(
  ...
  async (ctx, caller) => {
    ...
    if (!claim || !newStatus)
      return ehrValidationError('Body must include `claim` and `newStatus`.')
    ...
    try {
      ...
      return ehrValidationError(
        err instanceof Error ? err.message : 'Claim status update failed.',
      )
    }
  },
)
