...
import { SchedulingService, type UpdateAppointmentInput } from '@/lib/ehr-native/services'
 ...
 */
export const PATCH = withV1Contract('updateAppointment', async (ctx, caller) => {
  const appointmentId = ctx.params?.['id']
  if (!appointmentId) return ehrValidationError('Appointment ID is required.')
  const tenantId = resolveTenantId(caller.user.accountId)
  if (!tenantId) return ehrValidationError('Tenant association required for EHR access.')
  ...
  try {
    ...
    switch (action) {
      case 'cancel':
        appointment = await service.cancelAppointment(appointmentId, body['reason'] as string | undefined)
        break
      case 'reschedule': {
        const start = body['start'] as string | undefined
        const end = body['end'] as string | undefined
        if (!start || !end) return ehrValidationError('Reschedule requires start and end.')
        appointment = await service.rescheduleAppointment(appointmentId, start, end)
        break
      }
      case 'check-in':
      ...
      case 'no-show':
        ...
        break
      default:
        appointment = await service.updateAppointment(appointmentId, raw as UpdateAppointmentInput)
    }
    if (!appointment) return ehrNotFound('Appointment', appointmentId)
    return ehrSuccess(appointment)
  } catch (err) {
    return ehrValidationError(err instanceof Error ? err.message : 'Appointment update failed.')
  }
})
