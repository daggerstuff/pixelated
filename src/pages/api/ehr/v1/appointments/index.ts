...
export const GET = withV1Contract('listAppointments', async (ctx, caller) => {
  ...
  let appointments
  if (patientId) {
    appointments = await service.getPatientAppointments(patientId, {
      limit,
      offset,
    })
  } else if (practitioner) {
    appointments = await service.getPractitionerSchedule(practitioner, {
      upcomingOnly: upcoming,
      limit,
      offset,
    })
  } else if (status) {
    appointments = await service.getAppointmentsByStatus(status, {
      limit,
      offset,
    })
  } else if (startDate && endDate) {
    appointments = await service.getAppointmentsByDateRange({
      start: startDate,
      end: endDate,
      limit,
      offset,
    ...
    return ehrValidationError(
      'Provide at least one filter: patient, practitioner, status, or start+end.',
    )
  }
  return ehrPaginated(appointments, {
    limit,
    offset,
...
export const POST = withV1Contract('createAppointment', async (ctx, caller) => {
  ...
  try {
    const appointment = await service.createAppointment(
      raw as CreateAppointmentInput,
    )
    return ehrCreated(appointment)
  } catch (err) {
    return ehrValidationError(
      err instanceof Error ? err.message : 'Invalid appointment input.',
    )
  }
})
