...
export const GET = withV1Contract('verifyConsent', async (ctx, caller) => {
  ...
  const perm = await requireEHRPermission(caller.user.role, 'manage_consent', caller.user.id, tenantId)
  ...
  if (!patientId) return ehrValidationError('patientId query parameter is required.')
...
export const POST = withV1Contract('checkConsent', async (ctx, caller) => {
  ...
  if (!patientId || !minimumLevel) return ehrValidationError('patientId and minimumLevel are required.')
