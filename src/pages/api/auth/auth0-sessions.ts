...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    if (!token) {
      return new Response(JSON.stringify({ error: ERROR_MESSAGES.AUTHENTICATION_REQUIRED }), {
    ...
    if (startDateParam) {
      ...
      if (isNaN(startDate.getTime())) {
        return new Response(JSON.stringify({ error: ERROR_MESSAGES.INVALID_START_DATE }), {
    ...
    if (endDateParam) {
      ...
      if (isNaN(endDate.getTime())) {
        return new Response(JSON.stringify({ error: ERROR_MESSAGES.INVALID_END_DATE }), {
          status: HTTP_STATUS.BAD_REQUEST,
          headers: { "Content-Type": "application/json" },
        });
      }
    ...
    if (isNaN(limit) || limit <= 0) {
      return new Response(JSON.stringify({ error: ERROR_MESSAGES.INVALID_LIMIT }), {
        status: HTTP_STATUS.BAD_REQUEST,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (clientId) {
      ...
      if (user.role !== "admin" && user.id !== clientId) {
        ...
        const isTherapist = await repository.isTherapistForClient(user.id, clientId);
        if (!isTherapist) {
          ...
          return new Response(JSON.stringify({ error: ERROR_MESSAGES.CLIENT_ACCESS_DENIED }), {
    ...
    // Get sessions
    const sessions: TherapySession[] = (await repository.getSessions(filter)) as any;

    // Apply limit
    const limitedSessions = sessions.slice(0, limit);

    // Create audit log
    await createAuditLog(AuditEventType.ACCESS, "auth.sessions.access", user.id, "auth-sessions", {
      action: "get_sessions",
      count: String(limitedSessions.length),
