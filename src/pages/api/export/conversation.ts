...
import { ExportFormat, ExportService } from "../../../lib/export/index.node";
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!hasAccess) {
      return new Response(JSON.stringify({ error: "Access denied to this session" }), {
        status: 403,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    await recordExportAction(session.user.id, sessionId, exportResult.id, exportFormat);
 ...
 */
async function checkSessionAccess(userId: string, sessionId: string): Promise<boolean> {
...
function mapExportFormat(format: string): ExportFormat {
  switch (format.toLowerCase()) {
    ...
    case "json":
    default:
      return ExportFormat.JSON_FORMAT;
  }
}
