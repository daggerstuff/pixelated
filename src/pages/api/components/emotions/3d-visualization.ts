...
export const POST: APIRoute = protectRoute()(
  async (context: AuthAPIContext) => {
    try {
      ...
      const { emotion, valence, arousal, dominance, intensity, sessionId } =
        body;
...
function calculateEmotionalTrajectory(emotionPoints: EmotionPoint3D[]) {
  ...
  const valenceVariance = calculateVariance(
    emotionPoints.map((p) => p.valence),
  );
  const arousalVariance = calculateVariance(
    emotionPoints.map((p) => p.arousal),
  );
