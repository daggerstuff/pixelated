...
async function runRecoveryTest(config: unknown): Promise<RecoveryTestResult> {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? String(error) : "Unknown error";
...
export const POST = async (context: APIContext) => {
  ...
  const next = async () =>
    new Promise<Response>((resolve) => resolve(new Response(null, { status: 200 })));
  const middlewareResponse = await (adminGuard as any)("MANAGE_SECURITY")(context, next);
