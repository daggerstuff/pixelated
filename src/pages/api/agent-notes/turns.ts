...
import { PersistentTurnLedger, type NoteTurn } from "../../../lib/agent-note-collab";
import {
...
export const POST: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (submitPayload.replyTo) {
      ...
      if (!parentTurn) {
        return createErrorResponse(404, "PARENT_NOT_FOUND", "replyTo turn was not found.");
      }
...
function sortTurnsByUpdatedAt(turns: NoteTurn[], sort: "asc" | "desc" = "desc") {
  ...
  return [...turns].sort(
    (left, right) => (Date.parse(left.updatedAt) - Date.parse(right.updatedAt)) * direction,
  );
}
