...
function resolveSafeLlmBaseUrl(): string | undefined {
  const providerBaseUrl = LLM_PROVIDER_BASE_URLS.map((key) => getEnvValue(key)).find(Boolean);
...
export const POST: APIRoute = async ({ request }: APIContext) => {
  ...
  try {
    ...
    const llmApiKey = resolveProviderApiKey();
    if (!llmApiKey) {
    ...
    // Inject reverie prompt as system-level behavioral modifier
    const messagesWithReverie = data?.reveriePrompt
      ? [{ role: "system" as const, content: data.reveriePrompt }, ...formattedMessages]
      : formattedMessages;
 ...
 */
function mergeHeaders(target: Headers, source?: Headers | Record<string, unknown>): void {
  if (!source) return;

  if (source instanceof Headers) {
    source.forEach((value, key) => target.append(key, value));
  } else {
    Object.entries(source).forEach(([key, value]) => target.append(key, String(value)));
  }
}
...
async function performRequestPrechecks(request: Request): Promise<{
  ...
  const rateLimit = await applyRateLimit(request, "/api/ai/completion", RATE_LIMIT_CONFIG);
