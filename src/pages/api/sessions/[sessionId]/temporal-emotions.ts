  ...
  async ({
    ...
    try {
      ...
      const includePatterns =
        url.searchParams.get("includePatterns") === "true";
      const timeWindow = parseInt(
        url.searchParams.get("timeWindow") || "90",
        10,
      );
      const emotionTypes =
        url.searchParams.get("emotionTypes")?.split(",") || [];
      ...
      switch (analysisType) {
        case "trends": {
          ...
          const sessions = await analyzer.analyzeSessionEmotions([sessionId], {
            ...
            config: {
              ...
              includeDimensionalAnalysis: false,
            },
          });
        ...
        case "transitions": {
          ...
          const transitions = await analyzer.analyzeSessionEmotions(
            [sessionId],
            {
              timeRange: { startDate, endDate },
              config: {
        ...
        case "relationships": {
          ...
          const relationships =
            await analyzer.findEmotionCorrelations(clientId);
      ...
      return new Response(
        JSON.stringify({
          error: "Failed to retrieve temporal emotion analysis",
        }),
