...
predicate hasValidation(CallExpr call) {
  exists(CallExpr validateCall |
    ...
    (
      validateCall.getAnArgument().flowsTo(call.getAnArgument()) or
      call.getAnArgument().flowsTo(validateCall.getAnArgument()) or
      exists(Expr e | validateCall.getAnArgument() = e and call.getAnArgument() = e)
    )
  )
}
