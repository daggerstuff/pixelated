...
predicate hasInputSanitization(CallExpr call) {
  exists(CallExpr sanitizeCall |
    ...
    (
      sanitizeCall.getAnArgument().flowsTo(call.getAnArgument()) or
      call.getAnArgument().flowsTo(sanitizeCall.getAnArgument()) or
      exists(Expr e | sanitizeCall.getAnArgument() = e and call.getAnArgument() = e)
    )
  )
}
