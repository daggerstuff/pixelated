...
predicate isEHROperation(CallExpr ehrOp) {
  exists(string name |
    name = ehrOp.getCalleeName() and
    (
...
predicate hasLocalLogging(CallExpr ehrOp) {
  exists(CallExpr logCall |
    (
      logCall.getCalleeName().matches("%log%") or
      logCall.getCalleeName().matches("%audit%")
    ) and
    logCall.getEnclosingFunction() = ehrOp.getEnclosingFunction()
  )
}
