...
module InsecureEHRFlow = TaintTracking::Global<InsecureEHRConfigSig>;

module UnsafeEHRAccessConfigSig implements DataFlow::ConfigSig {
  predicate isSource(DataFlow::Node source) {
    source instanceof DataFlow::Node and source.asExpr() instanceof Identifier
  }
  ...
  predicate isSink(DataFlow::Node sink) {
    exists(DataFlow::CallNode call |
      ...
      any(EHREndpoint endpoint).getASuccessor*() = call.getAnArgument()
    )
  }
}

module UnsafeEHRAccessFlow = TaintTracking::Global<UnsafeEHRAccessConfigSig>;
