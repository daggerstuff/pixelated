...
module InsecureEHRConfigSig implements DataFlow::ConfigSig {
  predicate isSource(DataFlow::Node source) { source instanceof EHRCredentialSource }
  ...
  predicate isSink(DataFlow::Node sink) {
    exists(DataFlow::CallNode call |
      ...
      exists(EHREndpoint endpoint |
        DataFlow::localFlow(endpoint, call.getAnArgument())
      )
    )
  }
}

module UnsafeEHRAccessFlow = TaintTracking::Global<UnsafeEHRAccessSig>;
