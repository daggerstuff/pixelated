...
Build an interactive CLI tool that visualizes pull request age distribution for
a GitHub repository using Copilot's built-in capabilities.
...
You want to understand how long PRs have been open in a repository. This tool
detects the current Git repo or accepts a repo as input, then lets Copilot fetch
PR data via the GitHub MCP Server and generate a chart image.
...
2. **No custom tools**: Relies entirely on Copilot CLI's built-in capabilities:
   ...
   - **Code execution** - Generates charts using Python/matplotlib or other
     methods
