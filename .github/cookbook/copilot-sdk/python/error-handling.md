...
You need to handle various error conditions like connection failures, timeouts,
...
1. **Always clean up**: Use try-finally to ensure `await client.stop()` is
   called
2. **Handle connection errors**: The CLI might not be installed or running
3. **Set appropriate timeouts**: Use the `timeout` parameter on
   `send_and_wait()`
