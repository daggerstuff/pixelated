...
activity. You WILL use #file:./task-researcher.agent.md when research is missing
or incomplete.
...
3. **If research missing/incomplete**: You WILL IMMEDIATELY use
   #file:./task-researcher.agent.md
4. **If research needs updates**: You WILL use #file:./task-researcher.agent.md
   for refinement
...
you WILL update the research file first using #file:./task-researcher.agent.md ,
then update all dependent planning files.
...
- [ ] Task 1.1: {{specific_action_1_1}}
  - Details: .copilot-tracking/details/{{date}}-{{task_description}}-details.md
...
- **Research References**:
  - #file:../research/{{date}}-{{task_description}}-research.md (Lines
    {{research_line_start}}-{{research_line_end}}) -
...
You WILL create `{{date}}-{{task_description}}-changes.md` in #file:../changes/
if it does not exist.

### Step 2: Execute Implementation

You WILL follow
#file:../../.github/instructions/task-implementation.instructions.md You WILL
systematically implement
#file:../plans/{{date}}-{{task_description}}-plan.instructions.md task-by-task
You WILL follow ALL project standards and conventions

**CRITICAL**: If ${input:phaseStop:true} is true, you WILL stop after each Phase
for user review. **CRITICAL**: If ${input:taskStop:false} is true, you WILL stop
after each Task for user review.
...
1. You WILL provide a markdown style link and a summary of all changes from
   #file:../changes/{{date}}-{{task_description}}-changes.md to the user:
...
2. You WILL provide markdown style links to
   ...
   .copilot-tracking/details/{{date}}-{{task_description}}-details.md, and
...
3. **MANDATORY**: You WILL attempt to delete
   .copilot-tracking/prompts/{{implement_task_description}}.prompt.md
...
3. **If research missing/incomplete**: You WILL use
   #file:./task-researcher.agent.md immediately
4. **If research needs updates**: You WILL use #file:./task-researcher.agent.md
   for refinement
...
2. You WILL create plan, details, and prompt files using validated research
   findings
...
4. If content no longer exists, you WILL use #file:./task-researcher.agent.md to
   update research
...
- **If research missing**: You WILL use #file:./task-researcher.agent.md
  immediately
