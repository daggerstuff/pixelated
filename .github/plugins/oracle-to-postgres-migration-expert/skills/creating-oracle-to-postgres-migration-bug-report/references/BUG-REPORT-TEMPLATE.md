...
Use this template when creating bug reports for Oracle-to-PostgreSQL migration
defects.
...
**Status:** ✅ RESOLVED | ⛔ UNRESOLVED | ⏳ IN PROGRESS **Component:**
<High-level component/endpoint and key method(s)> **Test:**
<Related automated test names> **Severity:** Low | Medium | High | Critical

---

## Problem

<Observable incorrect behavior. State expected behavior (Oracle baseline) versus
actual behavior (PostgreSQL). Be specific and factual.>
...
<Changes made or required. Be explicit about data access layer changes, tracking
flags, and any client code modifications. Note whether changes are already
...
- `src/DataAccess/FooRepository.cs` — Added explicit NULL check for empty string
  parameter>
