...
Embed Copilot's agentic workflows in any application using Python, TypeScript,
Go, or .NET.

## Overview

The GitHub Copilot SDK exposes the same engine behind Copilot CLI: a
production-tested agent runtime you can invoke programmatically. No need to
build your own orchestration - you define agent behavior, Copilot handles
planning, tool invocation, file edits, and more.
...
Define tools that Copilot can invoke during reasoning. When you define a tool,
you tell Copilot:
...
Copilot decides when to call your tool based on the user's question and your
tool's description.
...
Connect to MCP (Model Context Protocol) servers for pre-built tools. Connect to
GitHub's MCP server for repository, issue, and PR access:
...
Run the CLI in server mode separately and connect the SDK to it. Useful for
debugging, resource sharing, or custom environments.
...
**Note:** When `cliUrl` is provided, the SDK will not spawn or manage a CLI
process - it only connects to the existing server.
...
1. **Always cleanup**: Use `try-finally` or `defer` to ensure `client.stop()` is
   called
...
The SDK manages the CLI process lifecycle automatically. All communication
happens via JSON-RPC over stdio or TCP.

## Resources

- **GitHub Repository**: https://github.com/github/copilot-sdk
- **Getting Started Tutorial**:
  https://github.com/github/copilot-sdk/blob/main/docs/tutorials/first-app.md
...
This SDK is in **Technical Preview** and may have breaking changes. Not
