...
Response shapes and behavioral notes for the FlowStudio Power Automate MCP
server.

> **For tool names and parameters**: Always call `tools/list` on the server. It
> returns the authoritative, up-to-date schema for every tool. This document
> covers what `tools/list` does NOT tell you: **response shapes** and
...
> If this document disagrees with `tools/list` or real API behavior, the API
> wins. Update this document accordingly.
...
> **Key field**: `id` is the `connectionName` value used in
...
> Access via `result["flows"]`. `id` is a plain UUID --- use directly as
> `flowName`.
>
...
> **`id` format**: `envId.flowId` --- split on the first `.` to extract the flow
> UUID: `flow_id = item["id"].split(".", 1)[1]`
...
> `runPeriodDurationAverage` / `runPeriodDurationMax` are in **milliseconds**
> (divide by 1000). `runError` is a **JSON string** --- parse with
> `json.loads()`.
...
**Create mode**: Omit `flowName` --- creates a new flow. `definition` and
...
> `error` is **always present** but may be `null`. Check
> `result.get("error") is not None`.
>
> On create: `created` is the new flow GUID (string). On update: `created` is
> `false`.
>
...
Migrates a non-solution flow into a solution. Returns error if already in a
solution.
...
> `failedActions` is ordered outer-to-inner --- the **last entry is the root
> cause**. Use `failedActions[-1]["actionName"]` as the starting point for
> diagnosis.
...
`flowKey`, `triggerName`, `triggerType`, `triggerKind`, `triggerMethod`,
`triggerUrl`.

### `trigger_live_flow`

Response keys: `flowKey`, `triggerName`, `triggerUrl`, `requiresAadAuth`,
`authType`, `responseStatus`, `responseBody`.
...
Update governance metadata (description, tags, monitor flag, notification rules,
business impact).
...
- `error` key is **always present** in response --- `null` means success. Do NOT
  check `if "error" in result`; check `result.get("error") is not None`.
- On create, `created` = new flow GUID (string). On update, `created` = `false`.

### `trigger_live_flow`

- **Only works for HTTP Request triggers.** Returns error for Recurrence,
  connector, and other trigger types.
- AAD-authenticated triggers are handled automatically (impersonated Bearer
  token).

### `get_live_flow_runs`

- `top` defaults to **30** with automatic pagination for higher values.
- Run ID field is `name`, not `runName`. Use this value as `runName` in other
  tools.
- Runs are returned newest-first.

### Teams `PostMessageToConversation` (via `update_live_flow`)

- **"Chat with Flow bot"**: `body/recipient` = `"user@domain.com;"` (string with
  trailing semicolon).
- **"Channel"**: `body/recipient` = `{"groupId": "...", "channelId": "..."}`
  (object).
