...
routing when the right skill for a task is not obvious.
...
- When an agent needs to identify the best skill for an ambiguous or
  multi-domain task.
...
Foresight is the persistent memory layer for AI agents — shared across all
...
- **Tag by category** — `preference`, `decision`, `lesson`, `fact` — makes
  search precise
...
- **MCP client can't connect**: Verify port `8764` is listening
  (`ss -ltn | grep 8764`) and the client URL is `http://127.0.0.1:8764/mcp`
