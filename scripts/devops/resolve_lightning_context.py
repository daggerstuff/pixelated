...
def _has_gpu_hint(name: str) -> bool:
    ...
    return any(
        ...
        for token in ("gpu", "l4", "l40", "h100", "h200", "a100", "p4", "p5", "g4", "g6", "g7", "t4")
    )


def resolve_lightning_context(
    creds_path: str, require_studio: bool, preferred_machine: str = ""
) -> LightningStudioContext:
    ...
    auth = base64.b64encode(f"{user_id}:{api_key}".encode()).decode("utf-8")
