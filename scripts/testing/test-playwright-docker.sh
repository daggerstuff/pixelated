...
docker-compose -f infra/docker/docker-compose.playwright.yml build

# Run a simple test to verify the setup
echo "Running Playwright test verification..."
docker-compose -f infra/docker/docker-compose.playwright.yml run --rm playwright pnpm list @playwright/test
