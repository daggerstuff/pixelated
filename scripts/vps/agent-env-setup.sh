...
install_node() {
  ...
  su "$R_USER" -c \'export NVM_DIR="$HOME/.nvm"; [ -s "$NVM_DIR/nvm.sh" ] && . "$NVM_DIR/nvm.sh"; nvm install 24.16.0 2>/dev/null; nvm alias default 24.16.0 2>/dev/null; npm install -g pnpm@11.12.0 2>/dev/null\' 2>/dev/null || true
