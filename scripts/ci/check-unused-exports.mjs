 ...
 * Mirrors the logic in check-unresolved-imports.mjs.
 */
function resolveImport(importSource, fileDir) {
  ...
  for (const idx of INDEX_FILES) {
    if (pathExists(resolvedPath + idx)) return resolvedPath + idx
  }
...
function extractExports(source, cleanSource) {
  ...
  const exportDeclRe = /export\s+(?:default\s+)?(?:const|function|class|interface|type|enum|let|var)\s+(\w+)/g
  let m
  while ((m = exportDeclRe.exec(cleanSource)) !== null) {
    ...
    const kind = m[0].includes('interface') ? 'type'
      : m[0].includes('type ') ? 'type'
      : m[0].includes('enum ') ? 'enum member'
      : m[0].includes('default') ? 'export'
      : 'export'
...
async function buildIndex() {
  ...
  for (const file of files) {
    ...
    for (const imp of imports) {
      if (!isRelativePath(imp.source)) continue
...
function runKnip() {
  return new Promise((resolvePromise, reject) => {
    const chunks = []
    const proc = spawn('pnpm', ['exec', 'knip', '--no-exit-code', '--exports', '--reporter', 'json'], {
      encoding: 'buffer',
      timeout: 45_000,
...
function reportViolations(violations, isScoped, fileCount) {
  ...
  console.error(
    '\nFound ' + violations.length + ' ' + label + '. Remove them or add the --no-exit-code flag.\n' +
...
async function main() {
  ...
  const changedFiles = changedFilesIdx >= 0
    ...
    if (foundCount === 0 && changedFiles.length > 0) {
      console.warn(
        'Warning: none of the ' +
          ...
          'Files under ' + SCAN_GLOB + ' are included; other paths are skipped.',
      )
    }
