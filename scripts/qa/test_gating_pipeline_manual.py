...
from ai.memory.gates import GatingReport, GateDecision
...
def main():
    ...
    if test_gate("Clean content not blocked", report.blocked, False): passed += 1
    else: failed += 1
    if test_gate("Document ID returned", doc_id is not None, True): passed += 1
    else: failed += 1
    ...
    if test_gate("Crisis content blocked", report.blocked, True): passed += 1
    else: failed += 1
    if test_gate("No document ID on block", doc_id is None, True): passed += 1
    else: failed += 1
    ...
    if test_gate("SSN content blocked", report.blocked, True): passed += 1
    else: failed += 1
    ...
    if test_gate("Revoked consent blocks", report.blocked, True): passed += 1
    else: failed += 1

    # Test 5: GatingReport serializes
    print("\n=== Test 5: GatingReport serializes ===")
    report_dict = report.to_dict()
    if test_gate("Report has source_id", "source_id" in report_dict, True): passed += 1
    else: failed += 1
    if test_gate("Report has content", "content" in report_dict, True): passed += 1
    else: failed += 1

    # Test 6: PII redaction scrubs content
    print("\n=== Test 6: PII redaction scrubs content ===")
    result = service.pii_redactor.redact("Contact me at test@example.com please.")
    if test_gate("Email redacted", "[EMAIL]" in result.scrubbed_text, True): passed += 1
    else: failed += 1
    if test_gate("PII types found", "email" in result.pii_types_found, True): passed += 1
    else: failed += 1

    # Test 7: Crisis detector tiers
    print("\n=== Test 7: Crisis detector tiers ===")
    crisis_none = service.crisis_detector.detect("Everything is fine.")
    if test_gate("No crisis tier", crisis_none.tier.value, "none"): passed += 1
    else: failed += 1

    crisis_high = service.crisis_detector.detect("I want to kill myself.")
    if test_gate("High/critical crisis tier", crisis_high.tier.value in ("high", "critical"), True): passed += 1
    else: failed += 1

    # Test 8: Trauma filter detects lexicon
    print("\n=== Test 8: Trauma filter detects lexicon ===")
    trauma_result = service.trauma_filter.filter("I was abused and felt helpless.")
    if test_gate("Trauma indicators found", len(trauma_result.indicators) > 0, True): passed += 1
    else: failed += 1

    # Test 9: evaluate_gates returns full report
    print("\n=== Test 9: evaluate_gates returns full report ===")
    report = service.evaluate_gates(content="I feel anxious about work.", user_id="user-1")
    if test_gate("Report is GatingReport", isinstance(report, GatingReport), True): passed += 1
    else: failed += 1
    if test_gate("Gate 0 PII evaluated", report.gate0_pii is not None, True): passed += 1
    else: failed += 1
    if test_gate("Gate 1 Crisis evaluated", report.gate1_crisis is not None, True): passed += 1
    else: failed += 1
    if test_gate("Gate 2 Trauma evaluated", report.gate2_trauma is not None, True): passed += 1
    else: failed += 1
    if test_gate("Gate 3 Consent evaluated", report.gate3_consent is not None, True): passed += 1
    else: failed += 1

    # Test 10: Negation suppression in crisis detector
    print("\n=== Test 10: Negation suppression in crisis detector ===")
    negated = service.crisis_detector.detect("I would never hurt myself.")
    if test_gate("Negated crisis suppressed", negated.tier.value, "none"): passed += 1
    else: failed += 1
