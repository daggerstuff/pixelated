...
function parseEmotionJson(raw: string): EmotionSignal {
  ...
  try {
    ...
    return {
      primary_emotion:
        typeof parsed.primary_emotion === 'string'
          ? parsed.primary_emotion
          : 'neutral',
      ...
      evidence_span: (typeof parsed.evidence_span === 'string'
        ? parsed.evidence_span
        : ''
      ).slice(0, 120),
    }
  } catch {
...
function normalizedRiskFlags(
  ...
  'distress' | 'crisis_ideation' | 'harm_to_others' | 'medical_emergency'
> {
  const valid = new Set([
    'distress',
    'crisis_ideation',
  ...
  return flags.filter(
    (
      ...
      | 'crisis_ideation'
      | 'harm_to_others'
      | 'medical_emergency' => typeof f === 'string' && valid.has(f),
  )
}
