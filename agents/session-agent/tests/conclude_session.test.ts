...
describe("conclude_session", () => {
  ...
  it("should return the session_id", async () => {
    const result = await execute({ session_id: sid, exit_reason: "trainee_ended" });
    expect(result.session_id).toBe(sid);
  });

  it("should echo the exit_reason", async () => {
    const result = await execute({ session_id: sid, exit_reason: "safety_violation" });
  ...
  it("should accept all five valid exit_reason values", async () => {
    const reasons = ["trainee_ended", "auto_cap", "supervisor_closed", "safety_violation", "system_error"] as const;
