...
async function execute(input: z.infer<typeof inputSchema>) {
  return {
    ...
    foresight_stub: { memory_id: null, note: "Foresight store call is not yet wired in this slice." },
    mongo_stub: {
      collection: "sessions",
...
describe("save_session", () => {
  ...
  it("should record the transcript count accurately", async () => {
    const result = await execute({
      ...
      state: "ACTIVE",
      transcripts: [
        { role: "trainee", text: "Turn 1", timestamp: "2026-01-01T09:00:00.000Z" },
        { role: "participant", text: "Turn 2", timestamp: "2026-01-01T09:00:01.000Z" },
        { role: "trainee", text: "Turn 3", timestamp: "2026-01-01T09:00:02.000Z" },
      ],
    });
  ...
  it("should count emotion_rollups when provided", async () => {
    const result = await execute({
      ...
      transcripts: baseTranscripts,
      emotion_rollups: [
        { primary_emotion: "neutral", intensity: 0, valence: 0, risk_flags: [], timestamp: "2026-01-01T09:00:00.000Z" },
        { primary_emotion: "joy", intensity: 0.6, valence: 0.8, risk_flags: [], timestamp: "2026-01-01T09:05:00.000Z" },
      ],
    });
  ...
  it("should return all three stub objects", async () => {
    ...
    expect((result.mongo_stub as Record<string, unknown>).collection).toBe("sessions");
  });
});
