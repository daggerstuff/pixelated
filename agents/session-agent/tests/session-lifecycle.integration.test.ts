...
async function checkBoundary(input: z.infer<typeof checkBoundarySchema>) {
  ...
  if (RISK_PATTERNS[0].test(lower)) flagged.push("self_harm_ideation_with_intent");
  ...
  const severity =
    flagged.length === 0
      ? "none"
      : ["acute_medical_emergency", "self_harm_ideation_with_intent", "harm_to_others_with_intent"].some((f) =>
            flagged.includes(f),
          )
        ? "critical"
        : "warning";
  return { session_id: input.session_id, severity, flagged_risk_criteria: flagged, escalate_to_supervisor: severity === "critical" };
}
...
describe("session lifecycle integration", () => {
  ...
  it("should move through NEW → ACTIVE → CLOSED states", async () => {
    // 1. Start session
    const start = await startSession({ trainee_id: traineeId, scenario_id: scenarioId });
  ...
  it("should carry transcript turns through the session", async () => {
    ...
    expect(msg.accepted_turns.map((t: Record<string, unknown>) => t.role)).toEqual([
      "trainee",
      "participant",
      "trainee",
    ]);
  ...
  it("should flag a crisis turn and escalate to supervisor", async () => {
    ...
    expect(boundary.flagged_risk_criteria).toContain("self_harm_ideation_with_intent");
  });
  ...
  it("should accept all valid exit reasons at close", async () => {
    ...
    for (const { reason, expected } of reasons) {
      const start = await startSession({ trainee_id: traineeId, scenario_id: scenarioId });
      const end = await conclude({
        session_id: start.session_id as string,
        exit_reason: reason,
  ...
  it("should fill in timestamps on turns that omit them", async () => {
    ...
    const ts = (msg.accepted_turns[0] as Record<string, unknown>).timestamp as string;
