...
describe("start_session", () => {
  it("should generate a new session UUID when none provided", async () => {
    const result = await execute({ trainee_id: "alice", scenario_id: "scenario-1" });
  ...
  it("should echo trainee_id and scenario_id in response", async () => {
    const result = await execute({ trainee_id: "alice", scenario_id: "scenario-x" });
  ...
  it("should return foresight_stub with memory_id=null", async () => {
    ...
    expect((result.foresight_stub as Record<string, unknown>).memory_id).toBeNull();
  });
