 ...
 *
 * The client is not created until the first tool call. This keeps the
 * pipeline agent's startup context small and avoids holding an idle SSE
 * connection when the orchestrator is not actively using Foresight.
 */
...
export async function getSystemStatus(): Promise<Record<string, unknown> | null> {
  try {
    const c = await getClient();
    const result = await c.callTool({
