  ...
  async execute(input) {
    ...
    if (model) {
      try {
        ...
        preEvalScore = typeof parsed.estimated_pass_rate === "number"
    ...
    return {
      ...
      workers_ai_pre_eval: preEvalScore !== null
        ? { estimated_pass_rate: preEvalScore, model: "@cf/meta/llama-3.2-3b-instruct" }
        : null,
      benchmark_run_stub: {
        note:
