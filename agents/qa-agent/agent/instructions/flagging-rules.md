...
The QA agent promotes a session to a Linear issue when any of the following are
true after the rubric is applied:
...
2. `boundaries` below the cohort's p10 AND the supervisor-observer recorded a
   `pause_session` intervention.
...
4. The trajectory detector surfaced `persistent_distress` across the cohort
   reference window.
...
- Wire the team program leads as reviewer defaults once the cohort governance is
  finalized.
- Replace the threshold defaults with cohort-aware brackets.
- Add a `audit_trail_stub` payload in `_notes` so a reviewer can pull the full
  trajectory inline.
