...
export default defineTool({
  ...
  async execute(input: SummarizeCohortInput) {
    ...
    const scoredSessions = (memories ?? [])
      .map((m): ParsedScore | null => {
        try {
          ...
          const scoredAtMs = parsed.scored_at ? new Date(parsed.scored_at).getTime() : NaN;
    ...
    const aggregates: Record<string, { mean: number; p10: number; p90: number }> = {};
    ...
    for (const session of scoredSessions) {
      ...
      const gapCount = session?.dimensions?.filter((d) => d.score < 5).length ?? 0;
