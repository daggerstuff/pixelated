  ...
  for (const s of curriculumSteps ?? []) {
    try {
      ...
      if (parsed.trainee_id) {
        stepByTrainee.set(parsed.trainee_id, (stepByTrainee.get(parsed.trainee_id) ?? 0) + 1);
      }
    } catch {
      /* skip */
    }
...
describe("get_cohort_progress", () => {
  ...
  it("should return progress for all trainees in a cohort", async () => {
    ...
    expect(result.progress.find((p) => p.trainee_id === "t1")?.completed_steps).toBe(2);
    expect(result.progress.find((p) => p.trainee_id === "t2")?.completed_steps).toBe(0);
  });
