...
describe('list_cohorts', () => {
  ...
  it('should return cohorts grouped by cohort_id', async () => {
    ...
    expect(
      result.cohorts.find((c) => c.cohort_id === 'CBT-2026-01')?.trainee_count,
    ).toBe(2)
    expect(
      result.cohorts.find((c) => c.cohort_id === 'ADV-TRAUMA-01')
        ?.trainee_count,
    ).toBe(1)
    expect(
      result.cohorts.find((c) => c.cohort_id === 'ADV-TRAUMA-01')?.skill_level,
    ).toBe('ADVANCED')
  })
  ...
  it('should filter by cohort_type when provided', async () => {
    ...
    expect(searchMemoriesMock.mock.calls[0][0].tag_filter).toContain(
      'cohort_type:SKILL_LEVEL',
    )
  })
})
