...
const SCHEMA = z.object({
  ...
  cohort_id: z
    .string()
    .optional()
    .describe('Existing cohort ID. Omit to create a new cohort.'),
  name: z
    .string()
    .min(1)
    .optional()
    .describe(
      'Cohort name (e.g. "2026-Q3"). Required when creating a new cohort.',
    ),
  skill_level: z
    .enum(SKILL_LEVELS)
    .optional()
    ...
    if (!cohortId) {
      if (!input.name || !input.skill_level || !input.start_date) {
        return {
          error: true,
          message:
            'Creating a new cohort requires name, skill_level, and start_date.',
        }
      }
