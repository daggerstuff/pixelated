...
const SCHEMA = z.object({
  ...
  dimension: z
    .string()
    .optional()
    .describe("Specific rubric dimension to filter (e.g. rapport, boundaries)."),
});
...
export default defineTool({
  ...
  async execute(input: z.infer<typeof SCHEMA>) {
    ...
    const dimensionAggregates: Record<string, { mean: number; p10: number; p90: number }> = {};
    for (const s of scores) {
      for (const [dim, val] of Object.entries(s.dimension_scores)) {
        if (!dimensionAggregates[dim]) dimensionAggregates[dim] = { mean: 0, p10: 0, p90: 0 };
      }
    }
    for (const dim of Object.keys(dimensionAggregates)) {
      const vals = scores.map((s) => s.dimension_scores[dim] ?? 0).sort((a, b) => a - b);
    ...
    const overallAvg =
      scores.length > 0 ? scores.reduce((a, s) => a + s.avg_score, 0) / scores.length : 0;
