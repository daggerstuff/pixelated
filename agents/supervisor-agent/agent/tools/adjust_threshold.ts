...
const SCHEMA = z.object({
  threshold_type: z.enum(THRESHOLD_TYPES).describe("Which threshold to adjust."),
  new_value: z.union([z.number(), z.string()]).describe("New threshold value."),
  reason: z.string().min(1).max(500).describe("Why this threshold is being changed."),
  scope: z.string().optional().default("global").describe('Scope: "global" or "cohort:<id>".'),
});
