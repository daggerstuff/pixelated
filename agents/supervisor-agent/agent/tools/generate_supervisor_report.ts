...
const SCHEMA = z.object({
  cohort_id: z
    .string()
    .optional()
    .describe('Cohort to report on. Omit for program-wide.'),
  time_range: z
    .object({
  ...
  async execute(input: z.infer<typeof SCHEMA>) {
    ...
    if (input.cohort_id) {
      ...
      if (cohortMemories && cohortMemories.length > 0) {
        try {
          cohortData = JSON.parse(cohortMemories[0].content) as Record<
            string,
            unknown
          >
        } catch {
    ...
    const query = input.cohort_id
      ? `cohort_id:${input.cohort_id} score_record`
      : 'score_record'
    const tagFilter = input.cohort_id
    ...
    const stored = await storeMemory({
      ...
      tags: [
        'supervisor_report',
        ...(input.cohort_id ? [`cohort:${input.cohort_id}`] : []),
      ],
    })
