/**
 * Workers AI MCP — lazy connection to Cloudflare Workers AI.
 *
 * Connects on first tool call via `createLazyResource`, not at eve startup.
 * Inference endpoint: @cf/meta/llama-3.2-3b-instruct on Cloudflare Workers.
 */
...
const lazyClient = createLazyResource<Client>(async () => {
  ...
  const client = new Client({ name: "demo-qa-agent", version: "1.0.0" }, { capabilities: {} });
