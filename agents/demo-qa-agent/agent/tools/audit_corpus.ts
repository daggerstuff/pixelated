  ...
  try {
    ...
    const current = new Set<string>(
      Array.isArray(data.current_event_ids)
        ? (data.current_event_ids as unknown[]).filter((x): x is string => typeof x === "string")
        : [],
    );
    ...
    if (current.size > 0) return { current_event_ids: current, future_event_ids: future };
  } catch {
...
const EMOJI_RE = /[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{1F000}-\u{1F02F}\u{FE0F}]/u;

const NUMERIC_RE = /\b\d+(?:[.,]\d+)?(?:%|ms|k|K|M|GB|hrs?|days?|users?|emails?)?\b/;
...
function firstHot3gram(body: string, ceilings: Map<string, number>): string | null {
...
function scoreThread(emails: EmailRecord[]): number {
  ...
  for (const e of emails) {
    ...
    if (/\n(?:vp|head|director|lead|founder|ceo|cto|coo)\b/i.test(body)) signatureHits++;
...
function loadArtifact(path: string): CorpusArtifact {
  ...
  if (data && typeof data === "object") {
    const obj = data as Record<string, unknown>;
    const probe =
      asList(obj["chats"]) ?? asList(obj["generated_chat_bursts"]) ?? asList(obj["chat_bursts"]);
...
export default defineTool({
  ...
  inputSchema: z.object({
    ...
    chat_path: z
      ...
      .min(1)
      .optional()
      .describe("Optional path to generated_chat_bursts.json for chat-frequency checks."),
    gate_report_path: z
      .string()
      .min(1)
      .optional()
  ...
  async execute(
    ...
    const gate = loadGateRegistry(input.corpus_path, input.gate_report_path, observedIds);
