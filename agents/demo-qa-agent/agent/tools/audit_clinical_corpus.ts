...
const THERAPY_SLOP_RE = /\b(it sounds like|i hear that|that must be|i can see that|let's explore that|that sounds difficult)\b/i;
...
export default defineTool({
  description: "Audit a clinical book QA dataset (JSONL) for therapy slop and repetition.",
  inputSchema: z.object({
    corpus_path: z.string().describe("Absolute path to the jsonl file to audit"),
    clean_output_path: z.string().optional().describe("Optional path to write a cleaned jsonl file without slop/repeats"),
  }),
  async execute(input) {
    const lines = readFileSync(input.corpus_path, "utf8").split("\n").filter(Boolean);
