...
export default defineTool({
  ...
  async execute(input: ScoreThreadInput): Promise<ThreadScoreResult> {
    ...
    if (!model || emails.length === 0) {
      return emails.length === 0 ? { ...base, dimensions: [], total_score: 0, max_total: 0 } : base;
    }
...
function loadThread(path: string, threadId: string): EmailRecord[] {
  ...
  const list: EmailRecord[] = Array.isArray(data)
    ...
    : data && typeof data === "object"
      ? ((((data as Record<string, unknown>).emails ?? (data as Record<string, unknown>).data) as
          | EmailRecord[]
          | undefined) ?? [])
      : [];
  return (list as EmailRecord[]).filter((e) => e.thread_id === threadId);
}
        ...
        ? parsed.dimensions.map((d: unknown) => {
            ...
            return {
              name: DIMENSIONS.includes(dd.name as (typeof DIMENSIONS)[number])
                ...
                : "unknown",
              score: typeof dd.score === "number" ? Math.max(0, Math.min(10, dd.score)) : 0,
              max_score: 10,
