...
export default defineTool({
  ...
  async execute(input: { corpus_path: string; target_count: number }): Promise<CurateResult> {
    ...
    const candidates = [...byThread.entries()]
      .map(([tid, msgs]) => {
        ...
        const senders = new Set(sorted.map((m) => m.sender ?? "").filter(Boolean));
        ...
        return {
          ...
          score: sorted.length * 2 + senders.size * 3 + Math.min(bodyLen / 500, 10),
        };
      })
