  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <ShieldAlert
          className={`h-4 w-4 ${data.trend === 'decreasing' ? 'text-green-500' : data.trend === 'stable' ? 'text-amber-500' : 'text-red-500'}`}
        />
      </CardHeader>
      <CardContent>
          ...
          <div className="space-y-1.5">
            {data.byPattern.map((p) => (
              <div key={p.pattern} className="flex items-center gap-2 text-xs">
                <span className="text-muted-foreground w-16">{p.pattern}</span>
                <div className="bg-muted h-1.5 flex-1 overflow-hidden rounded-full">
                  <div
                    className="bg-destructive/70 h-full rounded-full"
                    style={{
                      width: `${(p.count / data.totalIntercepted) * 100}%`,
                    }}
                  />
                </div>
                <span className="w-6 text-right font-medium">{p.count}</span>
              </div>
            ))}
          </div>
        </div>
    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <div
              className={`h-4 w-4 rounded-full ${data.chainValid ? 'bg-green-500' : 'bg-red-500'}`}
            />
            <span
              className={`font-semibold ${data.chainValid ? 'text-green-600' : 'text-red-600'}`}
            >
              {data.chainValid
          ...
          <div className="text-muted-foreground space-y-1 border-t pt-2 text-xs">
            <p>
              SHA-256 hash chain:{' '}
              <Badge
                variant={data.chainValid ? 'success' : 'destructive'}
                className="text-[10px]"
              >
                {data.chainValid ? 'Valid' : 'Invalid'}
              </Badge>
            </p>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-sm">
          Inference Latency (WebSocket → Celery → Response)
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="h-64">
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-1">
          <PHICard data={data.phiInterceptions} />
        </div>
        <div className="lg:col-span-1">
          <AuditChainCard data={data.auditChain} />
        </div>
