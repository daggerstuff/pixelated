...
## 2026-04-18 - Fix XSS in Head.astro | Vulnerability: Unescaped HTML control chars injected via set:html with JSON.stringify | Learning: JSON.stringify alone leaves inline scripts vulnerable to XSS if user input contains script tags | Prevention: Always append .replace(/</g, '<').replace(/>/g, '>') to JSON.stringify when injecting into set:html
