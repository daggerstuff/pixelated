...
| Prevention: Always use replace(/</g, "\u003c").replace(/>/g, "\u003e") for JSON data within set:html
...
| Learning: JSON.stringify does not escape HTML control characters like < or > which can allow breakout of script tags leading to XSS
| Prevention: Always escape HTML control characters in JSON.stringify when injecting into script tags, e.g., replacing < with \u003c.
