...
case, and passes verification using localized execution `npx vitest run <file>`
...
Pattern: Use `vi.useFakeTimers()` with a fixed system time to make relative-time
assertions deterministic

Action: Call `vi.setSystemTime(new Date('2023-05-15T10:00:00Z'))` in
`beforeAll`, then assert `formatDate(date, { relative: true })` returns expected
strings like "5 minutes ago" or "1 day ago". Restore with `vi.useRealTimers()`
in `afterAll`
