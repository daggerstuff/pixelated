...
Pattern: When testing Express asyncHandler wrappers the wrapped function executes asynchronously but the wrapper itself may return a non-promise or a promise that is not easily awaited in the test
...
## 2026-04-18 - QA 
Add test for template edge case
...
## 2026-04-19 - QA 
Add test for isPartialBiasDashboardSummary
Pattern: Finding untested utility functions like type guards and creating new targeted test files specifically for them under `__tests__` directory
...
## 2026-04-27 - QA 
Add test for analyzeTherapeuticTechniques edge case
...
## 2026-04-28 - QA 
Add test for getRecommendedScenario edge case
...
## 2026-04-29 - QA 
Fix privacy test ts jsdom environment
...
## 2026-04-30 - QA 
Add test for checkBrowserCompatibility edge case
...
## 2026-05-09 - QA 
Add test for formatDate relative edge case
Pattern: Use `vi.useFakeTimers()` with a fixed system time to make relative-time assertions deterministic
Action: Call `vi.setSystemTime(new Date('2023-05-15T10:00:00Z'))` in `beforeAll` then assert `formatDate(date, { relative: true })` returns expected strings like 5 minutes ago or 1 day ago
Restore with `vi.useRealTimers()` in `afterAll`