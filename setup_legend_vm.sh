...
start_dbs() {
  ...
  export REDIS_PASSWORD=$(grep -E "^REDIS_PASSWORD=" .env | cut -d'=' -f2-)
...
install_deps() {
  ...
  if [ -d "ai-services" ]; then
    ...
    cd "ai-services"
    sudo -u "$R_USER" bash -c 'export PATH="$HOME/.local/bin:$PATH"; uv venv && uv pip install -r requirements.txt'
    cd ..
  fi
