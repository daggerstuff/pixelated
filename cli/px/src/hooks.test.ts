...
function exec(cmd: string, cwd: string): { stdout: string; stderr: string; status: number | null } {
  const result = spawnSync('sh', ['-c', cmd], { cwd, encoding: 'utf-8', timeout: 15000 });
...
function runHookCmd(event: string, cwd: string, extraArgs: string[] = []): { stdout: string; stderr: string; status: number | null } {
...
describe('hook scripts: fail-open verification', () => {
  ...
  it('pr-merge hook — non-matching filter (no src/session/**) → silent exit 0', () => {
    ...
    const result = runHookCmd('pr-merge', tempRepo, ['--pr', 'https://github.com/test/repo/pull/2']);
    expect(result.status).toBe(0);
  });
