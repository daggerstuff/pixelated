...
const DEFAULT_CONFIG = {
  agents: {
    ...
    content: {
      ...
      tools: ['audit_corpus', 'audit_clinical_corpus', 'score_thread', 'curate_showcase', 'gate_injection'],
      async: false,
      timeout: 30000,
    },
    qa: {
      endpoint: 'http://qa-agent.pixelated.svc.cluster.local:2000',
      tools: ['score_session', 'fetch_sessions', 'detect_emotional_patterns', 'flag_training_gap', 'summarize_cohort', 'generate_report'],
      async: true,
      timeout: 30000,
    },
    pipeline: {
      endpoint: 'http://pipeline-agent.pixelated.svc.cluster.local:2000',
      tools: ['curate_dataset', 'run_training', 'run_evaluation', 'promote_to_staging', 'promote_to_production', 'rollback_model', 'check_pipeline_health', 'evaluate_pipeline_review'],
      async: true,
      timeout: 30000,
    },
    intake: {
      endpoint: 'http://intake-agent.pixelated.svc.cluster.local:2000',
      tools: ['register_trainee', 'assign_cohort', 'list_cohorts', 'get_trainee_status', 'get_cohort_progress', 'record_curriculum_step'],
      async: false,
      timeout: 30000,
    },
    supervisor: {
      endpoint: 'http://supervisor-agent.pixelated.svc.cluster.local:2000',
      tools: ['query_cohort_trends', 'compare_trainees', 'list_flagged_sessions', 'generate_supervisor_report', 'query_trainee_timeline', 'adjust_threshold', 'adjust_trainee_status', 'notify_slack'],
      async: false,
      timeout: 30000,
    },
    session: {
      endpoint: 'http://session-agent.pixelated.svc.cluster.local:2000',
      tools: ['start_session', 'process_message', 'analyze_emotion', 'analyze_pace', 'check_clinical_boundary', 'validate_response', 'hydrate_session', 'save_session', 'conclude_session'],
      async: false,
      timeout: 30000,
    },
  },
  ...
  hooks: {
    'pre-commit': { agent: 'content', tool: 'audit_clinical_corpus', filter: 'scenarios/**' },
    'pre-push': { agent: 'advisor', tool: 'review' },
    'post-merge': { agent: 'pipeline', tool: 'check_pipeline_health', async: true },
    'pr-open': { agent: 'advisor', tool: 'review', async: true },
    'pr-merge': { agent: 'qa', tool: 'score_session', filter: 'src/session/**', async: true },
  },
};
  ...
  program
    ...
    .action(() => {
      ...
      try {
        ...
        const toolCount = (Object.values(config.agents) as AgentConfig[]).reduce(
          (sum, a) => sum + a.tools.length,
          0,
        );
