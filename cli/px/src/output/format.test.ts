...
describe('formatHealthResult', () => {
  it('shows ok/down/error counts and per-agent status', () => {
    const results = [
      { agent: 'advisor', endpoint: 'http://advisor:2000', status: 'ok' as const },
      { agent: 'content', endpoint: 'http://content:2000', status: 'down' as const, detail: 'HTTP 503' },
      { agent: 'qa', endpoint: 'http://qa:2000', status: 'error' as const, detail: 'ECONNREFUSED' },
    ];
