...
function runCli(args: string[], cwd: string): { stdout: string; stderr: string; status: number | null } {
  const result = spawnSync('node', [join(__dirname, '..', 'dist', 'index.js'), ...args], {
    cwd,
    encoding: 'utf-8',
    timeout: 15000,
  });
...
beforeAll(async () => {
  ...
  spawnSync('git', ['config', 'user.email', 'test@test.com'], { cwd: tempRepo, encoding: 'utf-8' });
  spawnSync('git', ['config', 'user.name', 'Test'], { cwd: tempRepo, encoding: 'utf-8' });
  // Disable global hooks that would interfere with test commits
  spawnSync('git', ['config', 'core.hooksPath', ''], { cwd: tempRepo, encoding: 'utf-8' });
  ...
  const config = {
    ...
    slack: { webhook: 'https://hooks.slack.com/services/fake', channel: '#test-results' },
    hooks: {
  ...
  spawnSync('git', ['commit', '-m', 'init'], { cwd: tempRepo, encoding: 'utf-8' });
});

afterAll(() => {
  if (mockServer) {
    try { mockServer.send('shutdown'); } catch { /* ignore */ }
...
describe('integration: CLI against mock eve server', () => {
  ...
  it('px hook pre-push --dry-run — prints payload with git context', () => {
    ...
    spawnSync('git', ['add', '.'], { cwd: tempRepo, encoding: 'utf-8' });
    spawnSync('git', ['commit', '-m', 'add test file'], { cwd: tempRepo, encoding: 'utf-8' });
  ...
  it('px hook pre-commit --dry-run — filter excludes non-scenarios files', () => {
    ...
    spawnSync('git', ['add', 'regular-code.ts'], { cwd: tempRepo, encoding: 'utf-8' });
