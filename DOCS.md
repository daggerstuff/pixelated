...
> The `docs/` directory is a **git submodule**. If it appears empty after
> cloning, run:
>
> ```bash
...
This index maps all 27 subdirectories inside `docs/` so you can find what you
need without browsing blindly.
...
The `docs/` submodule uses [Fuma Docs](https://fumadocs.vercel.app/) for its
documentation portal. The configuration lives in
[`docs/docs.json`](docs/docs.json). To serve locally:
