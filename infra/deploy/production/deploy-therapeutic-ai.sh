...
deploy_config() {
    ...
    local database_url="${THERAPEUTIC_DATABASE_URL:-}"
    local redis_url="${THERAPEUTIC_REDIS_URL:-redis://redis:6379/0}"
    local jwt_secret="${JWT_SECRET:-}"
    local sentry_dsn="${SENTRY_DSN:-}"
    
    if [[ -z "$database_url" ]]; then
        log_error "THERAPEUTIC_DATABASE_URL is not set. Export it or add to .env before deploying."
        exit 1
    fi
    if [[ -z "$jwt_secret" ]]; then
        log_error "JWT_SECRET is not set. Export it or add to .env before deploying."
        exit 1
    fi
    if [[ -z "$sentry_dsn" ]]; then
        log_error "SENTRY_DSN is not set. Export it or add to .env before deploying."
        exit 1
    fi
...
data:
  ...
  sentry-dsn: $(echo -n "$sentry_dsn" | base64)
EOF
...
deploy_microservices() {
    ...
    # API Gateway
    deploy_service "therapi.pixelatedempathy.com" 3 "500m" "1Gi" "1000m" "2Gi" "/health"
...
spec:
  tls:
  - hosts:
    - therapi.pixelatedempathy.com
    secretName: therapeutic-ai-tls
  rules:
  - host: therapi.pixelatedempathy.com
    http:
      paths:
      - path: /
        pathType: Prefix
        backend:
          service:
            name: therapi.pixelatedempathy.com
            port:
              number: 80
EOF
...
health_check() {
    ...
    kubectl run health-check-pod \
        ...
        http://therapi.pixelatedempathy.com/health || true
...
main() {
    ...
    log_info "• API Endpoint: https://therapi.pixelatedempathy.com"
