...
Selectively extracts Python and Linux shell (`bash`/`sh`) fenced code blocks
from markdown files into `scripts/`.
...
4. Inline fences are replaced with a short run reference (`python scripts/...`
   or `bash scripts/...`)
