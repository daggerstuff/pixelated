...
from skillrevise.benchmarks.skillsbench import (
    SkillsBenchTaskLoader,
    build_family_index,
    select_sibling_tasks,
)
...
from skillrevise.core.models import ExecutionTrace, TaskSpec, TrajectoryEvent
...
def _apply_budget_override(tasks: list[TaskSpec], budget_seconds: int | None) -> list[TaskSpec]:
...
def _resolve_utility_weights(args: argparse.Namespace) -> UtilityWeights:
...
def _experiment_config(args: argparse.Namespace, weights: UtilityWeights) -> dict[str, Any]:
    ...
    return {
        ...
        "principle_embedding_model": getattr(
            args, "principle_embedding_model", "qwen/qwen3-embedding-4b"
        ),
...
def _load_baseline_trace_cache(path: str | Path) -> dict[str, ExecutionTrace]:
    ...
    if not isinstance(items, list):
        raise SystemExit(
            "--baseline-run must point to a baseline-only run JSON containing baseline_results."
        )
...
def _execution_trace_from_json(data: dict[str, Any]) -> ExecutionTrace:
    return ExecutionTrace(
        ...
        events=[
            _trajectory_event_from_json(event)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run SkillRevise skill generation, diagnosis, and revision."
    )
    parser.add_argument("tasks", help="Path to a JSON file containing task specs.")
    parser.add_argument(
        "--output", default="skillrevise_run.json", help="Where to save the run artifact."
    )
    parser.add_argument(
        "--summary-output", help="Optional path for a compact summary JSON artifact."
    )
    parser.add_argument(
        "--task-id", action="append", help="Run only the given task id. Can be repeated."
    )
    parser.add_argument(
        "--family", action="append", help="Run only tasks from the given family. Can be repeated."
    )
    parser.add_argument("--limit", type=int, help="Run at most this many tasks after filtering.")
    parser.add_argument(
        "--max-revisions", type=int, default=1, help="Maximum number of revise/re-evaluate rounds."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--repeat", type=int, default=1, help="Repeat each selected task this many times."
    )
    parser.add_argument(
        "--budget-seconds", type=int, help="Override each task timeout/budget in seconds."
    )
    parser.add_argument(
        "--baseline-only",
        action="store_true",
    ...
    parser.add_argument(
        "--utility-alpha", type=float, help="Override same-task reward/success gain weight."
    )
    ...
    parser.add_argument(
        "--utility-lambda", type=float, help="Override interference penalty weight."
    )
    parser.add_argument(
        "--manifest-kind",
    ...
    parser.add_argument(
        "--workspace-root", help="Workspace root used to resolve relative repo paths."
    )
    parser.add_argument(
        "--harness-command", help="External harness command for real benchmark execution."
    )
    ...
    parser.add_argument(
        "--artifacts-root", help="Optional artifact directory for real benchmark runs."
    )
    parser.add_argument(
        "--max-heldout",
        type=int,
        default=None,
        help="Maximum number of sibling tasks used for transfer.",
    )
    parser.add_argument(
        "--author-mode",
        choices=(
    ...
    parser.add_argument(
        "--principle-bank", help="Optional JSON repair-principle bank for structured LLM revision."
    )
    parser.add_argument(
        "--principle-limit",
        type=int,
    ...
    parser.add_argument(
        "--principle-keyword-weight", type=float, default=0.5, help="RRF weight for BM25 retrieval."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--llm-command",
        help="External LLM command that reads prompt from stdin and writes response to stdout.",
    )
    parser.add_argument(
        "--llm-timeout", type=int, default=600, help="Timeout for each LLM command call."
    )
    parser.add_argument(
        "--initial-skill", help="Optional Markdown skill file to use as the initial skill."
    )
    parser.add_argument(
    ...
    if args.baseline_only:
        ...
        write_json(
            args.output,
            {
                ...
                "summary": summary,
                "baseline_results": [
                    {"task": to_jsonable(task), "no_skill": to_jsonable(trace)}
                    for task, trace in baseline_items
                ],
            },
        )
        if args.summary_output:
    ...
    llm = (
        CommandLLMClient(args.llm_command, timeout_seconds=args.llm_timeout)
        if args.llm_command
        else None
    )
    if (
    ...
    loop = HarnessLoop(
        ...
        runner=PairedRunner(
            adapter,
            weights=weights,
            baseline_traces=_load_baseline_trace_cache(args.baseline_run)
