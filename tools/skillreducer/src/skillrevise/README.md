...
Vendored implementation of **SkillRevise** (Liu et al.) — execution-grounded
skill revision.
...
SkillRevise improves skill **behavior** from execution traces. SkillReducer
improves skill **token cost**.
...
See also:
[PAPER_DETAIL.md §21](../../PAPER_DETAIL.md#21-skillrevise--trace-conditioned-skill-revision)
...
LLM-authored skills often look correct but fail at runtime. SkillRevise runs a
**trace-conditioned** loop:
...
4. **Revise** the skill conditioned on that diagnosis (optional principle
   memory)
...
The `benchmarks/` package is **only for paper/eval harnesses** (SkillsBench,
SkillLearnBench, ALFWorld).  
...
**Not vendored:** large upstream benchmark `data/` bundles. Clone
...
**Not vendored:** large upstream benchmark `data/` bundles — see
[Benchmarks (eval only)](#benchmarks-eval-only).
...
`skillreducer revise` only forwards arguments to `skillrevise.cli` — it does
**not** change `audit` / `reduce` / `agent`:
...
- **LLM mode:** use SkillRevise env vars / flags documented by upstream and
  `skillrevise --help` / `skillrevise-llm --help` (e.g.
  author/diagnosis/revision modes).
- **Heuristic mode:** works without a remote LLM for many paths
  (`--baseline-only`, heuristic diagnosis/revision).
- **Outputs:** JSON reports via `--output`, optional `--summary-output` and
