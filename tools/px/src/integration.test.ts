...
function runCli(
  ...
  const result = spawnSync(
    'node',
    [join(__dirname, '..', 'dist', 'index.mjs'), ...args],
    {
      cwd,
      encoding: 'utf-8',
      timeout: 15000,
...
beforeAll(async () => {
  ...
  writeFileSync(
    join(tempRepo, 'agents', 'px.config.json'),
...
describe('integration: CLI against mock eve server', () => {
  ...
  it('px advisor review --timeout 1 — timeout exits with error (interactive)', () => {
    ...
    writeFileSync(
      join(tempRepo, '.px', 'config.json'),
      JSON.stringify(slowConfig),
    )
