...
describe('matchGlob', () => {
  ...
  describe('** wildcard (multi-segment)', () => {
    it('matches files under directory', () => {
      expect(
        matchGlob(['scenarios/foo.yml', 'src/bar.ts'], 'scenarios/**'),
      ).toBe(true)
    })

    it('matches nested files', () => {
      expect(matchGlob(['scenarios/sub/dir/foo.yml'], 'scenarios/**')).toBe(
        true,
      )
    })
    ...
    it('matches multiple files if one matches', () => {
      expect(
        matchGlob(['src/bar.ts', 'scenarios/foo.yml'], 'scenarios/**'),
      ).toBe(true)
    })
  })
  ...
  describe('* wildcard (single segment)', () => {
    ...
    it('does not match across directory boundary', () => {
      expect(matchGlob(['scenarios/sub/foo.yml'], 'scenarios/*.yml')).toBe(
        false,
      )
    })
  })
  ...
  describe('real-world filters', () => {
    ...
    it('does not match src/session/** for non-session files', () => {
      expect(
        matchGlob(['src/cli/index.ts', 'README.md'], 'src/session/**'),
      ).toBe(false)
    })
  })
})
