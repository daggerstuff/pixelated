...
const TOOL_STUBS: Record<string, (body: unknown) => unknown> = {
  ...
  review: () => ({
    findings: [
      ...
      {
        severity: 'medium',
        message: 'Consider extracting helper — function exceeds 50 lines',
  ...
  list_flagged_sessions: () => ({
    flagged: [{ session_id: 'sess_042', reason: 'low_empathy' }],
  }),
  ...
  program
    ...
    .action(async (opts: ServeOptions) => {
      ...
      if (filteredAgents.length === 0) {
        console.error(
          pc.red(`px serve: agent "${opts.agent}" not found in config`),
        )
        process.exitCode = 1
        return
      }
      ...
      server.listen(port, () => {
        ...
        console.log(
          pc.green(
            `px serve: stub server listening on http://localhost:${port}`,
          ),
        )
        ...
        console.log(
          pc.gray(
            `  session: POST /eve/v1/session, GET /eve/v1/session/:id/stream`,
          ),
        )
        console.log(
          pc.gray(`  set PX_LOCAL=1 to route px commands to localhost`),
        )
      })
