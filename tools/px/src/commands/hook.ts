...
const GIT_HOOK_EVENTS = ['pre-commit', 'pre-push', 'post-merge'] as const
...
async function runInstall(opts: InstallOptions): Promise<void> {
  ...
  try {
    ...
    console.error(
      pc.red(
        `px hook install: failed to load config — ${e instanceof Error ? e.message : 'unknown'}`,
      ),
    )
    process.exitCode = 1
    return
  ...
  if (installableEvents.length === 0) {
    console.log(
      pc.yellow(
        'px hook install: no git hooks configured (pre-commit, pre-push, post-merge)',
      ),
    )
    return
  }
  ...
  if (!gitHooksDir) {
    console.error(
      pc.red('px hook install: could not locate .git/hooks directory'),
    )
    process.exitCode = 1
    return
  }
  ...
  for (const event of installableEvents) {
    ...
    if (existsSync(hookPath) && !opts.force) {
      ...
      if (!content.includes('px: managed hook')) {
        console.log(
          pc.yellow(
            `  skipped ${event} — file exists (use --force to overwrite)`,
          ),
        )
        continue
      }
    ...
    console.log(
      pc.green(`  installed ${event} → ${hookConfig.agent}.${hookConfig.tool}`),
    )
  }
