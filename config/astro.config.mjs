...
const isBuildCommand =
  process.argv.includes("build") || process.env.CI === "true" || !!process.env.VERCEL;
...
function getChunkName(id) {
  ...
  if (normalizedId.includes("/src/components/")) {
    ...
    const componentExtension = componentImport ? path.extname(componentImport) : "";
  ...
  if (normalizedId.includes("/recharts") || normalizedId.includes("/react-chartjs-2")) {
    return "charts-vendor";
  }
  if (normalizedId.includes("/chart.js") || normalizedId.includes("/chart.js/")) {
    ...
    if (segments.length > 0 && segments[0] === ".pnpm" && segments.length > 1) {
      const packageName =
        segments[1].includes("@") && segments[1].includes("/")
          ...
          : segments[1];
      const sanitizedName = packageName.replace("@", "at-").replace(/[^a-zA-Z0-9-]/g, "-");
      ...
      ...(hasSentryDSN
        ? [
            ...createScopedSentryVitePlugins({
              ssr: false,
              assets: ["./dist/client/_astro/**/*.js", "./dist/client/_astro/**/*.js.map"],
